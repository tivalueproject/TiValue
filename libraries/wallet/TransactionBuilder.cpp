...
void  TransactionBuilder::set_wallet_implementation(std::unique_ptr<TiValue::wallet::detail::WalletImpl>& wimpl)
{
    _wimpl = wimpl.get();
}
...
TransactionBuilder& TransactionBuilder::deposit_asset(const TiValue::wallet::WalletAccountEntry& payer,
    const TiValue::blockchain::AccountEntry& recipient,
    const Asset& amount,
    const string& memo,
    ...
    const string& sub_account)
{
    try {
        ...
        if (memo.size() > TIV_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE)
        ...
        if (sub_account != "")
        {
            ...
            trx.sub_account = sub_account;
            trx.sub_inport_asset = amount;
        }
...
TransactionBuilder& TransactionBuilder::deposit_asset_with_escrow(const TiValue::wallet::WalletAccountEntry& payer,
    const TiValue::blockchain::AccountEntry& recipient,
    const TiValue::blockchain::AccountEntry& escrow_agent,
    ...
    const string& sub_account)
{
    try {
        ...
        // Don't automatically truncate memos as long as users still depend on them via deposit ops rather than mail
        if (memo.size() > TIV_BLOCKCHAIN_MAX_MEMO_SIZE)
            FC_CAPTURE_AND_THROW(memo_too_long, (memo));
    ...
    )
{
    try {
        ...
        if (market_fee >= 0 || market_fee <= TIV_BLOCKCHAIN_MAX_UIA_MARKET_FEE)
            actual_market_fee = TIV_BLOCKCHAIN_MAX_UIA_MARKET_FEE * market_fee;
