...
namespace TiValue {
    namespace wallet {
		...
		void detail::WalletImpl::scan_file_store()
		{
			//scan_file_upload
			const auto pending_state = _blockchain->get_pending_state();
			
			const auto scan_upload_requests = [&](const UploadRequestEntry& upload_request_entry)
			{
				auto uploader = upload_request_entry.id.uploader;

				//check whether the owner of the contract is in wallet account list
				const oWalletKeyEntry key_entry = _wallet_db.lookup_key(Address(uploader));
				if (key_entry.valid() && key_entry->has_private_key())
				{
					if(_upload_request_entrys.find(Address(uploader))== _upload_request_entrys.end())

					_upload_request_entrys[Address(uploader)].insert(upload_request_entry);
					//store walletDb
					_wallet_db.store_upload_request(upload_request_entry);
				}
			};
			_upload_request_entrys.clear();
			_blockchain->scan_upload_request(scan_upload_requests);

			//scan_store_request
			const auto  scan_store_request=[&](const StoreRequestEntry& store_request_entry)
			{
				
				auto node_end = store_request_entry.store_request.end();
				bool my_request = false;
				auto node = store_request_entry.store_request.begin();
				for (; node!=node_end;node++)
				{
					const oWalletKeyEntry key_entry = _wallet_db.lookup_key(Address(node->second));
					if (key_entry.valid() && key_entry->has_private_key())
					{
						break;
					}
				...
				//{
					...
					bool found = false;
					while (it != _upload_request_entrys.end())
					{
			...
			_blockchain->scan_pieced_saved(scan_pieced_saved);
			const auto  scan_store_reject = [&](const StoreRejectEntry& entry)
			{
				auto info_range=entry.info.equal_range(file_store_node);
				for (auto it = info_range.first; it != info_range.second; it++)
				{
			...
			{
				for (auto fetcher : entry.fetcher)
				{
					const oWalletKeyEntry key_entry = _wallet_db.lookup_key(Address(fetcher));
					if (key_entry.valid())
					{
						my_access_info[entry.file_id].insert(fetcher);
					}
				}
			};
			my_access_info.clear();
			_blockchain->scan_enable_access(scan_enable_access);
		}
