...
using namespace TiValue::wallet::detail;
    ...
    bool bNeedcreate)
{
    ...
    // 
        ...
        {
            if (store_entry && transaction.sub_account != "")
            {
                SubTrxidBalance sub_entry;
                ...
                for (const auto item : pretty_trx.ledger_entries)
                {
                    ...
                    {
                        sub_entry.from_account = item.from_account;
                        break;
                    }
                }
                string strToAccount;
                string strSubAccount;
                self->accountsplit(transaction.sub_account, strToAccount, strSubAccount);
                auto blockchain_trx = _blockchain->get_transaction(transaction_id);
                bool bDiffDeop = false;
                if (blockchain_trx.valid())
                {
                    ShareType tiv_amount = 0;
                    string preowner;
                    int iLoop = 0;
                ...
                sub_entry.create_time = transaction_entry->created_time;
                sub_entry.sub_account = transaction.sub_account;
                sub_entry.block_num = transaction_entry->block_num;
                transaction_entry->trx.sub_account = transaction.sub_account;
                transaction_entry->trx.sub_inport_asset = transaction.sub_inport_asset;
                sub_entry.trx_id = transaction.id();
                    sub_entry.asset_trx = Asset(tiv_amount, 0);
                    {
                        writelock wlock(m_mutex_for_sub);
                    }
                }
            }
        }
...
bool TiValue::wallet::detail::WalletImpl::scan_withdraw_contract(const WithdrawContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
...
bool TiValue::wallet::detail::WalletImpl::scan_ondestroycontract_withdraw(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		const auto amount = Asset(op.amount); // Always base asset
...
bool TiValue::wallet::detail::WalletImpl::scan_ondestroycontract_deposit(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try {
...
bool TiValue::wallet::detail::WalletImpl::scan_deposit_contract(const DepositContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try{
		auto entry = LedgerEntry();
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        if (!asset_symbol.empty() && asset_symbol != TIV_BLOCKCHAIN_SYMBOL)
        {
            try
            {
...
PrettyTransaction		Wallet::to_pretty_trx(const TiValue::blockchain::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    pretty_trx.trx_type = TiValue::blockchain::TransactionType::normal_transaction;
    ...
    for (const auto & op : trx_entry.trx.operations)
    {
		...
		case withdraw_pay_op_type:
		{
            ...
            pretty_trx.trx_type = TiValue::blockchain::TransactionType::withdraw_pay_transaction;
		}
		default:
			break;
		}
    ...
    for (const auto& op : trx.operations)
    {
        ...
        case register_account_op_type:
        {
            ...
            if (register_op.delegate_pay_rate > 100)
            {
                ...
                pretty_trx.trx_type = TiValue::blockchain::TransactionType::register_account_transaction;
            }
            else
            {
                pretty_entry.memo = "register delegate " + account_name_entry->name; // Can't tell if initially registered as a delegate
                pretty_trx.trx_type = TiValue::blockchain::TransactionType::register_delegate_transaction;
            }
		...
		case update_account_op_type:
		{
            ...
            //所以现在只能简单粗暴得用一个1000TIC来进行划分，只要是大于1000TIC都认为是升级代理
            if (total_fee <= Asset( 1000 * TIV_BLOCKCHAIN_PRECISION ))
            {
                pretty_entry.memo = "update " + account_name_entry->name;
                pretty_trx.trx_type = TiValue::blockchain::TransactionType::update_account_transaction;
            }
            else
            {
                pretty_entry.memo = "upgrade " + account_name_entry->name;
                pretty_trx.trx_type = TiValue::blockchain::TransactionType::upgrade_account_transaction;
            }
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const TiValue::blockchain::TransactionEntry& trx_entry) const
{
    PrettyContractTransaction pretty_trx;
    SignedTransaction result_trx = trx_entry.trx;

    FC_ASSERT((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type == TiValue::blockchain::OperationTypeEnum::transaction_op_type) , 
        "not result contract transaction");
    SignedTransaction orig_trx = trx_entry.trx.operations[0].as<TiValue::blockchain::TransactionOperation>().trx;
    ...
    FC_ASSERT((orig_trx.operations.size() > 0) && 
        ((orig_trx.operations[0].type >= fc::enum_type<uint8_t, TiValue::blockchain::OperationTypeEnum>(register_contract_op_type))
            && (orig_trx.operations[0].type <= fc::enum_type<uint8_t, TiValue::blockchain::OperationTypeEnum>(transfer_contract_op_type))), "invalid original contract transaction");


    TiValue::blockchain::OperationTypeEnum contract_op_type = orig_trx.operations[0].type;
    ...
    if (contract_op_type == TiValue::blockchain::OperationTypeEnum::register_contract_op_type)
    {
        pretty_trx.trx_type = TiValue::blockchain::TransactionType::register_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        TiValue::blockchain::RegisterContractOperation register_contract_op = orig_trx.operations[0].as<TiValue::blockchain::RegisterContractOperation>();
        Address contract_owner = TiValue::blockchain::Address(register_contract_op.owner);
        ...
        to_contract_ledger_entry.amount = Asset(TIV_DEFAULT_CONTRACT_MARGIN, 0);
        ...
        for (const auto& op : result_trx.operations)
        {
            ...
            if (op.type == fc::enum_type<uint8_t, TiValue::blockchain::OperationTypeEnum>(balances_withdraw_op_type))
            {
        ...
        if (register_success)
        {
            to_contract_ledger_entry.fee = Asset(all_cost - TIV_DEFAULT_CONTRACT_MARGIN, 0);
    ...
    {
        pretty_trx.trx_type = TiValue::blockchain::TransactionType::upgrade_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        TiValue::blockchain::UpgradeContractOperation upgrade_contract_op = orig_trx.operations[0].as<TiValue::blockchain::UpgradeContractOperation>();
    ...
    else if (contract_op_type == TiValue::blockchain::OperationTypeEnum::destroy_contract_op_type)
    {
        pretty_trx.trx_type = TiValue::blockchain::TransactionType::destroy_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        TiValue::blockchain::DestroyContractOperation destroy_contract_op = orig_trx.operations[0].as<TiValue::blockchain::DestroyContractOperation>();
    ...
    else if (contract_op_type == TiValue::blockchain::OperationTypeEnum::call_contract_op_type)
    {
        pretty_trx.trx_type = TiValue::blockchain::TransactionType::call_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        TiValue::blockchain::CallContractOperation call_contract_op = orig_trx.operations[0].as<TiValue::blockchain::CallContractOperation>();
    ...
    else if (contract_op_type == TiValue::blockchain::OperationTypeEnum::transfer_contract_op_type)
    {
        pretty_trx.trx_type = TiValue::blockchain::TransactionType::transfer_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        TiValue::blockchain::TransferContractOperation transfer_contract_op = orig_trx.operations[0].as<TiValue::blockchain::TransferContractOperation>();
        Address contract_depositer = TiValue::blockchain::Address(transfer_contract_op.from);
