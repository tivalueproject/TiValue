    ...
    namespace wallet {
        ...
        namespace detail {
            ...
            void WalletImpl::state_changed(const PendingChainStatePtr& state)
            {
                ...
                if (_blockchain->get_head_block_num() < last_unlocked_scanned_number)
                {
                    ...
                    self->set_last_scanned_block_number_for_tiv(_blockchain->get_head_block_num());
                }
            }
            ...
            void WalletImpl::block_applied(const BlockSummary& summary)
            {
                ...
                if ((t - _blockchain->get_head_block_timestamp()) <= fc::seconds(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            */
                ...
                try {
                    ...
                    try
                    {
                        ...
                        while (current_block_num <= head_block_num && count < limit && !_scan_in_progress.canceled())
                        {
                            ...
                            self->set_last_scanned_block_number_for_tiv(prev_block_num);
                            ++current_block_num;
            ...
            void WalletImpl::upgrade_version()
            {
                ...
                if (current_version > TIV_WALLET_VERSION)
                {
                    FC_THROW_EXCEPTION(unsupported_version, "Wallet version newer than client supports!",
                        ("wallet_version", current_version)("supported_version", TIV_WALLET_VERSION));
                }
                else if (current_version == TIV_WALLET_VERSION)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        self->set_transaction_fee(Asset(TIV_WALLET_DEFAULT_TRANSACTION_FEE));
                        self->set_transaction_imessage_fee_coe(TIV_BLOCKCHAIN_MIN_MESSAGE_FEE_COE);
                        self->set_transaction_imessage_soft_max_length(TIV_BLOCKCHAIN_MAX_SOFT_MAX_MESSAGE_SIZE);
                    }

                    if (current_version < 106)
                    {
                        self->set_transaction_expiration(TIV_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC);
                    }
                    ...
                    {
                        self->set_last_scanned_block_number_for_tiv(0);
                    }
                    if (_unlocked_upgrade_tasks.empty())
                    {
                        self->set_version(TIV_WALLET_VERSION);
                ...
                const string& brainkey)
            {
                try {
                    ...
                    if (password.size() < TIV_WALLET_MIN_PASSWORD_LENGTH)
             ...
             *  returned will contain no more than TIV_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
             */
            SlateEntry WalletImpl::get_delegate_slate(const VoteStrategy strategy)const
            {
                ...
                {
                    slate_size = std::min<size_t>(TIV_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size());
                }
                else if (strategy == vote_random)
                {
                    slate_size = std::min<size_t>(TIV_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size());
                    slate_size = rand() % (slate_size + 1);
                }
                else if (strategy == vote_recommended && for_candidates.size() < TIV_BLOCKCHAIN_MAX_SLATE_SIZE)
                {
                    ...
                    while (for_candidates.size() < TIV_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0)
                    {
                        int best_rank = 0;
                ...
                FC_ASSERT(entry.slate.size() <= TIV_BLOCKCHAIN_MAX_SLATE_SIZE);
            ...
            const string& brainkey)
        {
            try {
                ...
                try
                {
                    ...
                    open(wallet_name);
                    unlock(password, TIV_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::create_from_json(const path& filename, const string& wallet_name, const string& passphrase)
        {
            try {
                ...
                try
                {
                    ...
                    close();
                    open(wallet_name);
                    unlock(passphrase, TIV_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::unlock(const string& password, uint32_t timeout_seconds)
        {
            try {
                ...
                try
                {
					...
					fc::time_point new_lock_time;
					fc::time_point now;
					if (timeout_seconds > 0)
					{
						const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - TiValue::blockchain::now().sec_since_epoch();
						if (timeout_seconds > max_timeout_seconds)
							FC_THROW_EXCEPTION(invalid_timeout, "Timeout too large!", ("max_timeout_seconds", max_timeout_seconds));

						now = fc::time_point::now();
						new_lock_time = now + fc::seconds(timeout_seconds);
        ...
        bool Wallet::check_passphrase(const string& passphrase)
        {
            try {
                ...
                if (passphrase.size() < TIV_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
        ...
        void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
        {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                if (new_passphrase.size() < TIV_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            const variant& private_data)
        {
            try {
                ...
                if (num_accounts_before == 0)
                {
                    ...
                    set_last_scanned_block_number_for_tiv(0);
                }
            ...
            bool create_account)
        {
            try {
                ...
                auto key = TiValue::utilities::wif_to_key(wif_key);
                if (key.valid())
                {
        ...
        VoteSummary Wallet::get_vote_status(const string& account_name)
        {
            ...
            for (const auto& item : items)
            {
                ...
                for (const auto& entry : entrys)
                {
                    if (entry.asset_id() != 0)
                        continue;
                    if (entry.slate_id() != my_slate.id() && entry.balance > 1 * TIV_BLOCKCHAIN_PRECISION) // ignore dust
                    ...
                    total_possible += entry.get_spendable_balance(my->_blockchain->now()).amount * TIV_BLOCKCHAIN_MAX_SLATE_SIZE;
                }
            }
            ...
            bool sign)
        {
            try {
                ...
                const auto version = TiValue::client::version_info()["client_version"].as_string();
        ...
        void Wallet::repair_entrys(const optional<string>& collecting_account_name)
        {
            try {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (name.find(TIV_ADDRESS_PREFIX) != 0)
                        continue;
        ...
        std::map<std::string, TiValue::blockchain::DelegatePaySalary> Wallet::query_delegate_salarys()
        {
            FC_ASSERT(is_open());
            FC_ASSERT(is_unlocked());
            std::map<std::string, TiValue::blockchain::DelegatePaySalary> Salary_Map;
        ...
        WalletTransactionEntry Wallet::register_contract(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
        {
            ...
            {
                FC_THROW_EXCEPTION(TiValue::blockchain::invalid_contract_filename, "contract bytecode file name should end with .gpc");
            }
            ...
            FC_ASSERT(asset_symbol == TIV_BLOCKCHAIN_SYMBOL, "asset_symbol must be TV");

            const auto asset_rec = data_ptr->get_asset_entry(asset_symbol);
            FC_ASSERT(asset_rec.valid(), "Asset not exist!");
            FC_ASSERT(asset_rec->id == 0, "asset_symbol must be TV");
            ...
            ContractIdType contract_id = trx.register_contract(contract_code, owner_public_key, asset_limit,fee, balances);//�����Լע��op
        ...
        std::vector<TiValue::blockchain::Asset> Wallet::register_contract_testing(const string& owner, const fc::path codefile)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(owner), "Invalid account name");
            ...
            auto trans_entry = register_contract(owner,
                codefile,
                TIV_BLOCKCHAIN_SYMBOL,
                (double)asset_for_exec.amount / TIV_BLOCKCHAIN_PRECISION, 
                true
                );
            ...
            std::vector<TiValue::blockchain::Asset> asset_vec;
		...
		TiValue::wallet::WalletTransactionEntry Wallet::call_contract(const string caller, const ContractIdType contract, const string method, const string& arguments, const string& asset_symbol, double cost_limit, bool is_testing)
		{
			ChainInterfacePtr data_ptr = get_correct_state_ptr();
      ...
      if (CallContractOperation::is_function_not_allow_call(method))
        ...
        TiValue::wallet::oScriptEntry Wallet::get_script_entry(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        TiValue::blockchain::ScriptIdType Wallet::add_script(const fc::path& filename, const string& description/*=string("")*/)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        TiValue::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract(
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                FC_ASSERT(amount_to_transfer_symbol == TIV_BLOCKCHAIN_SYMBOL, "Asset symbol should be TV");
                ...
                FC_ASSERT(asset_rec.valid(), "Asset not exist!");
                const auto asset_id = asset_rec->id;
                FC_ASSERT(asset_id == 0, "Asset symbol should be TV");
                const int64_t precision = asset_rec->precision ? asset_rec->precision : 1;
        ...
        std::vector<TiValue::blockchain::Asset> Wallet::transfer_asset_to_contract_testing(
            double real_amount_to_transfer,
            const string& amount_to_transfer_symbol,
            const string& from_account_name,
            const Address& to_contract_address,
		...
		std::vector<TiValue::blockchain::Asset> Wallet::upgrade_contract_testing(
			...
			)
		{
			try
			{
				...
				if (margin_balance_entry->balance != TIV_DEFAULT_CONTRACT_MARGIN)
					FC_CAPTURE_AND_THROW(invalid_margin_amount, (margin_balance_entry->balance));


				Asset margin = Asset(TIV_DEFAULT_CONTRACT_MARGIN);
				Asset fee = get_transaction_fee(0);
				Asset asset_for_exec = my->_blockchain->get_amount(CONTRACT_TESTING_LIMIT_MAX);
			...
			bool is_testing)
        {
            try
			{
				...
				FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not TV!");
		...
		std::vector<TiValue::blockchain::Asset> Wallet::destroy_contract_testing(
			const Address& contract_id,
			...
			bool is_testing)
        {
            try
			{
                ...
                FC_ASSERT(balance_entry.valid(), "Can not get margin balance!");
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not TV!");
                FC_ASSERT(balance_entry->balance == TIV_DEFAULT_CONTRACT_MARGIN, "invalid margin balance amount");
                ...
                {
                    FC_ASSERT(balance_entry->asset_id() == 0, "contract balance asset is not TV!");
                }
            ...
            bool sign)
        {
            try {
                ...
                {
                    required_fees += Asset(TIV_BLOCKCHAIN_REGISTER_ACCOUNT_FEE, 0);
                }
            ...
            bool sign)
        {
            try {
                ...
                FC_ASSERT(TIV_BLOCKCHAIN_MAX_SHARES > max_share_supply_in_internal_units);
        ...
        void Wallet::set_transaction_fee(const Asset& fee)
        {
            try {
                ...
                if (fee.amount < TIV_DEFAULT_TRANSACTION_FEE || fee.asset_id != 0)
        ...
        Asset Wallet::get_transaction_imessage_fee(const std::string & imessage)const
        {
            try
            {
                ...
                Asset require_fee;
                    ...
                    if (TIV_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE >= imessage.size())
                    {
                        return require_fee;
                    }
                    auto min_fee_coe = get_transaction_imessage_fee_coe();
                    require_fee.amount = min_fee_coe * (imessage.size() - TIV_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const
        {
            try {
                ...
                Asset xts_fee(TIV_WALLET_DEFAULT_TRANSACTION_FEE, 0);
                try
                {
        ...
        void Wallet::set_last_scanned_block_number_for_tiv(uint32_t block_num)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                my->_wallet_db.set_property(last_scanned_block_number_for_tichain, fc::variant(block_num));
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        uint32_t Wallet::get_last_scanned_block_number_for_tiv()const
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                try
                {
                    return my->_wallet_db.get_property(last_scanned_block_number_for_tichain).as<uint32_t>();
                }
                catch (...)
                {
                }
        ...
        void Wallet::set_transaction_imessage_fee_coe(const ImessageIdType& coe)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (coe < TIV_BLOCKCHAIN_MIN_MESSAGE_FEE_COE)
                    {
        ...
        void Wallet::set_transaction_imessage_soft_max_length(const ImessageLengthIdType& length)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (length < 0 || length > TIV_BLOCKCHAIN_MAX_MESSAGE_SIZE)
                    {
        ...
        void Wallet::set_transaction_expiration(uint32_t secs)
        {
            try {
                ...
                if (secs > TIV_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC)
		...
		{
			vector<TiValue::blockchain::UploadRequestEntry> res;
			for (auto entry_set = my->_upload_request_entrys.begin();
				entry_set!=my->_upload_request_entrys.end();
				entry_set++)
			{
