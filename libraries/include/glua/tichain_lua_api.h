 ...
 * @author zoowii@TiValue
 */

#ifndef tichain_lua_api_h
#define tichain_lua_api_h
...
public:
    ...
    int  contract_state;
    // 合约中storage的类型
    std::map<std::string, TiValue::blockchain::StorageValueTypes> contract_storage_properties;
...
#define tichain_API_NO_ERROR 0
#define tichain_API_SIMPLE_ERROR 1
#define tichain_API_MEMORY_ERROR 2
#define tichain_API_LVM_ERROR 3
#define tichain_API_PARSER_ERROR 4
#define tichain_API_COMPILE_ERROR 5
#define tichain_API_LVM_LIMIT_OVER_ERROR 6
#define tichain_API_THROW_ERROR 7   // glua代码中主动抛出的错误
...
#define lua_storage_is_table(t) (TiValue::blockchain::is_any_table_storage_value_type(t)||TiValue::blockchain::is_any_array_storage_value_type(t))
#define lua_storage_is_array(t) (TiValue::blockchain::is_any_array_storage_value_type(t))
#define lua_storage_is_hashtable(t) (TiValue::blockchain::is_any_table_storage_value_type(t))
...
{
    TiValue::blockchain::StorageValueTypes type;
    union GluaStorageValueUnion value;
    inline GluaStorageValue()
    {
        type = TiValue::blockchain::StorageValueTypes::storage_value_null;
    }
    inline static GluaStorageValue from_int(int val)
    {
        GluaStorageValue sv;
        sv.type = TiValue::blockchain::StorageValueTypes::storage_value_int;
    ...
    {
        GluaStorageValue sv;
        sv.type = TiValue::blockchain::StorageValueTypes::storage_value_string;
        sv.value.string_value = val;
        return sv;
    }

	// 尝试类型转换
	inline void try_parse_type(TiValue::blockchain::StorageValueTypes new_type)
    {
	    switch(new_type)
	    {
    ...
    {
	    if(type == TiValue::blockchain::StorageValueTypes::storage_value_number)
	    {
			value.int_value = (lua_Integer)value.number_value;
			type = TiValue::blockchain::StorageValueTypes::storage_value_int;
	    }
    }
	...
	inline void try_parse_to_number_type()
  {
    ...
    inline bool equals(GluaStorageValue &other)
    {
        ...
        case TiValue::blockchain::StorageValueTypes::storage_value_null:
            return true;
        default: {
            if (TiValue::blockchain::is_any_table_storage_value_type(type)
                || TiValue::blockchain::is_any_array_storage_value_type(type))
            {
        ...
        namespace api {
          ...
          public:
            ...
            */
            ...
            /**
            * get contract info stored before from TiValue api
            ...
            */
            ...
            virtual std::shared_ptr<GluaModuleByteStream> get_bytestream_from_code(lua_State *L, const TiValue::blockchain::Code& code) = 0;
            /**
            ...
            */
            ...
            /**
             * get contract address/id from TiValue by contract name
             */
            ...
            /**
             * register new storage name of contract to TiValue
             */
            virtual bool register_storage(lua_State *L, const char *contract_name, const char *name) = 0;

            virtual GluaStorageValue get_storage_value_from_tichain(lua_State *L, const char *contract_name, std::string name) = 0;

            virtual GluaStorageValue get_storage_value_from_tichain_by_address(lua_State *L, const char *contract_address, std::string name) = 0;

            /**
             * after lua merge storage changes in lua_State, use the function to store the merged changes of storage to TiValue
             */
            virtual bool commit_storage_changes_to_tichain(lua_State *L, AllContractsChangesMap &changes) = 0;
