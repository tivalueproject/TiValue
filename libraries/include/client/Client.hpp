...
namespace TiValue {
    namespace client {
        ...
        //    using TiValue::mail::mail_client_ptr;
        //    using TiValue::mail::mail_server_ptr;
        ...
        struct Config
        {
            ...
            uint16_t            maximum_number_of_connections = TIV_NET_DEFAULT_MAX_CONNECTIONS;
            bool                use_upnp = true;
            ...
            ShareType          min_relay_fee = TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;
            string              wallet_callback_url;

            ShareType          light_network_fee = TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;
            ShareType          light_relay_fee = TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;
            ...
            optional<string>    growl_tiv_client_identifier;
         ...
         *
         */
        class Client : public TiValue::rpc_stubs::CommonApiClient,
        ...
        public:
            ...
            TiValue::rpc::RpcServerPtr   get_rpc_server()const;
            TiValue::net::NodePtr         get_node()const;
            fc::path                   get_data_dir()const;

            // returns true if the client is connected to the network
            bool                is_connected() const;
            TiValue::net::NodeIdType get_node_id() const;
            ...
            bool handle_message(const TiValue::net::Message&, bool sync_mode);
            void sync_status(uint32_t item_type, uint32_t item_count);
            DelegateConfig get_delegate_config();
        protected:
            virtual TiValue::api::CommonApi* get_impl() const override;
...
extern const std::string TIV_MESSAGE_MAGIC;

FC_REFLECT(TiValue::client::ClientNotification, (timestamp)(message)(signature))
FC_REFLECT(TiValue::client::RpcServerConfig, (enable)(enable_cache)(rpc_user)(rpc_password)(rpc_endpoint)(httpd_endpoint)
(encrypted_rpc_endpoint)(encrypted_rpc_wif_key)(htdocs))
FC_REFLECT(TiValue::client::ChainServerConfig, (enabled)(listen_port))
FC_REFLECT(TiValue::client::Config,
(logging)
(ignore_console)
(client_debug_name)
(rpc)
