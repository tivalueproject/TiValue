...
namespace TiValue {
    namespace blockchain {
        ...
        struct SubTrxidBalance
        {
            SubTrxidBalance(){};
            SubTrxidBalance(string tiv_account, TransactionIdType trx_id, Asset asset_trx, uint32_t
                block_num, string from_account, fc::time_point_sec create_time) :
                sub_account(tiv_account),
                trx_id(trx_id),
                asset_trx(asset_trx),
                block_num(block_num),
            ...
            friend bool operator > (const SubTrxidBalance& suba, const SubTrxidBalance& subb){
                return suba.block_num > subb.block_num;
            }
            friend bool operator < (const SubTrxidBalance& suba, const SubTrxidBalance& subb){
                return suba.block_num < subb.block_num;
            }
        ...
        struct SubBalanceEntry
        {
            std::set<TransactionIdType> sub_trxid_sort;
            std::multimap<uint32_t, SubTrxidBalance> sub_block_sort;
        };
...
FC_REFLECT_ENUM(TiValue::blockchain::TransactionType,
    (normal_transaction)
    ...
    FC_REFLECT_ENUM(TiValue::blockchain::ResultTransactionType,
    (origin_transaction)
    (complete_result_transaction)
    (incomplete_result_transaction)
    )

    FC_REFLECT(TiValue::blockchain::SubTrxidBalance, (sub_account)(trx_id)(asset_trx)(block_num)(from_account)(create_time))
    FC_REFLECT(TiValue::blockchain::SubBalanceEntry, (sub_trxid_sort)(sub_block_sort))
    FC_REFLECT(TiValue::blockchain::Transaction, (expiration)(reserved)(sub_account)(sub_inport_asset)(operations)(result_trx_type)(result_trx_id))
    FC_REFLECT_DERIVED(TiValue::blockchain::SignedTransaction, (TiValue::blockchain::Transaction), (signatures))
    FC_REFLECT(TiValue::blockchain::TransactionLocation, (block_num)(trx_num))
