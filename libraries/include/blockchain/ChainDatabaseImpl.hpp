    ...
    namespace blockchain {
        ...
        namespace detail
        {
            ...
            public:
                ...
                * Get database_version from property database and check whether it equals to TIV_BLOCKCHAIN_DATABASE_VERSION
                ...
                */
                ...
                TiValue::db::LevelMap<TransactionIdType, SignedTransaction>                 _pending_transaction_db;
                map<fee_index, TransactionEvaluationStatePtr>                            _pending_fee_index;
                ShareType                                                                  _relay_fee = TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;

                /* Block processing */
                uint32_t /* Only used to skip undo states when possible during replay */    _min_undo_block = 0;

                fc::mutex                                                                   _push_block_mutex;
                ShareType															_block_per_account_reword_amount = TIV_MAX_DELEGATE_PAY_PER_BLOCK;
                TiValue::db::LevelMap<BlockIdType, FullBlock>                               _block_id_to_full_block;
                TiValue::db::fast_level_map<BlockIdType, PendingChainState>                 _block_id_to_undo_state;

                TiValue::db::LevelMap<uint32_t, vector<BlockIdType>>                         _fork_number_db; // All siblings
                TiValue::db::LevelMap<BlockIdType, BlockForkData>                          _fork_db;

                TiValue::db::LevelMap<BlockIdType, int32_t>                                  _revalidatable_future_blocks_db; //int32_t is unused, this is a set

                TiValue::db::LevelMap<uint32_t, BlockIdType>                                 _block_num_to_id_db; // Current chain

                TiValue::db::LevelMap<BlockIdType, BlockEntry>                             _block_id_to_block_entry_db; // Statistics

                /* Current primary state */
                BlockIdType                                                               _head_block_id;
                SignedBlockHeader                                                         _head_block_header;

                TiValue::db::fast_level_map<uint8_t, PropertyEntry>                           _property_id_to_entry;

                TiValue::db::fast_level_map<AccountIdType, AccountEntry>                    _account_id_to_entry;
                TiValue::db::fast_level_map<string, AccountIdType>                            _account_name_to_id;
                TiValue::db::fast_level_map<Address, AccountIdType>                           _account_address_to_id;
                set<VoteDel>                                                               _delegate_votes;

                TiValue::db::fast_level_map<AssetIdType, AssetEntry>                        _asset_id_to_entry;
                TiValue::db::fast_level_map<string, AssetIdType>                              _asset_symbol_to_id;

                TiValue::db::fast_level_map<SlateIdType, SlateEntry>                        _slate_id_to_entry;

                TiValue::db::fast_level_map<BalanceIdType, BalanceEntry>                    _balance_id_to_entry;
                TiValue::db::LevelMap<TransactionIdType, TransactionEntry>                 _transaction_id_to_entry;
                set<UniqueTransactionKey>                                                 _unique_transactions;
                TiValue::db::LevelMap<Address, unordered_set<TransactionIdType>>             _address_to_transaction_ids;




                TiValue::db::LevelMap<SlotIndex, SlotEntry>                                 _slot_index_to_entry;
                TiValue::db::LevelMap<time_point_sec, AccountIdType>                         _slot_timestamp_to_delegate;

                TiValue::db::LevelMap<int, data_version>                      _block_extend_status;
                // TODO: Just store whitelist in asset_entry
                //TiValue::db::level_map<pair<asset_id_type,address>, object_id_type>             _auth_db;
...
FC_REFLECT_TYPENAME(std::vector<TiValue::blockchain::BlockIdType>)
FC_REFLECT_TYPENAME(std::unordered_set<TiValue::blockchain::TransactionIdType>)
FC_REFLECT(TiValue::blockchain::fee_index, (_fees)(_trx))
FC_REFLECT(TiValue::blockchain::data_version, (id)(version))
