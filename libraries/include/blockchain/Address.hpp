...
namespace TiValue {
    namespace blockchain {
        ...
        public:
            ...
            explicit Address(const std::string& base58str, const AddressType& address_type = AddressType::tic_address);   ///< converts to binary, validates checksum
            ...
            * @return bool
            */
            static bool is_valid(const std::string& base58str, const std::string& prefix = TIV_ADDRESS_PREFIX);

            std::string AddressToString(const AddressType& address_type = AddressType::tic_address)const;
...
} // namespace TiValue::blockchain

namespace fc
{
    void to_variant(const TiValue::blockchain::Address& var, fc::variant& vo);
    void from_variant(const fc::variant& var, TiValue::blockchain::Address& vo);
}

namespace std
{
    template<>
    struct hash < TiValue::blockchain::Address >
    {
    public:
        size_t operator()(const TiValue::blockchain::Address &a) const
        {
...
FC_REFLECT_ENUM(TiValue::blockchain::AddressType,
    (tic_address)
    (contract_address)
    (script_id)
    )

    FC_REFLECT(TiValue::blockchain::Address, (addr))
