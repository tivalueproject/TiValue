...
namespace TiValue {
	 namespace rpc_stubs {
  class CommonApiClient : public TiValue::api::CommonApi
  {
  protected:
    virtual TiValue::api::CommonApi* get_impl() const = 0;

  public:
    std::vector<TiValue::blockchain::UploadRequestEntry> blockchain_list_file_saved_info() const override;
    std::vector<TiValue::blockchain::CanApplyEntry> blockchain_list_can_apply_file() const override;
    ...
    TiValue::blockchain::TransactionIdType blockchain_get_transaction_id(const TiValue::blockchain::SignedTransaction& transaction_to_broadcast) const override;
