...
namespace TiValue {
    namespace net {
        ...
        const uint32_t core_protocol_version = TIV_NET_PROTOCOL_VERSION;
...
FC_REFLECT_ENUM(TiValue::net::CoreMessageTypeEnum,
    (trx_message_type)
    (block_message_type)
    ...
    FC_REFLECT(TiValue::net::ItemId, (item_type)
    (item_hash))
    FC_REFLECT(TiValue::net::ItemIdsInventoryMessage, (item_type)
    (item_hashes_available))
    FC_REFLECT(TiValue::net::BlockchainItemIdsInventoryMessage, (total_remaining_item_count)
    (item_type)
    (item_hashes_available))
    FC_REFLECT(TiValue::net::FetchBlockchainItemIdsMessage, (item_type)
    (blockchain_synopsis))
    FC_REFLECT(TiValue::net::FetchItemsMessage, (item_type)
    (items_to_fetch))
    FC_REFLECT(TiValue::net::ItemNotAvailableMessage, (requested_item))
    FC_REFLECT(TiValue::net::HelloMessage, (user_agent)
    ...
    FC_REFLECT_EMPTY(TiValue::net::ConnectionAcceptedMessage)
    FC_REFLECT_ENUM(TiValue::net::RejectionReasonCode, (unspecified)
    (different_chain)
    (already_connected)
    (connected_to_self)
    (not_accepting_connections)
    ...
    (invalid_hello_message)
    (client_too_old))
    FC_REFLECT(TiValue::net::ConnectionRejectedMessage, (user_agent)
    (core_protocol_version)
    (remote_endpoint)
    (reason_code)
    (reason_string))
    FC_REFLECT_EMPTY(TiValue::net::AddressRequestMessage)
    FC_REFLECT(TiValue::net::AddressInfo, (remote_endpoint)
    (last_seen_time)
    (latency)
    (node_id)
    (direction)
    (firewalled))
    FC_REFLECT(TiValue::net::AddressMessage, (addresses))
    FC_REFLECT(TiValue::net::ClosingConnectionMessage, (reason_for_closing)
    (closing_due_to_error)
    (error))
    FC_REFLECT_ENUM(TiValue::net::PeerConnectionDirection, (unknown)
    (inbound)
    (outbound))
    FC_REFLECT_ENUM(TiValue::net::FirewalledState, (unknown)
    (firewalled)
    (not_firewalled))

    FC_REFLECT(TiValue::net::CurrentTimeRequestMessage, (request_sent_time))
    FC_REFLECT(TiValue::net::CurrentTimeReplyMessage, (request_sent_time)
    (request_received_time)
    (reply_transmitted_time))
    FC_REFLECT_ENUM(TiValue::net::FirewallCheckResult, (unable_to_check)
    (unable_to_connect)
    (connection_successful))
    FC_REFLECT(TiValue::net::CheckFirewallMessage, (node_id)(endpoint_to_check))
    FC_REFLECT(TiValue::net::CheckFirewallReplyMessage, (node_id)(endpoint_checked)(result))
    FC_REFLECT_EMPTY(TiValue::net::GetCurrentConnectionsRequestMessage)
    FC_REFLECT(TiValue::net::CurrentConnectionData, (connection_duration)
    (remote_endpoint)
    (node_id)
    (clock_offset)
    (round_trip_delay)
    ...
    (firewalled)
    (user_data))
    FC_REFLECT(TiValue::net::GetCurrentConnectionsReplyMessage, (upload_rate_one_minute)
...
{
    template<>
    struct hash < TiValue::net::ItemId >
    {
        size_t operator()(const TiValue::net::ItemId& item_to_hash) const
        {
