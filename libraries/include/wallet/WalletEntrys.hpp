...
namespace TiValue {
    namespace wallet {
        ...
        struct AccountData : public TiValue::blockchain::AccountEntry
        {
...
{
    template<>
    struct hash < TiValue::wallet::ScriptRelationKey >
    {
        size_t operator()(const TiValue::wallet::ScriptRelationKey& s)const
        {
...
FC_REFLECT_ENUM(TiValue::wallet::WalletEntryTypeEnum,
    ...
    FC_REFLECT_ENUM(TiValue::wallet::PropertyEnum,
    (version)
    (next_entry_number)
    (next_child_key_index)
    (automatic_backups)
...
(last_scanned_block_number_for_tichain)
    ...
    FC_REFLECT(TiValue::wallet::WalletProperty,
    (key)
    (value)
    )

    FC_REFLECT_DERIVED(TiValue::wallet::AccountData, (TiValue::blockchain::AccountEntry),
    (is_my_account)
    (approved)
    (is_favorite)
    ...
    FC_REFLECT_DERIVED(TiValue::wallet::AccountAddressData, (TiValue::wallet::AccountData),
    (owner_address)
    (active_address)
    )

    FC_REFLECT(TiValue::wallet::MasterKey,
    (encrypted_key)
    (checksum)
    )

    FC_REFLECT(TiValue::wallet::KeyData,
    (account_address)
    (public_key)
    ...
    FC_REFLECT_ENUM(TiValue::wallet::ContactData::ContactTypeEnum,
    (account_name)
    (public_key)
    (address)
    (btc_address)
    ...
    FC_REFLECT(TiValue::wallet::ContactData,
    (contact_type)
    (data)
    (label)
    (favorite)
    ...
    FC_REFLECT(TiValue::wallet::TransactionData,
    (entry_id)
    (block_num)
    (is_virtual)
    (is_confirmed)
    ...
    FC_REFLECT(TiValue::wallet::LedgerEntry,
    (from_account)
    (to_account)
    (amount)
    (memo)
    ...
    FC_REFLECT(TiValue::wallet::TransactionLedgerEntry,
    (id)
    (block_num)
    (block_id)
    (timestamp)
    ...
    FC_REFLECT_DERIVED(TiValue::wallet::PrettyTransactionExperimental, (TiValue::wallet::TransactionLedgerEntry),
    (inputs)
    (outputs)
    (balances)
    (notes)
    ...
    FC_REFLECT(TiValue::wallet::Setting,
    (name)
    (value)
    )

    FC_REFLECT(TiValue::wallet::GenericWalletEntry,
    (type)
    (data)
    )

    FC_REFLECT(TiValue::wallet::ScriptEntry,
    (id)
    (enable)
    (code)
    (description)
    ...
    FC_REFLECT(TiValue::wallet::ScriptEntryPrintable,
    (id)
    (enable)
    (code_printable)
    (description)
    ...
    FC_REFLECT(TiValue::wallet::ScriptRelationKey,
    (contract_id)
    (event_type)
    )
...
namespace fc {
    ...
    template<typename T, TiValue::wallet::WalletEntryTypeEnum N>
    struct get_typename < TiValue::wallet::WalletEntry<T, N> >
    {
    ...
    template<typename Type, TiValue::wallet::WalletEntryTypeEnum N>
    struct reflector < TiValue::wallet::WalletEntry<Type, N> >
    {
        typedef TiValue::wallet::WalletEntry<Type, N>  type;
        ...
        {
            {
                typedef decltype(((TiValue::wallet::BaseEntry<N>*)nullptr)->wallet_entry_index) member_type;
                visitor.TEMPLATE operator() < member_type, TiValue::wallet::BaseEntry<N>, &TiValue::wallet::BaseEntry<N>::wallet_entry_index > ("index");
            }
