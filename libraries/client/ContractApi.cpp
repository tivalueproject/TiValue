    ...
    namespace client {
        namespace detail {
            ...
            int ClientImpl::save_code_to_file(const string& name, GluaModuleByteStream *stream, char* err_msg) const
            {
                ...
                for (int i = 0; i < 5; ++i)
                    TiValue::utilities::common_fwrite_int(f, (int*)&digest[i]);

				int p_new_stream_buf_size = (int) p_new_stream->buff.size();
                TiValue::utilities::common_fwrite_int(f, &p_new_stream_buf_size);
				p_new_stream->buff.resize(p_new_stream_buf_size);
                TiValue::utilities::common_fwrite_stream(f, p_new_stream->buff.data(), p_new_stream->buff.size());

                int contract_apis_count = (int)p_new_stream->contract_apis.size();
                TiValue::utilities::common_fwrite_int(f, &contract_apis_count);
                for (int i = 0; i < contract_apis_count; ++i)
                {
                    int api_len = p_new_stream->contract_apis[i].length();
                    TiValue::utilities::common_fwrite_int(f, &api_len);
                    TiValue::utilities::common_fwrite_stream(f, p_new_stream->contract_apis[i].c_str(), api_len);
                }

                int offline_apis_count = (int)p_new_stream->offline_apis.size();
                TiValue::utilities::common_fwrite_int(f, &offline_apis_count);
                for (int i = 0; i < offline_apis_count; ++i)
                {
                    int offline_api_len = p_new_stream->offline_apis[i].length();
                    TiValue::utilities::common_fwrite_int(f, &offline_api_len);
                    TiValue::utilities::common_fwrite_stream(f, p_new_stream->offline_apis[i].c_str(), offline_api_len);
                }

                int contract_emit_events_count = p_new_stream->contract_emit_events.size();
                TiValue::utilities::common_fwrite_int(f, &contract_emit_events_count);
                for (int i = 0; i < contract_emit_events_count; ++i)
                {
                    int event_len = p_new_stream->contract_emit_events[i].length();
                    TiValue::utilities::common_fwrite_int(f, &event_len);
                    TiValue::utilities::common_fwrite_stream(f, p_new_stream->contract_emit_events[i].c_str(), event_len);
                }

                int contract_storage_properties_count = p_new_stream->contract_storage_properties.size();
                TiValue::utilities::common_fwrite_int(f, &contract_storage_properties_count);
                for (const auto& storage_info : p_new_stream->contract_storage_properties)
                {
                    int storage_len = storage_info.first.length();
                    TiValue::utilities::common_fwrite_int(f, &storage_len);
                    TiValue::utilities::common_fwrite_stream(f, storage_info.first.c_str(), storage_len);
                    int storage_type = storage_info.second;
                    TiValue::utilities::common_fwrite_int(f, &storage_type);
                }
            ...
            ContractEntryPrintable ClientImpl::load_contract_to_file(const string &contract, const fc::path &file)
            {
                ...
                TiValue::utilities::common_fwrite_int(f, &byte_code_len);
                TiValue::utilities::common_fwrite_stream(f, contract_entry->code.byte_code.data(), byte_code_len);
                ...
                for (const auto& abi: contract_entry->code.abi)
                {
                    ...
                    TiValue::utilities::common_fwrite_int(f, &api_len);
                    TiValue::utilities::common_fwrite_stream(f, abi.c_str(), api_len);
                }
                ...
                for (const auto& offline_abi: contract_entry->code.offline_abi)
                {
                    ...
                    TiValue::utilities::common_fwrite_int(f, &offline_api_len);
                    TiValue::utilities::common_fwrite_stream(f, offline_abi.c_str(), offline_api_len);
                }
                ...
                for (const auto& event: contract_entry->code.events)
                {
                    ...
                    TiValue::utilities::common_fwrite_int(f, &event_len);
                    TiValue::utilities::common_fwrite_stream(f, event.c_str(), event_len);
                }
            ...
            TiValue::blockchain::Address ClientImpl::get_contract_address(const string& contract,ChainInterfacePtr db) const
            {
                if (db == nullptr)
                {
                    db = _chain_db;
            ...
            fc::path ClientImpl::compile_contract_helper(const fc::path& filename) const
            {
                ...
                {
                    FC_THROW_EXCEPTION(TiValue::blockchain::invalid_contract_filename, "contract source file name should end with .lua or .glua");
                }
                ...
                time_diff.start();
                if (NOT TiValue::lua::lib::compile_contract_to_stream(filename_str.c_str(), p_lua_module, err_msg, &lua_state_pre, USE_TYPE_CHECK))
                {
                    delete p_lua_module;
					err_msg[LUA_EXCEPTION_MULTILINE_STRNG_MAX_LENGTH] = '\0';
                    FC_THROW_EXCEPTION(TiValue::blockchain::compile_contract_fail, err_msg);
                ...
                if (save_code_to_file(out_filename, p_lua_module, err_msg) < 0)
                {
					...
					err_msg[LUA_EXCEPTION_MULTILINE_STRNG_MAX_LENGTH] = '\0';
                    FC_THROW_EXCEPTION(TiValue::blockchain::save_bytecode_to_gpcfile_fail, err_msg);
                }
				if(p_lua_module)
					delete p_lua_module;
            ...
            std::vector<TiValue::blockchain::Asset> ClientImpl::register_contract_testing(
                const std::string& owner,
                const fc::path& codefile)
            {
				// set limit in  sandbox state
            ...
            TiValue::wallet::WalletTransactionEntry ClientImpl::call_contract(const std::string& contract, const std::string& caller_name, const std::string& function_name, const std::string& params, const std::string& asset_symbol, const fc::optional<double>& call_limit)
            {
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            std::vector<TiValue::blockchain::Asset> ClientImpl::call_contract_testing(const std::string& contract, const std::string& caller_name, const std::string& function_name, const std::string& params)
            {
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            TiValue::wallet::WalletTransactionEntry ClientImpl::upgrade_contract(
			...
			std::vector<TiValue::blockchain::Asset> ClientImpl::upgrade_contract_testing(
				const std::string& contract_address,
				const std::string& upgrader_name,
				const std::string& new_contract_name,
				const std::string& new_contract_desc
            ...
            TiValue::wallet::WalletTransactionEntry ClientImpl::destroy_contract(
                const std::string& contract_address,
			...
			std::vector<TiValue::blockchain::Asset> ClientImpl::destroy_contract_testing(const std::string& contract_address, const std::string& destroyer_name)
			{
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            TiValue::blockchain::ContractEntryPrintable ClientImpl::get_contract_info(const std::string& contract)
            {
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                ...
                TiValue::blockchain::oContractEntry entry = _chain_db->get_contract_entry(contract_address);
                FC_ASSERT(entry.valid());

                ContractEntryPrintable print_entry(*entry);

                TiValue::blockchain::oAccountEntry account_entry = _chain_db->get_account_entry(print_entry.owner_address);
            ...
            std::vector<TiValue::blockchain::BalanceEntry> ClientImpl::get_contract_balance(const std::string& contract)
            {
				// set limit in  sandbox state
				if (_chain_db->get_is_in_sandbox())
					FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

                std::vector<TiValue::blockchain::BalanceEntry> entry_list;
            ...
            std::vector<TiValue::blockchain::Asset> ClientImpl::wallet_transfer_to_contract_testing(
                double amount_to_transfer,
                const string& asset_symbol,
                const string& from_account_name,
			...
			std::string ClientImpl::get_contract_registered_in_transaction(const TiValue::blockchain::TransactionIdType& trx_id)
			{
			...
			TiValue::blockchain::TransactionIdType ClientImpl::get_transaction_id_contract_registered(const std::string& contract_id)
			{
...
} // namespace TiValue::client::detail
