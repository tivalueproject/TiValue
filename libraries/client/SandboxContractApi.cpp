    ...
    namespace client
    {
        namespace detail
        {
                ...
                const fc::optional<double>& init_limit)
            {
                ...
                sandbox_state->sandbox_evaluate_transaction(entry.trx, TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE);

                return entry.trx.operations[0].as<RegisterContractOperation>().get_contract_id().AddressToString(AddressType::contract_address);
            }

            std::vector<TiValue::blockchain::Asset> ClientImpl::sandbox_register_contract_testing(
            ...
            TiValue::wallet::WalletTransactionEntry ClientImpl::sandbox_call_contract(const std::string& contract, const std::string& caller_name, const std::string& function_name, const std::string& params, const std::string& asset_symbol, const fc::optional<double>& call_limit)
            {
            ...
            std::vector<TiValue::blockchain::Asset> ClientImpl::sandbox_call_contract_testing(const std::string& contract, const std::string& caller_name, const std::string& function_name, const std::string& params)
            {
                Address contract_address;
			...
			std::vector<TiValue::blockchain::Asset> ClientImpl::sandbox_upgrade_contract_testing(const std::string& contract_address, const std::string& upgrader_name, const std::string& new_contract_name, const std::string& new_contract_desc)
			{
				Address addr;
            ...
            TiValue::wallet::WalletTransactionEntry ClientImpl::sandbox_upgrade_contract(const std::string& contract_address,
			...
			std::vector<TiValue::blockchain::Asset> ClientImpl::sandbox_destroy_contract_testing(const std::string& contract_address, const std::string& destroyer_name)
			{
				Address addr;
            ...
            TiValue::wallet::WalletTransactionEntry ClientImpl::sandbox_destroy_contract(const std::string& contract_address,
            ...
            TiValue::blockchain::ContractEntryPrintable ClientImpl::sandbox_get_contract_info(const std::string& contract)
            {
                Address contract_address = sandbox_get_contract_address(contract);
                ChainInterfacePtr data_ptr = sandbox_get_correct_state_ptr();

                TiValue::blockchain::oContractEntry entry = data_ptr->get_contract_entry(contract_address);
            ...
            CodePrintAble ClientImpl::get_contract_info_from_gpc_file(const fc::path &file)
            {
                ...
                for (int i = 0; i < 5; ++i)
                {
                    TiValue::utilities::common_fread_int(fd, (int*)&digest[i]);
                ...
                //read byte_code
                int byte_code_len;
                TiValue::utilities::common_fread_int(fd, &byte_code_len);
                code.byte_code.resize(byte_code_len);
                TiValue::utilities::common_fread_octets(fd, code.byte_code.data(), byte_code_len);

                //read api and offline_api
                int api_count;
                TiValue::utilities::common_fread_int(fd, &api_count);
                for (int i = 0; i < api_count; ++i)
                {
                    int api_len;
                    TiValue::utilities::common_fread_int(fd, &api_len);
                    std::string abi_str;
                    abi_str.resize(api_len);
                    TiValue::utilities::common_fread_octets(fd, (void*)(abi_str.data()), api_len);
                    code.abi.insert(abi_str);
                }

                int offline_api_count;
                TiValue::utilities::common_fread_int(fd, &offline_api_count);
                for (int i = 0; i < offline_api_count; ++i)
                {
                    int offline_api_len;
                    TiValue::utilities::common_fread_int(fd, &offline_api_len);
                    std::string offline_abi_str;
                    offline_abi_str.resize(offline_api_len);
                    TiValue::utilities::common_fread_octets(fd, (void*)offline_abi_str.data(), offline_api_len);
                    code.offline_abi.insert(offline_abi_str);
                }

                //read event api
                int event_count;
                TiValue::utilities::common_fread_int(fd, &event_count);
                for (int i = 0; i < event_count; ++i)
                {
                    int event_len;
                    TiValue::utilities::common_fread_int(fd, &event_len);
                    std::string event_str;
                    event_str.resize(event_len);
                    TiValue::utilities::common_fread_octets(fd, (void*)event_str.data(), event_len);
                    code.events.insert(event_str);
                }

                //read storage info
                int storage_info_count;
                TiValue::utilities::common_fread_int(fd, &storage_info_count);
                for (int i = 0; i < storage_info_count; ++i)
                {
                    int storage_name_len;
                    TiValue::utilities::common_fread_int(fd, &storage_name_len);
                    std::string storage_name;
                    storage_name.resize(storage_name_len);
                    TiValue::utilities::common_fread_octets(fd, (void*)storage_name.data(), storage_name_len);
                    
                    TiValue::blockchain::StorageValueTypes storage_type;
                    TiValue::utilities::common_fread_int(fd, (int*)&storage_type);
            ...
            std::vector<TiValue::blockchain::BalanceEntry> ClientImpl::sandbox_get_contract_balance(const std::string& contract)
            {
            ...
            std::vector<TiValue::blockchain::Asset> ClientImpl::sandbox_transfer_to_contract_testing(
