...
//#include <TiValue/mail/server.hpp>
...
namespace TiValue {
    namespace client {
        ...
        const string TIV_MESSAGE_MAGIC = "Tiv Signed Message:\n";
        ...
        program_options::variables_map parse_option_variables(int argc, char** argv)
        {
            ...
            option_config.add_options()
                ...
                ("growl-identifier", program_options::value<std::string>(), "A name displayed in growl messages to identify this tv_client instance")
                ;
            ...
            {
                std::cout << fc::json::to_pretty_string(TiValue::client::version_info()) << "\n";
                exit(0);
            }
        ...
        fc::path get_data_dir(const program_options::variables_map& option_variables)
        {
            try {
                ...
                else
                {
                    ...
                    datadir = fc::app_path() / get_os_specific_dir_name(TIV_BLOCKCHAIN_NAME);
                }
                return datadir;
        ...
        void load_checkpoints(const fc::path& data_dir)
        {
            try {
                ...
                decltype(TiValue::blockchain::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                ...
                {
                    decltype(TiValue::blockchain::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                    if (TiValue::blockchain::detail::CHECKPOINT_BLOCKS.empty() || external_checkpoints.crbegin()->first >= TiValue::blockchain::detail::CHECKPOINT_BLOCKS.crbegin()->first)
                    {
                        ulog("Using blockchain checkpoints from file: ${x}", ("x", checkpoint_file.preferred_string()));
                        TiValue::blockchain::detail::CHECKPOINT_BLOCKS = external_checkpoints;
                        return;
                    }
                }
                ...
                if (!file_exception.valid())
                {
                    ...
                    fc::json::save_to_file(TiValue::blockchain::detail::CHECKPOINT_BLOCKS, checkpoint_file);
                }
                else
                {
                ...
                if (!TiValue::blockchain::detail::CHECKPOINT_BLOCKS.empty())
        ...
        namespace detail
        {
                ...
                const program_options::variables_map& option_variables)
            {
                if (option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable)
                {
                    ...
                    // TiValue::rpc::rpc_server::config rpc_config(cfg.rpc);
            ...
            void ClientImpl::start_delegate_loop()
            {
                ...
                if (!_time_discontinuity_connection.connected())
                    _time_discontinuity_connection = TiValue::blockchain::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
            ...
            void ClientImpl::delegate_loop()
            {
                ...
                {
                    set_target_connections(TIV_NET_DELEGATE_DESIRED_CONNECTIONS);
                ...
                if (next_block_time.valid())
                {
                    ...
                    if (*next_block_time <= now)
                    {
                        try
                        {
                            ...
                            FC_ASSERT((now - *next_block_time) < fc::seconds(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
            ...
            void ClientImpl::start_rebroadcast_pending_loop()
            {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
                    "rebroadcast_pending");
            }
            void ClientImpl::start_fork_update_time_loop()
            {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
                    "rebroadcast_pending");
            }
            ...
            TiValue::blockchain::PublicKeyType ClientImpl::wallet_get_account_owner_publickey(const std::string& account_name)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::rebroadcast_pending_loop()
            {
                ...
                else
                {
                    try
                    {
                        ...
                        std::vector<TiValue::blockchain::SignedTransaction> temp_vec;
                    ...
                    try
                    {
                        ...
                        if (fork_data && fork_data->is_known)
                        {
                            if (sync_mode && !fork_data->is_linked)
                                FC_THROW_EXCEPTION(TiValue::blockchain::unlinkable_block,
                        ...
                        else
                        {
                            ...
                            if (sync_mode && !result.is_linked)
                                FC_THROW_EXCEPTION(TiValue::blockchain::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
            ...
            bool ClientImpl::has_item(const TiValue::net::ItemId& id)
            {
            ...
            */
            bool ClientImpl::handle_message(const TiValue::net::Message& message_to_handle, bool sync_mode)
            {
                if (g_client_quit)
                    return true;
                ...
                try
                {
                    ...
                    case block_message_type:
                    {
                        ...
                        if (block_message_to_handle.block.block_size() > TIV_BLOCKCHAIN_MAX_BLOCK_SIZE)
                        {
                        ...
                        TiValue::blockchain::BlockIdType old_head_block = _chain_db->get_head_block_id();
                    ...
                    case trx_message_type:
                    {
                        ...
                        if (trx_message_to_handle.trx.data_size() > TIV_BLOCKCHAIN_MAX_TRX_SIZE)
                        {
                ...
                catch (const TiValue::blockchain::insufficient_relay_fee& original_exception)
                {
                    FC_THROW_EXCEPTION(TiValue::net::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
                        ("original_exception", original_exception.to_detail_string()));
                }
                catch (const TiValue::blockchain::block_older_than_undo_history& original_exception)
                {
                    FC_THROW_EXCEPTION(TiValue::net::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
                        ("original_exception", original_exception.to_detail_string()));
                }
            }
            ...
            */
            std::vector<TiValue::net::ItemHashType> ClientImpl::get_item_ids(uint32_t item_type,
                const std::vector<TiValue::net::ItemHashType>& blockchain_synopsis,
                uint32_t& remaining_item_count,
                uint32_t limit /* = 2000 */)
            {
                // limit = 20; // for testing
                FC_ASSERT(item_type == TiValue::client::block_message_type);
                ...
                TiValue::net::ItemHashType last_seen_block_hash;
                for (const TiValue::net::ItemHashType& item_hash : boost::adaptors::reverse(blockchain_synopsis))
                {
                    try
                    {
                ...
                std::vector<TiValue::net::ItemHashType> hashes_to_return;
            ...
            TiValue::net::Message ClientImpl::get_item(const TiValue::net::ItemId& id)
            {
                if (id.item_type == block_message_type)
                {
                    //   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
                    TiValue::client::BlockMessage block_message_to_send(_chain_db->get_block(id.item_hash));
            ...
            fc::path ClientImpl::compile_script(const fc::path& filename) const
            {
                ...
                try{
                    ...
                    {
                        FC_THROW_EXCEPTION(TiValue::blockchain::invalid_script_source_filename, "script source file name should end with .lua or .glua");
                    }
            ...
            uint32_t ClientImpl::get_block_number(const TiValue::net::ItemHashType& block_id)
            {
                return _chain_db->get_block_num(block_id);
            }

            fc::time_point_sec ClientImpl::get_block_time(const TiValue::net::ItemHashType& block_id)
            {
                if (block_id == TiValue::net::ItemHashType())
                {
            ...
            fc::time_point_sec ClientImpl::get_blockchain_now()
            {
                ...
                return TiValue::blockchain::nonblocking_now();
            }

            TiValue::net::ItemHashType ClientImpl::get_head_block_id() const
            {
            ...
            {
                return TiValue::blockchain::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
            }
            ...
            void ClientImpl::blocks_too_old_monitor_task()
            {
                ...
                if (_chain_db->get_head_block().timestamp < TiValue::blockchain::now() - fc::seconds(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
                    !_sync_mode &&
                ...
                if (!_blocks_too_old_monitor_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                    "block_monitor_task");
            }
        ...
        Client::Client(const std::string& user_agent,
            TiValue::net::SimulatedNetworkPtr network_to_connect_to)
            : my(new detail::ClientImpl(this, user_agent))
        {
            ...
            const std::function<void(float)> replay_status_callback)
        {
            try {
                ...
                my->_wallet = std::make_shared<TiValue::wallet::Wallet>(my->_chain_db, my->_config.wallet_enabled);
                my->_wallet->set_data_directory(wallet_dir / "wallets");

                /*  //delete?
                if (my->_config.mail_server_enabled)
                {
                my->_mail_server = std::make_shared<TiValue::mail::server>();
                my->_mail_server->open(data_dir / "mail");
                }
                my->_mail_client = std::make_shared<TiValue::mail::client>(my->_wallet, my->_chain_db);
                ...
                */
                ...
                if (!my->_p2p_node)
                    my->_p2p_node = std::make_shared<TiValue::net::Node>(my->_user_agent);
        ...
        */
        ...
        TiValue::rpc::RpcServerPtr Client::get_rpc_server()const { return my->_rpc_server; }
        TiValue::net::NodePtr Client::get_node()const { return my->_p2p_node; }

        fc::variant_object version_info()
        {
            string client_version(TiValue::utilities::git_revision_description);
            ...
            info["blockchain_name"] = TIV_BLOCKCHAIN_NAME;
            info["blockchain_description"] = TIV_BLOCKCHAIN_DESCRIPTION;
            info["client_version"] = client_version;
            info["tiv_revision"] = TiValue::utilities::git_revision_sha;
            info["tiv_revision_age"] = fc::get_approximate_relative_time_string(fc::time_point_sec(TiValue::utilities::git_revision_unix_timestamp));
...
#if defined(TIV_CLIENT_JENKINS_BUILD_NUMBER)
            info["jenkins_build_number"] = TIV_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(TIV_CLIENT_JENKINS_BUILD_URL)
            info["jenkins_build_url"] = TIV_CLIENT_JENKINS_BUILD_URL;
#endif
        ...
        void Client::start_networking(std::function<void()> network_started_callback)
        {
            ...
            {
                TiValue::net::ChainDownloader* chain_downloader = new TiValue::net::ChainDownloader();
        ...
        void Client::configure_from_command_line(int argc, char** argv)
        {
            ...
            {
                my->_cli = new TiValue::cli::Cli(this, nullptr, &std::cout);
                return;
            }
            g_client = this;
            // parse command-line options
            ...
            fc::path datadir = TiValue::client::get_data_dir(option_variables);
            fc::path walletDir = TiValue::client::get_wallet_dir(option_variables, datadir);
            ...
            // else we use the default set in TiValue::net::node
            ...
            else //we will accept input from the console
            {
                ...
                if (option_variables.count("log-commands") <= 0)
                {
                    ...
                    my->_cli = new TiValue::cli::Cli(this, my->_command_script_holder.get(), &std::cout);
                }
                else
                {
                    ...
                    my->_cli = new TiValue::cli::Cli(this, my->_command_script_holder.get(), my->_tee_stream.get());
            ...
            if (option_variables["upnp"].as<bool>())
            {
                ...
                my->_upnp_service = std::unique_ptr<TiValue::net::UpnpService>(new TiValue::net::UpnpService);
            ...
            if (option_variables.count("growl") || my->_config.growl_notify_endpoint)
            {
                ...
                std::string tiv_instance_identifier = "Tiv";
                if (option_variables.count("growl-identifier"))
                    tiv_instance_identifier = option_variables["growl-identifier"].as<std::string>();
                else if (my->_config.growl_tiv_client_identifier)
                    tiv_instance_identifier = *my->_config.growl_tiv_client_identifier;
                my->_notifier = std::make_shared<TvGntpNotifier>(host_to_notify, port_to_notify, tiv_instance_identifier, growl_password);
            ...
            {
                my->_chain_server = std::unique_ptr<TiValue::net::ChainServer>(
                    new TiValue::net::ChainServer(my->_chain_db,
        ...
        TiValue::net::NodeIdType Client::get_node_id() const
        {
        ...
        void Client::add_node(const string& remote_endpoint, int32_t oper_flag)
        {
            ...
            catch (const TiValue::net::already_connected_to_requested_peer&)
            {
            }
        }
        ...
        {
            TiValue::net::ItemId head_item_id;
            head_item_id.item_type = TiValue::client::block_message_type;
            uint32_t last_block_num = my->_chain_db->get_head_block_num();
            if (last_block_num == (uint32_t)-1)
                head_item_id.item_hash = TiValue::net::ItemHashType();
            else
                head_item_id.item_hash = my->_chain_db->get_head_block_id();
            my->_p2p_node->sync_from(head_item_id, TiValue::blockchain::get_list_of_fork_block_numbers());
        ...
        bool Client::handle_message(const TiValue::net::Message& message, bool sync_mode)
        {
        ...
        */
        ...
        TiValue::api::CommonApi* Client::get_impl() const
        {
            return my.get();
        }
