...
namespace TiValue {
    namespace client {
        namespace detail {
            ...
            {
                return TiValue::client::version_info();
            }
            ...
            variant_object ClientImpl::get_info()const
            {
                ...
                const auto share_entry = _chain_db->get_asset_entry(TIV_BLOCKCHAIN_SYMBOL);
                if (share_entry.valid())
                    info["blockchain_share_supply"] = share_entry->current_share_supply;

                const auto blocks_left = TIV_BLOCKCHAIN_NUM_DELEGATES - (head_block_num % TIV_BLOCKCHAIN_NUM_DELEGATES);
                ...
                if (head_block_num > 0)
                {
                    ...
                    const auto next_round_timestamp = current_round_timestamp + (blocks_left * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                ...
                info["client_version"] = TiValue::client::version_info()["client_version"].as_string();
            ...
            fc::variant_object ClientImpl::validate_address(const string& address) const
            {
                ...
                try
                {
                    TiValue::blockchain::PublicKeyType test_key(address);
                    result["isvalid"] = true;
                }
                ...
                const std::vector<fc::variants>& parameters_list) const
            {
                ...
                FC_ASSERT(_self->get_rpc_server()->get_method_data(method_name).prerequisites == TiValue::api::no_prerequisites,
