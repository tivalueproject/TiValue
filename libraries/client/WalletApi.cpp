...
namespace TiValue {
    namespace client {
        namespace detail {
            ...
            std::vector<TiValue::blockchain::AccountEntry> ClientImpl::wallet_get_all_approved_accounts(int8_t approval)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            void detail::ClientImpl::wallet_create(const string& wallet_name, const string& password, const string& brain_key)
            {
                ...
                if (brain_key.size() && brain_key.size() < TIV_WALLET_MIN_BRAINKEY_LENGTH) FC_CAPTURE_AND_THROW(brain_key_too_short);
                if (password.size() < TIV_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            void read_keys(const fc::variant& vo, vector<PrivateKeyType>& keys, const string& password)
            {
                ...
                try
                {
                    ...
                    const auto key = TiValue::utilities::wif_to_key(wif_key);
                ...
                catch (const TiValue::wallet::invalid_password&)
                {
                    throw;
                }
                catch (...)
            ...
            */
            ...
            vector<TiValue::wallet::PrettyTransaction> detail::ClientImpl::wallet_transaction_history_splite(const std::string& account_name,
                ...
                int32_t transaction_type)const
            {
                ...
                try {
                    const auto history = _wallet->get_pretty_transaction_history(account_name, 0, -1, asset_symbol, (TiValue::wallet::Wallet::TrxType)transaction_type, true);
            ...
            map<TransactionIdType, fc::exception> detail::ClientImpl::wallet_get_pending_transaction_errors(const TiValue::blockchain::FilePath& filename)const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            vector<TiValue::wallet::AccountAddressData> detail::ClientImpl::wallet_list_my_addresses() const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            string detail::ClientImpl::wallet_get_account_public_address(const string& account_name) const
            {
                ...
                try {
                    ...
                    return (string(acct.owner_address()));
                } FC_RETHROW_EXCEPTIONS(warn, "", ("account_name", account_name))
            }
            ...
            // vector<TiValue::wallet::escrow_summary> ClientImpl::wallet_escrow_summary( const string& account_name ) const
            // { try {
            ...
            std::map<std::string, TiValue::blockchain::DelegatePaySalary>  ClientImpl::wallet_active_delegate_salary()
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::wallet_set_transaction_imessage_fee_coe(const string& fee_coe)
            {
                ...
                try
                {
                    ...
                    if (ipos != string::npos)
                    {
                        ...
                        FC_ASSERT((precision_input <= TIV_BLOCKCHAIN_PRECISION), "Precision is not correct");
                    }
                    double dFee = std::stod(fee_coe);
                    _wallet->set_transaction_imessage_fee_coe(static_cast<int64_t>(floor(dFee * TIV_BLOCKCHAIN_PRECISION + 0.5)));
                }
            ...
            double ClientImpl::wallet_get_transaction_imessage_fee_coe()
            {
                ...
                return ((double)fee_coe) / TIV_BLOCKCHAIN_PRECISION;
            }
            ...
            Asset ClientImpl::wallet_get_transaction_fee(const string& fee_symbol)
            {
                ...
                if (fee_symbol.empty())
                    return _wallet->get_transaction_fee(_chain_db->get_asset_id(TIV_BLOCKCHAIN_SYMBOL));
            ...
            string ClientImpl::wallet_account_balance_rpc(const string& account_name)const
            {
                ...
                try {
                    ...
                    else
                    {
                        ...
                        auto resu = re_val.find(TiValue::blockchain::Asset(0, 0).asset_id);
                        if (resu == re_val.end())
                        {
                            res += "0\"}";
                        }
