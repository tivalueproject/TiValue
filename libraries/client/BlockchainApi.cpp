    ...
    namespace client {
        namespace detail {
            ...
            vector<AccountEntry> ClientImpl::blockchain_list_active_delegates(uint32_t first, uint32_t count)const
            {
                ...
                if (first > 0) --first;
                FC_ASSERT(first < TIV_BLOCKCHAIN_NUM_DELEGATES, "Only the first 99 delegates are active");
                FC_ASSERT(first + count <= TIV_BLOCKCHAIN_NUM_DELEGATES, "Only the first 99 delegates are active");
                vector<AccountIdType> all_delegate_ids = _chain_db->get_active_delegates();
                FC_ASSERT(all_delegate_ids.size() == TIV_BLOCKCHAIN_NUM_DELEGATES, "The num of active delegates should equal to TIV_BLOCKCHAIN_NUM_DELEGATES");
            ...
            vector<string> ClientImpl::blockchain_list_missing_block_delegates(uint32_t block_num)
            {
                ...
                timestamp += TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                while (timestamp != this_block.timestamp)
                    // if time interval between prev_block and this block greater than TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
            ...
            std::vector<std::pair<TiValue::blockchain::TransactionIdType, TiValue::blockchain::SignedTransaction>>  ClientImpl::blockchain_list_pending_transactions() const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

                std::vector<std::pair<TiValue::blockchain::TransactionIdType, TiValue::blockchain::SignedTransaction>> trxs;
            ...
            oAccountEntry detail::ClientImpl::blockchain_get_account(const string& account)const
            {
                ...
                try
                {
                    ...
                    else if (account.substr(0, string(TIV_ADDRESS_PREFIX).size()) == TIV_ADDRESS_PREFIX) {
			...
			std::vector<fc::string> ClientImpl::blockchain_list_pub_all_address(const std::string& pub_key) const
			{
				...
				try
				{
					...
					if (fc::string::npos != pub_key.find(TIV_ADDRESS_PREFIX))
					{
						auto pub_type = TiValue::blockchain::PublicKeyType(pub_key);
            ...
            variant_object ClientImpl::blockchain_get_info()const
            {
                ...
                info["name"] = TIV_BLOCKCHAIN_NAME;
                info["symbol"] = TIV_BLOCKCHAIN_SYMBOL;
                info["address_prefix"] = TIV_ADDRESS_PREFIX;
                info["db_version"] = TIV_BLOCKCHAIN_DATABASE_VERSION;
                info["genesis_timestamp"] = _chain_db->get_genesis_timestamp();

                info["block_interval"] = TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                info["delegate_num"] = TIV_BLOCKCHAIN_NUM_DELEGATES;
                ...
                info["name_size_max"] = TIV_BLOCKCHAIN_MAX_NAME_SIZE;
                info["memo_size_max"] = TIV_BLOCKCHAIN_MAX_MESSAGE_SIZE;
                info["data_size_max"] = TIV_BLOCKCHAIN_MAX_NAME_DATA_SIZE;

                info["symbol_size_max"] = TIV_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE;
                info["symbol_size_min"] = TIV_BLOCKCHAIN_MIN_SYMBOL_SIZE;
                info["asset_shares_max"] = TIV_BLOCKCHAIN_MAX_SHARES;
                info["short_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(TIV_BLOCKCHAIN_MIN_SYMBOL_SIZE);
                info["long_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(TIV_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE);

                info["statistics_enabled"] = _chain_db->get_statistics_enabled();

                info["relay_fee"] = _chain_db->get_relay_fee();
                info["max_pending_queue_size"] = TIV_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
                info["max_trx_per_second"] = TIV_BLOCKCHAIN_MAX_TRX_PER_SECOND;
                ...
                if (fp != NULL)
                {
                    while (true)
                    {
                        ...
                        if (fgets(c, 256, fp) != NULL)
                        {
                            ...
                            std::string allstring = (std::string)TiValue::blockchain::Address(TiValue::blockchain::PtsAddress(pow_address));
                            allstring += balance;
            ...
            TiValue::blockchain::BlockchainSecurityState ClientImpl::blockchain_get_security_state()const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                ...
                state.estimated_confirmation_seconds = (uint32_t)(required_confirmations * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                state.participation_rate = participation_rate;
                if (!blockchain_is_synced())
                {
                    state.alert_level = TiValue::blockchain::BlockchainSecurityState::grey;
                }
                else if (required_confirmations <= TIV_BLOCKCHAIN_NUM_DELEGATES / 2
                    && participation_rate > 80)
                {
                    state.alert_level = TiValue::blockchain::BlockchainSecurityState::green;
                }
                else if (required_confirmations > TIV_BLOCKCHAIN_NUM_DELEGATES
                    || participation_rate < 60)
                {
                    state.alert_level = TiValue::blockchain::BlockchainSecurityState::red;
                }
                else
                {
                    state.alert_level = TiValue::blockchain::BlockchainSecurityState::yellow;
                }
            ...
            bool ClientImpl::blockchain_is_synced() const
            {
                ...
                return (blockchain::now() - _chain_db->get_head_block().timestamp) < fc::seconds(TIV_BLOCKCHAIN_NUM_DELEGATES * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
            }
            ...
            std::string ClientImpl::blockchain_export_fork_graph(uint32_t start_block, uint32_t end_block, const TiValue::blockchain::FilePath& filename)const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            bool ClientImpl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
            {
                ...
                catch (...)
                {
                    try {
                        ...
                        // NB LHS of == operator is TiValue::blockchain::public_key_type and RHS is fc::ecc::public_key,
            ...
            TiValue::blockchain::Asset ClientImpl::blockchain_unclaimed_genesis() const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
