...
namespace TiValue {
    namespace client {
        namespace detail {
            ...
            TiValue::blockchain::TransactionIdType detail::ClientImpl::network_broadcast_transactions(const std::vector<TiValue::blockchain::SignedTransaction>& transactions_to_broadcast)
            {
            ...
            TiValue::blockchain::TransactionIdType detail::ClientImpl::network_broadcast_transaction(const TiValue::blockchain::SignedTransaction& transaction_to_broadcast)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            std::vector<fc::variant_object> detail::ClientImpl::network_get_peer_info(bool hide_firewalled_nodes)const
            {
                ...
                std::vector<TiValue::net::PeerStatus> peer_statuses = _p2p_node->get_connected_peers();
                for (const TiValue::net::PeerStatus& peer_status : peer_statuses)
            ...
            // void detail::client_impl::network_set_allowed_peers(const vector<TiValue::net::node_id_t>& allowed_peers)
            // {
            ...
            TiValue::net::MessagePropagationData detail::ClientImpl::network_get_transaction_propagation_data(const TransactionIdType& transaction_id)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            TiValue::net::MessagePropagationData detail::ClientImpl::network_get_block_propagation_data(const BlockIdType& block_id)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            vector<TiValue::net::PotentialPeerEntry> ClientImpl::network_list_potential_peers()const
            {
