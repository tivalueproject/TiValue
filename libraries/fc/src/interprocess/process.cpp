...
class process::impl 
{
  ...
  std::shared_ptr<bp::detail::pipe>  _inp;
  std::shared_ptr<bp::detail::pipe>  _outp;
  std::shared_ptr<bp::detail::pipe>  _errp;
                         ...
                         const fc::path& work_dir, int opt  ) 
{
  ...
  */
  ...
  if( opt & open_stdout ) {
     ...
     my->_outp.reset( new bp::detail::pipe( fc::asio::default_io_service(), outh.release() ) );
  }
  if( opt & open_stderr ) {
     bp::handle errh = my->child->get_handle( bp::stderr_id );
     my->_errp.reset( new bp::detail::pipe( fc::asio::default_io_service(), errh.release() ) );
  }
  if( opt & open_stdin ) {
     bp::handle inh  = my->child->get_handle( bp::stdin_id );
     my->_inp.reset(  new bp::detail::pipe( fc::asio::default_io_service(), inh.release()  ) );
  }
  ...
  if( opt & open_stdin )
    my->_in     = std::make_shared<buffered_ostream>(std::make_shared<fc::asio::ostream<bp::detail::pipe>>(my->_inp));
  if( opt & open_stdout )
    my->_out    = std::make_shared<buffered_istream>(std::make_shared<fc::asio::istream<bp::detail::pipe>>(my->_outp));
  if( opt & open_stderr )
    my->_err    = std::make_shared<buffered_istream>(std::make_shared<fc::asio::istream<bp::detail::pipe>>(my->_errp));
  my->_exited = p;
  return *this;
}
