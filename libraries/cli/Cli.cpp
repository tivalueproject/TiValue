...
namespace TiValue {
    namespace cli {
        ...
        FC_DECLARE_EXCEPTION(cli_exception, 11000, "CLI Error")
            FC_DECLARE_DERIVED_EXCEPTION(abort_cli_command, TiValue::cli::cli_exception, 11001, "command aborted by user");
        FC_DECLARE_DERIVED_EXCEPTION(exit_cli_command, TiValue::cli::cli_exception, 11002, "exit CLI client requested by user");
            ...
            )
        {
            if (input_stream == nullptr)
                FC_CAPTURE_AND_THROW(TiValue::cli::exit_cli_command); //_input_stream != nullptr );
        ...
        namespace detail
        {
            ...
            public:
				...
				//sandbox command limit
				std::vector<std::string>  cmd_limit;


                CliImpl(TiValue::client::Client* client, std::istream* command_script, std::ostream* output_stream);

				bool sandbox_set_cmd_limit()
				{
					cmd_limit = { "sandbox_open", "sandbox_close", "sandbox_register_contract",
						"sandbox_call_contract", "sandbox_upgrade_contract", "sandbox_destroy_contract",
						"sandbox_transfer_to_contract", "sandbox_get_contract_info",
						"sandbox_get_contract_balance", "sandbox_account_balance",
						"sandbox_register_contract_testing", "sandbox_call_contract_testing",
						"sandbox_transfer_to_contract_testing", "sandbox_compile_contract",
						"sandbox_list_my_addresses", "sandbox_destroy_contract_testing",
						"sandbox_upgrade_contract_testing",
                        "get_contract_info_from_gpc_file", "wallet_get_contracts", "wallet_scan_contracts",
						"wallet_delegate_set_block_production", "load_contract_to_file",
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (_client->get_chain()->get_is_in_sandbox())
                                prompt += " (unlocked) (in sandbox) " CLI_PROMPT_SUFFIX;
                            else
                    ...
                    fc::istream_ptr argument_stream)
                {
                    ...
                    if (command_is_valid)
                    {
                        ...
                        catch (const TiValue::cli::exit_cli_command&)
                        {
                            throw;
                        }
                        catch (const fc::exception& e)
                ...
                fc::variants parse_interactive_command(fc::buffered_istream& argument_stream, const string& command)
                {
                    try
                    {
                        const TiValue::api::MethodData& MethodData = _rpc_server->get_method_data(command);
                        return _self->parse_recognized_interactive_command(argument_stream, MethodData);
                    }
                ...
                fc::variants parse_recognized_interactive_command(fc::buffered_istream& argument_stream, const TiValue::api::MethodData& MethodData)
                {
                    try {
                        fc::variants arguments;
                        for (unsigned i = 0; i < MethodData.parameters.size(); ++i)
                        {
                            bool parse_argument_threw_eof = false;
                            try
                            {
                                arguments.push_back(_self->parse_argument_of_known_type(argument_stream, MethodData, i));
                            }
                            ...
                            catch (fc::parse_error_exception& e)
                            {
                                FC_RETHROW_EXCEPTION(e, error, "Error parsing argument ${argument_number} of command \"${command}\": ${detail}",
                                    ("argument_number", i + 1)("command", MethodData.name)("detail", e.get_log()));
                            }

                            if (parse_argument_threw_eof)
                            {
                                if (MethodData.parameters[i].classification != TiValue::api::required_positional)
                                {
                                    return arguments;
                                }
                                else //if missing required argument, prompt for that argument
                                {
                                    const TiValue::api::ParameterData& this_parameter = MethodData.parameters[i];
                                    ...
                                    else //not a passphrase
                                    {
                                        ...
                                        try
                                        {
                                            arguments.push_back(_self->parse_argument_of_known_type(buffered_argument_stream, MethodData, i));
                                        }
                                        catch (const fc::eof_exception& e)
                                        {
                                            FC_THROW("Missing argument ${argument_number} of command \"${command}\"",
                                                ("argument_number", i + 1)("command", MethodData.name)("cause", e.to_detail_string()));
                                        }
                                        catch (fc::parse_error_exception& e)
                                        {
                                            FC_RETHROW_EXCEPTION(e, error, "Error parsing argument ${argument_number} of command \"${command}\": ${detail}",
                            ...
                            if (MethodData.parameters[i].classification == TiValue::api::optional_named)
                                break;
                        }
                        return arguments;
                ...
                fc::variant parse_argument_of_known_type(fc::buffered_istream& argument_stream, const TiValue::api::MethodData& MethodData, unsigned parameter_index)
                {
                    try {
                        ...
                        if (this_parameter.type == "asset")
                        {
                            ...
                            {
                                FC_RETHROW_EXCEPTION(e, error, "Error parsing argument ${argument_number} of command \"${command}\": ${detail}",
                                    ("argument_number", parameter_index + 1)("command", MethodData.name)("detail", e.get_log()));
                            }
                            catch (fc::parse_error_exception& e)
                            {
                                FC_RETHROW_EXCEPTION(e, error, "Error parsing argument ${argument_number} of command \"${command}\": ${detail}",
                            ...
                            return fc::variant(TiValue::blockchain::Asset(amount_as_int));
                        }
                        else if (this_parameter.type == "address")
                        {
                            ...
                            if (!TiValue::blockchain::Address::is_valid(address_string))
                            {
                                FC_THROW_EXCEPTION(fc::parse_error_exception, "Error parsing argument ${argument_number} of command \"${command}\": ${detail}",
                                    ("argument_number", parameter_index + 1)("command", MethodData.name));
                            }
                            return fc::variant(TiValue::blockchain::Address(address_string));
                        }
                ...
                fc::variant execute_interactive_command(const string& command, const fc::variants& arguments)
                {
                    ...
                    {
                        _quit = true;
                        TiValue::client::g_client_quit = true;
                        FC_THROW_EXCEPTION(TiValue::cli::exit_cli_command, "quit command issued");
                    }
                ...
                {
                    try {
						//if in sandbox, the command must be sandbox like command
						if (_client->get_chain()->get_is_in_sandbox())
						{
							sandbox_set_cmd_limit();

							std::vector<std::string>::const_iterator pos = std::find(cmd_limit.begin(), cmd_limit.end(), command);
							if (pos == cmd_limit.end())
							{
								*_out << "In sandbox you can not execute this command! \n";
								
								FC_THROW_EXCEPTION(TiValue::cli::abort_cli_command, "");
							}

							//if already in sandbox, call sandbox_open should show prompt
							if (command == "sandbox_open")
							{
								*_out << "You are already in sandbox which means if you call sandbox_open again, the state will be reset! \n";
								*_out << "So if you mean to do this operation, please close the sandbox first.\n";
                        ...
                        while (true)
                        {
                            ...
                            //sandbox related
                            bool sandbox_open_needed = false;
                            ...
                            catch (const rpc_sandbox_open_needed_exception&)
                            {
                                sandbox_open_needed = true;
                            }
                            ...
                            else if (sandbox_open_needed)
                            {
                                interactive_open_sandbox();
                            }
                ...
                string prompt_for_input(const string& prompt, bool no_echo = false, bool verify = false)
                {
                    try {
                        ...
                        while (true)
                        {
                            ...
                            if (input.empty())
                                FC_THROW_EXCEPTION(TiValue::cli::abort_cli_command, "input aborted");
                ...
                void interactive_open_sandbox()
                {
                    if (_client->get_chain()->get_is_in_sandbox())
                        return;

                    *_out << "The sandbox must be open to execute this command. You can:\n";
                    *_out << "(o) Open the sandbox\n";
                    *_out << "(q) Abort the command\n";
                    ...
                    if (choice == "o")
                    {
                        ...
                        try
                        {
                            parse_and_execute_interactive_command("sandbox_open", argument_stream);
                        }
                        catch (const TiValue::cli::abort_cli_command&)
                        {
                        }
                    ...
                    const fc::variant& result)
                {
                    ...
                    try
                    {
                        ...
                        auto MethodData = _rpc_server->get_method_data(command);
                    ...
                    {
                        string result_type;
                        const TiValue::api::MethodData& MethodData = _rpc_server->get_method_data(method_name);
                        result_type = MethodData.return_type;

                        if (result_type == "asset")
                        {
                            *_out << (string)result.as<TiValue::blockchain::Asset>() << "\n";
                        }
                        else if (result_type == "address")
                        {
                            *_out << (string)result.as<TiValue::blockchain::Address>() << "\n";
                        }
...
#ifdef HAVE_READLINE
                typedef std::map<string, TiValue::api::MethodData> MethodDataMapType;
            ...
            CliImpl::CliImpl(TiValue::client::Client* client, std::istream* command_script, std::ostream* output_stream)
                :_client(client)
      ...
      //rl_bind_keyseq("\\C-c", &control_c_handler);
            ...
            void CliImpl::initialize_method_data_if_necessary()
            {
                if (!_method_data_is_initialized)
                {
                    ...
                    vector<TiValue::api::MethodData> method_data_list = _rpc_server->get_all_method_data();
                    for (const auto& MethodData : method_data_list)
                    {
                        _method_data_map[MethodData.name] = MethodData;
                        _method_alias_map[MethodData.name] = MethodData.name;
                        for (const auto& alias : MethodData.aliases)
                            _method_alias_map[alias] = MethodData.name;
                    }
                }
            }
            extern "C" int get_character(FILE* stream)
            ...
            char** CliImpl::json_completion(const char* text, int start, int end)
            {
                ...
                else
                {
                    ...
                    if (!trimmed_command_to_parse.empty())
                    {
                        ...
                        try
                        {
                            const TiValue::api::MethodData& MethodData = cli_impl_instance->_rpc_server->get_method_data(trimmed_command_to_parse);
                            if (MethodData.name == "help")
                            {
                                return rl_completion_matches(text, &json_command_completion_generator_function);
                            }
                        }
                        catch (const TiValue::rpc::unknown_method&)
                        {
                            // do nothing
                        }
                    }
        ...
        Cli::Cli(TiValue::client::Client* client, std::istream* command_script, std::ostream* output_stream)
        ...
        {
            return my->parse_argument_of_known_type(argument_stream, MethodData, parameter_index);
        }
        ...
        fc::variants Cli::parse_recognized_interactive_command(fc::buffered_istream& argument_stream,
            const TiValue::api::MethodData& MethodData)
        {
            return my->parse_recognized_interactive_command(argument_stream, MethodData);
        }
