            ...
            ClientRawPtr client, std::ostream& out)const
        {
            try
            {
                ...
                else
                {
                    ...
                    const TiValue::api::MethodData& MethodData = client->get_rpc_server()->get_method_data(method_name);
                    result_type = MethodData.return_type;

                    if (result_type == "asset")
                    {
                        out << string(result.as<TiValue::blockchain::Asset>()) << "\n";
                    }
                    else if (result_type == "address")
                    {
                        out << (string)result.as<TiValue::blockchain::Address>() << "\n";
                    }
...
#define blank4 "    "
            ...
            {
                auto res = result.as<TiValue::wallet::ScriptEntryPrintable>();
                string res_str = "";
                string res_desc = "";
        ...
        void PrintResult::f_wallet_transfer_2(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
            ...
            for (int i = 0; i < entry.trx.operations.size(); ++i)
                if (entry.trx.operations[i].type == TiValue::blockchain::OperationTypeEnum::imessage_memo_op_type)
                {
        ...
        void PrintResult::f_blockchain_get_transaction(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
            ...
            for (int i = 0; i < trx_pair.second.trx.operations.size(); ++i)
                if (trx_pair.second.trx.operations[i].type == TiValue::blockchain::OperationTypeEnum::imessage_memo_op_type)
                {
        ...
        void PrintResult::f_blockchain_get_block_transactions(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
            ...
            for (auto iter = trx_map.begin(); iter != trx_map.end(); ++iter)
                for (int i = 0; i < iter->second.trx.operations.size(); ++i)
                    if (iter->second.trx.operations[i].type == TiValue::blockchain::OperationTypeEnum::imessage_memo_op_type)
                    {
        ...
        {
            auto transactions = result.as<vector<std::pair<TiValue::blockchain::TransactionIdType, TiValue::blockchain::SignedTransaction>>>();
