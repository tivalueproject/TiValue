...
} // end namespace TiValue::net::detail
FC_REFLECT(TiValue::net::detail::NodeConfiguration, (listen_endpoint)
    ...
    namespace net {
        namespace detail {
            ...
            struct PrioritizedItemId
            {
                ...
                fc::time_point time_stamp;
                PrioritizedItemId(const ItemId& item, unsigned sequence_number) :
                    item(item),
                    sequence_number(sequence_number),
                    time_stamp(fc::time_point::now())
                {}
                bool operator<(const PrioritizedItemId& rhs) const
                {
                    static_assert(TiValue::client::block_message_type > TiValue::client::trx_message_type,
...
#undef DECLARE_ACCUMULATOR
            ...
            public:
                ...
                std::vector<ItemHashType> get_blockchain_synopsis(uint32_t item_type,
                    const TiValue::net::ItemHashType& reference_point = TiValue::net::ItemHashType(),
...
#define BLACKLIST_DATABASE_FILENAME      "blacklist.leveldb"
				...
				fc::time_point_sec early_ntp_update_time = fc::time_point_sec();
                ...
                TiValue::db::LevelMap<std::string, fc::ip::endpoint> _black_list_db;
                ...
                typedef std::unordered_map<TiValue::blockchain::BlockIdType, fc::time_point> ActiveSyncRequestsMap;

                ActiveSyncRequestsMap              _active_sync_requests; /// list of sync blocks we've asked for from peers but have not yet received
                std::list<TiValue::client::BlockMessage> _new_received_sync_items; /// list of sync blocks we've just received but haven't yet tried to process
                std::list<TiValue::client::BlockMessage> _received_sync_items; /// list of sync blocks we've received, but can't yet process because we are still missing blocks that come earlier in the chain
                // @}
                ...
                void send_sync_block_to_node_delegate(const TiValue::client::BlockMessage& block_message_to_send);
                void process_backlog_of_sync_blocks();
                void trigger_process_backlog_of_sync_blocks();
                void process_block_during_sync(PeerConnection* originating_peer, const TiValue::client::BlockMessage& block_message, const MessageHashType& message_hash);
                void process_block_during_normal_operation(PeerConnection* originating_peer, const TiValue::client::BlockMessage& block_message, const MessageHashType& message_hash);
                ...
                MessagePropagationData   get_transaction_propagation_data(const TiValue::blockchain::TransactionIdType& transaction_id);
                MessagePropagationData   get_block_propagation_data(const TiValue::blockchain::BlockIdType& block_id);
...
#define MAXIMUM_NUMBER_OF_BLOCKS_TO_PREFETCH (10 * MAXIMUM_NUMBER_OF_BLOCKS_TO_HANDLE_AT_ONE_TIME)
            ...
            NodeImpl::NodeImpl(const std::string& user_agent) :
                ...
                _desired_number_of_connections(TIV_NET_DEFAULT_DESIRED_CONNECTIONS),
                _maximum_number_of_connections(TIV_NET_DEFAULT_MAX_CONNECTIONS),
                _peer_connection_retry_timeout(TIV_NET_DEFAULT_PEER_CONNECTION_RETRY_TIME),
                _peer_inactivity_timeout(TIV_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT),
                ...
                _maximum_blocks_per_peer_during_syncing(TIV_NET_MAX_BLOCKS_PER_PEER_DURING_SYNCING),
                _sync_item_type(0)
            {
            ...
            void NodeImpl::p2p_network_connect_loop()
            {
                ...
                while (!_p2p_network_connect_loop_done.canceled())
                {
                    try
                    {
                        ...
                        try
                        {
                            _retrigger_connect_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("TiValue::net::retrigger_connect_loop"));
                            if (is_wanting_new_connections() || !_add_once_node_list.empty())
                            {
                                ...
                                _retrigger_connect_loop_promise->wait_until(fc::time_point::now() + fc::seconds(TIV_PEER_DATABASE_RETRY_DELAY));
                            }
                            else
                            {
            ...
            bool NodeImpl::have_already_received_sync_item(const ItemHashType& item_hash)
            {
                ...
                return std::find_if(_received_sync_items.begin(), _received_sync_items.end(),
                    [&item_hash](const TiValue::client::BlockMessage& message) { return message.block_id == item_hash; }) != _received_sync_items.end() ||
                    std::find_if(_new_received_sync_items.begin(), _new_received_sync_items.end(),
                    [&item_hash](const TiValue::client::BlockMessage& message) { return message.block_id == item_hash; }) != _new_received_sync_items.end();;
            }
            ...
            void NodeImpl::request_sync_item_from_peer(const PeerConnectionPtr& peer, const ItemHashType& item_to_request)
            {
                ...
                ItemId item_id_to_request(TiValue::client::block_message_type, item_to_request);
            ...
            void NodeImpl::request_sync_items_from_peer(const PeerConnectionPtr& peer, const std::vector<ItemHashType>& items_to_request)
            {
                ...
                peer->send_message(FetchItemsMessage(TiValue::client::block_message_type, items_to_request));
            }
            ...
            void NodeImpl::fetch_sync_items_loop()
            {
                ...
                while (!_fetch_sync_items_loop_done.canceled())
                {
                    ...
                    if (!_sync_items_to_fetch_updated)
                    {
                        ...
                        _retrigger_fetch_sync_items_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("TiValue::net::retrigger_fetch_sync_items_loop"));
            ...
            void NodeImpl::fetch_items_loop()
            {
                ...
                while (!_fetch_item_loop_done.canceled())
                {
                    ...
                    fc::time_point oldest_timestamp_to_fetch = fc::time_point::now() - fc::seconds(10 * 5);
                    ...
                    {
                        PeerConnectionPtr peer;
                        std::vector<ItemId> item_ids;
                        peer_and_items_to_fetch(const PeerConnectionPtr& peer) : peer(peer) {}
                    ...
                    typedef boost::multi_index_container<peer_and_items_to_fetch,
                        boost::multi_index::indexed_by<boost::multi_index::ordered_unique<boost::multi_index::member<peer_and_items_to_fetch, PeerConnectionPtr, &peer_and_items_to_fetch::peer> >,
                    ...
                    {
                        if (item_iter->time_stamp < oldest_timestamp_to_fetch)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                const PeerConnectionPtr& peer = peer_iter->peer;
                                // if they have the item and we haven't already decided to ask them for too many other items
                                if (peer_iter->item_ids.size() < TIV_NET_MAX_ITEMS_PER_PEER_DURING_NORMAL_OPERATION &&
                                    peer->inventory_peer_advertised_to_us.find(item_iter->item) != peer->inventory_peer_advertised_to_us.end())
                                {
                                    if (item_iter->item.item_type == TiValue::net::trx_message_type && peer->is_transaction_fetching_inhibited())
                                    ...
                                    else
                                    {
                                        ...
                                        ItemId item_id_to_fetch = item_iter->item;
                                        peer->items_requested_from_peer.insert(PeerConnection::item_to_time_map_type::value_type(item_id_to_fetch, fc::time_point::now()));
                                        item_iter = _items_to_fetch.erase(item_iter);
                                        item_fetched = true;
                    ...
                    for (const peer_and_items_to_fetch& peer_and_items : items_by_peer)
                    {
                        ...
                        std::map<uint32_t, std::vector<ItemHashType> > items_to_fetch_by_type;
                        for (const ItemId& item : peer_and_items.item_ids)
                        ...
                        for (auto& items_by_type : items_to_fetch_by_type)
                        {
                            ...
                            peer_and_items.peer->send_message(FetchItemsMessage(items_by_type.first,
                    ...
                    {
                        _retrigger_fetch_item_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("TiValue::net::retrigger_fetch_item_loop"));
            ...
            void NodeImpl::advertise_inventory_loop()
            {
                ...
                while (!_advertise_inventory_loop_done.canceled())
                {
                    ...
                    {
                        _retrigger_advertise_inventory_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("TiValue::net::retrigger_advertise_inventory_loop"));
            ...
            void NodeImpl::terminate_inactive_connections_loop()
            {
                ...
                uint32_t active_disconnect_timeout = std::max<uint32_t>(5 * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 2, 30);
                uint32_t active_send_keepalive_timeount = std::max<uint32_t>(active_disconnect_timeout / 2, 11);
                uint32_t active_ignored_request_timeount = std::max<uint32_t>(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 4, 10);
                ...
                fc::time_point closing_disconnect_threshold = fc::time_point::now() - fc::seconds(TIV_NET_PEER_DISCONNECT_TIMEOUT);
                for (const PeerConnectionPtr& closing_peer : _closing_connections)
                    ...
                    {
                        // we asked this peer to close their connectoin to us at least TIV_NET_PEER_DISCONNECT_TIMEOUT
                ...
                if (!_node_is_shutting_down && !_terminate_inactive_connections_loop_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(TIV_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT / 2),
            ...
            bool NodeImpl::merge_address_info_with_potential_peer_database(const std::vector<AddressInfo>& addresses)
            {
                ...
                {
                    if (address.firewalled == TiValue::net::FirewalledState::not_firewalled)
                    {
            ...
            void NodeImpl::on_message(PeerConnection* originating_peer, const Message& received_message)
            {
                ...
                dlog("handling message ${type} ${hash} size ${size} from peer ${endpoint}",
                    ("type", TiValue::net::CoreMessageTypeEnum(received_message.msg_type))("hash", message_hash)
                ...
                case CoreMessageTypeEnum::closing_connection_message_type:
                    ...
                    break;
                case TiValue::client::MessageTypeEnum::block_message_type:
            ...
            fc::variant_object NodeImpl::generate_hello_user_data()
            {
                ...
                user_data["tichain_git_revision_sha"] = TiValue::utilities::git_revision_sha;
                user_data["tichain_git_revision_unix_timestamp"] = TiValue::utilities::git_revision_unix_timestamp;
                ...
                user_data["delegate_num"] = TIV_BLOCKCHAIN_NUM_DELEGATES;
            ...
            void NodeImpl::parse_hello_user_data_for_peer(PeerConnection* originating_peer, const fc::variant_object& user_data)
            {
                ...
                if (user_data.contains("tichain_git_revision_sha"))
                    originating_peer->tichain_git_revision_sha = user_data["tichain_git_revision_sha"].as_string();
                if (user_data.contains("tichain_git_revision_unix_timestamp"))
                    originating_peer->tichain_git_revision_unix_timestamp = fc::time_point_sec(user_data["tichain_git_revision_unix_timestamp"].as<uint32_t>());
            ...
            void NodeImpl::on_hello_message(PeerConnection* originating_peer, const HelloMessage& hello_message_received)
            {
                ...
                if (originating_peer->last_known_fork_block_number == 0 &&
                    originating_peer->tichain_git_revision_unix_timestamp)
                {
                    uint32_t unix_timestamp = originating_peer->tichain_git_revision_unix_timestamp->sec_since_epoch();
                ...
                if (originating_peer->their_state == PeerConnection::TheirConnectionState::just_connected)
                {
                    ...
                    if (delegate_num != TIV_BLOCKCHAIN_NUM_DELEGATES)
                    {
                        wlog("Invalid client");
                    ...
                    if (originating_peer->last_known_fork_block_number != 0)
                    {
                        ...
                        if (next_fork_block_number != 0)
                        {
                            ...
                            if (next_fork_block_number < head_block_num)
                            {
                                ...
                                ulog("Rejecting connection from peer because their version is too old.  Their version date: ${date}", ("date", originating_peer->tichain_git_revision_unix_timestamp));
#endif
            ...
            void NodeImpl::on_address_message(PeerConnection* originating_peer, const AddressMessage& address_message_received)
            {
                ...
                std::vector<TiValue::net::AddressInfo> updated_addresses = address_message_received.addresses;
                ...
                const BlockchainItemIdsInventoryMessage& blockchain_item_ids_inventory_message_received)
            {
                ...
                if (originating_peer->item_ids_requested_from_peer)
                {
                    ...
                    fc::time_point_sec minimum_time_of_last_offered_block =
                        ...
                        originating_peer->number_of_unfetched_item_ids * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    if (minimum_time_of_last_offered_block > _delegate->get_blockchain_now() + TIV_NET_FUTURE_SYNC_BLOCKS_GRACE_PERIOD_SEC)
                    {
                    ...
                    if (blockchain_item_ids_inventory_message_received.total_remaining_item_count != 0)
                    {
                        ...
                        if (originating_peer->ids_of_items_to_get.size() > TIV_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                        {
            ...
            void NodeImpl::on_fetch_items_message(PeerConnection* originating_peer, const FetchItemsMessage& fetch_items_message_received)
            {
                ...
                {
                    TiValue::client::BlockMessage block = last_block_message_sent->as<TiValue::client::BlockMessage>();
                ...
                for (const Message& reply : reply_messages)
                {
                    if (reply.msg_type == block_message_type)
                        originating_peer->send_item(ItemId(block_message_type, reply.as<TiValue::client::BlockMessage>().block_id));
                    else
                        originating_peer->send_message(reply);
                }
            }
            ...
            void NodeImpl::on_item_ids_inventory_message(PeerConnection* originating_peer, const ItemIdsInventoryMessage& item_ids_inventory_message_received)
            {
                ...
                for (const ItemHashType& item_hash : item_ids_inventory_message_received.item_hashes_available)
                {
                    ...
                    if (!we_advertised_this_item_to_a_peer)
                    {
                        ...
                        if ((item_ids_inventory_message_received.item_type == TiValue::client::trx_message_type &&
            ...
            void NodeImpl::send_sync_block_to_node_delegate(const TiValue::client::BlockMessage& block_message_to_send)
            {
...
#ifdef ENABLE_DEBUG_ULOGS
                                    ulog("Disconnecting from peer during sync because their version is too old.  Their version date: ${date}", ("date", peer->tichain_git_revision_unix_timestamp));
#endif
                ...
                do
                {
                        ...
                        ++received_block_iter)
                    {
                        ...
                        if (potential_first_block)
                        {
                            ...
                            {
                                TiValue::client::BlockMessage block_message_to_process = *received_block_iter;
            ...
            void NodeImpl::process_block_during_sync(PeerConnection* originating_peer,
                const TiValue::client::BlockMessage& block_message_to_process, const MessageHashType& message_hash)
            {
                VERIFY_CORRECT_THREAD();
            ...
            void NodeImpl::process_block_during_normal_operation(PeerConnection* originating_peer,
                const TiValue::client::BlockMessage& block_message_to_process,
                const MessageHashType& message_hash)
            {
                ...
                try
                {
                    ...
                    ItemId block_message_item_id(TiValue::client::MessageTypeEnum::block_message_type, message_hash);
...
#ifdef ENABLE_DEBUG_ULOGS
                                    ulog("Disconnecting from peer because their version is too old.  Their version date: ${date}", ("date", peer->tichain_git_revision_unix_timestamp));
#endif
                                }
                        }
                    }
                ...
                const MessageHashType& message_hash)
            {
                ...
                TiValue::client::BlockMessage block_message_to_process(message_to_process.as<TiValue::client::BlockMessage>());
                
                if (block_message_to_process.block.block_size() > TIV_BLOCKCHAIN_MAX_BLOCK_SIZE)
                {
                ...
                auto item_iter = originating_peer->items_requested_from_peer.find(ItemId(TiValue::client::block_message_type, message_hash));
                ...
                else
                {
                    ...
                    auto sync_item_iter = originating_peer->sync_items_requested_from_peer.find(ItemId(TiValue::client::block_message_type,
                    ...
                    if (sync_item_iter != originating_peer->sync_items_requested_from_peer.end())
                    {
                        ...
                        if (originating_peer->idle())
                        {
                            ...
                            if (originating_peer->number_of_unfetched_item_ids > 0 &&
                                originating_peer->ids_of_items_to_get.size() < TIV_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                                fetch_next_batch_of_item_ids_from_peer(originating_peer);
                            else
                                trigger_fetch_sync_items_loop();
                        }
                ...
                fc::exception detailed_error(FC_LOG_MESSAGE(error, "You sent me a block that I didn't ask for, block_id: ${block_id}",
                    ("block_id", block_message_to_process.block_id)
                    ("tichain_git_revision_sha", originating_peer->tichain_git_revision_sha)
                    ("tichain_git_revision_unix_timestamp", originating_peer->tichain_git_revision_unix_timestamp)
            ...
            {
                VERIFY_CORRECT_THREAD();
				fc::time_point request_received_time(_delegate->handle_ntp_time(false));
            ...
            {
                VERIFY_CORRECT_THREAD();
				fc::time_point reply_received_time = _delegate->handle_ntp_time(false);
                ...
                // TODO
				if (abs(originating_peer->clock_offset.to_seconds()) >= 3)
				{
					auto time_triv = reply_received_time - early_ntp_update_time;
					if (abs(time_triv.to_seconds()) > 60)
					{
						early_ntp_update_time = reply_received_time;
						_delegate->handle_ntp_time(true);
					}
				}
            }
                ...
                const GetCurrentConnectionsRequestMessage& get_current_connections_request_message_received)
            {
                ...
                for (const PeerConnectionPtr& peer : _active_connections)
                {
                    ...
                    fc::mutable_variant_object user_data;
                    if (peer->tichain_git_revision_sha)
                        user_data["tichain_git_revision_sha"] = *peer->tichain_git_revision_sha;
                    if (peer->tichain_git_revision_unix_timestamp)
                        user_data["tichain_git_revision_unix_timestamp"] = *peer->tichain_git_revision_unix_timestamp;
                ...
                const Message& message_to_process, const MessageHashType& message_hash)
            {
                ...
                else
                {
                    ...
                    catch (const insufficient_relay_fee&)
                    {
                        ...
                        assert(message_to_process.msg_type == TiValue::client::trx_message_type); // we only support throttling transactions.
                        if (message_to_process.msg_type == TiValue::client::trx_message_type)
                            originating_peer->transaction_fetching_inhibited_until = fc::time_point::now() + fc::seconds(TIV_NET_INSUFFICIENT_RELAY_FEE_PENALTY_SEC);
                        return;
                    }
                    catch (const fc::canceled_exception&)
                    {
            ...
            void NodeImpl::load_configuration(const fc::path& configuration_directory)
            {
                ...
                try
                {
                    ...
                    TiValue::db::LevelMap<std::string, fc::ip::endpoint>::iterator  it = _black_list_db.begin();
                    while (it.valid())
                    {
            ...
            std::vector<PeerStatus> NodeImpl::get_connected_peers() const
            {
                ...
                for (const PeerConnectionPtr& peer : _active_connections)
                {
                    ...
                    if (peer->tichain_git_revision_sha)
                    {
                        std::string revision_string = *peer->tichain_git_revision_sha;
                        if (*peer->tichain_git_revision_sha == TiValue::utilities::git_revision_sha)
                            revision_string += " (same as ours)";
                        else
                            revision_string += " (different from ours)";
                        peer_details["tichain_git_revision_sha"] = revision_string;

                    }
                    if (peer->tichain_git_revision_unix_timestamp)
                    {
                        peer_details["tichain_git_revision_unix_timestamp"] = *peer->tichain_git_revision_unix_timestamp;
                        std::string age_string = fc::get_approximate_relative_time_string(*peer->tichain_git_revision_unix_timestamp);
                        if (*peer->tichain_git_revision_unix_timestamp == fc::time_point_sec(TiValue::utilities::git_revision_unix_timestamp))
                            age_string += " (same as ours)";
                        else if (*peer->tichain_git_revision_unix_timestamp > fc::time_point_sec(TiValue::utilities::git_revision_unix_timestamp))
                            age_string += " (newer than ours)";
                        else
                            age_string += " (older than ours)";
                        peer_details["tichain_git_revision_age"] = age_string;
                    }
            ...
            void NodeImpl::broadcast(const Message& item_to_broadcast, const MessagePropagationData& propagation_data)
            {
                ...
                if (item_to_broadcast.msg_type == TiValue::client::block_message_type)
                {
                    TiValue::client::BlockMessage block_message_to_broadcast = item_to_broadcast.as<TiValue::client::BlockMessage>();
                ...
                else if (item_to_broadcast.msg_type == TiValue::client::trx_message_type)
                {
                    TiValue::client::TrxMessage transaction_message_to_broadcast = item_to_broadcast.as<TiValue::client::TrxMessage>();
            ...
            MessagePropagationData NodeImpl::get_transaction_propagation_data(const TiValue::blockchain::TransactionIdType& transaction_id)
            {
                VERIFY_CORRECT_THREAD();
                return _message_cache.get_message_propagation_data(transaction_id);
            }

            MessagePropagationData NodeImpl::get_block_propagation_data(const TiValue::blockchain::BlockIdType& block_id)
            {
                VERIFY_CORRECT_THREAD();
...
# define INVOKE_IN_IMPL(method_name, ...) \
    return my->method_name(__VA_ARGS__)
        ...
        MessagePropagationData Node::get_transaction_propagation_data(const TiValue::blockchain::TransactionIdType& transaction_id)
        {
            INVOKE_IN_IMPL(get_transaction_propagation_data, transaction_id);
        }

        MessagePropagationData Node::get_block_propagation_data(const TiValue::blockchain::BlockIdType& block_id)
        {
...
#define INVOKE_AND_COLLECT_STATISTICS(returnValue,method_name, ...) \
			...
			fc::time_point_sec StatisticsGatheringNodeDelegateWrapper::handle_ntp_time(bool need_update)
			{
				INVOKE_AND_COLLECT_STATISTICS(fc::time_point_sec, handle_ntp_time, need_update);
			}
            ...
            std::vector<ItemHashType> StatisticsGatheringNodeDelegateWrapper::get_blockchain_synopsis(uint32_t item_type,
                const TiValue::net::ItemHashType& reference_point /* = TiValue::net::item_hash_t() */,
            ...
            /** returns TiValue::blockchain::now() */
