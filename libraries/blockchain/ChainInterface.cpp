...
namespace TiValue {
    namespace blockchain {
        ...
        {
            try {
                if (name.size() < TIV_BLOCKCHAIN_MIN_NAME_SIZE) return false;
                if (name.size() > TIV_BLOCKCHAIN_MAX_NAME_SIZE) return false;
        ...
        {
            try {
                // if( get_head_block_num() < TIV_V0_5_0_FORK_BLOCK_NUM )
                // return is_valid_symbol_name_v1( symbol );

                FC_ASSERT(symbol != "TV", "Symbol must not be TV");

                if (symbol.size() < TIV_BLOCKCHAIN_MIN_SYMBOL_SIZE)
                ...
                if (dots == 0)
                {
                    if (position > TIV_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
                else //dots == 1 means hierarchial asset name
                {
                    if (position - dot_position - 1 > TIV_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
                        FC_ASSERT(false, "Sub-symbol name too large");
                    if (symbol.size() > TIV_BLOCKCHAIN_MAX_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }

                if (symbol.find("TV") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with TV");
        ...
        {
            ShareType pay_per_block = TIV_MAX_DELEGATE_PAY_PER_BLOCK;
        ...
        {
            // if( get_head_block_num() < TIV_V0_4_24_FORK_BLOCK_NUM )
            ...
            const ShareType max_pay_per_two_weeks = max_total_pay_per_two_weeks / TIV_BLOCKCHAIN_NUM_DELEGATES;
            const ShareType registration_fee = (max_pay_per_two_weeks * pay_rate) / 100;
            FC_ASSERT(registration_fee >= 0, "Registration_fee must not be negtive");
            return registration_fee;
        }
        ShareType ChainInterface::get_imessage_need_fee(const string & imessage)const
        {
            if (imessage.size() < TIV_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
            {
                return 0;
            }
            ImessageIdType fee_coe = TIV_BLOCKCHAIN_MIN_MESSAGE_FEE_COE;
            ShareType fee = 0;
            fee = fee_coe * (imessage.size() - TIV_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
            return fee;
        }
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
            ...
            static const ShareType long_symbol_price = 500 * TIV_BLOCKCHAIN_PRECISION; // $10 at $0.02/XTS
        ...
        double ChainInterface::to_pretty_price_double(const Price& price_to_pretty_print)const
        {
            ...
            return fc::variant(string(price_to_pretty_print.ratio * obase_asset->precision / oquote_asset->precision)).as_double() / (TIV_BLOCKCHAIN_MAX_SHARES * 1000);
        }
        ...
        uint64_t ChainInterface::get_required_confirmations()const
        {
            try {
                ...
                return TIV_BLOCKCHAIN_NUM_DELEGATES * 3;
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        TiValue::blockchain::Asset ChainInterface::get_amount(ShareType limit, AssetIdType asset_id)
        {
        ...
        {
            Asset fee(TIV_DEFAULT_TRANSACTION_FEE, 0);
            return fee;
        }
        ...
        {
            Asset fee(TIV_DEFAULT_CONTRACT_MARGIN, 0);
            return fee;
        }
        ...
        {
            return TIV_DEFAULT_TRANSACTION_EXPIRATION_SEC;
        }
        */
		...
		oUploadRequestEntry ChainInterface::get_upload_request(const FileIdType& id)
		{
		...
		void ChainInterface::store_piece_saved_entry(const PieceSavedEntry & entry)
		{
			PieceSavedEntry::store(*this, entry.piece_id, entry);
		}
		void ChainInterface::remove_piece_saved_entry(const FilePieceIdType & id)
		{
		...
		void ChainInterface::remove_file_saved_entry(const FileIdType & id)
		{
			FileSavedEntry::remove(*this,id);
		}
		...
		{
			UploadRequestEntry::store(*this, entry.id, entry);
		}
            ...
            const Address& to_account_address, SignedTransaction& trx)
        {
            try {
				...
				{
					FC_CAPTURE_AND_THROW(invalid_asset_symbol,("Only TV Allowed"));
				}
