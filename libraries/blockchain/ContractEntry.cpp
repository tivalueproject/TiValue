...
{\
read_count = TiValue::utilities::common_fread_int(f, &api_count); \
if (read_count != 1)\
{\
fclose(f); \
throw except_1(); \
...
{\
int api_len = 0; \
read_count = TiValue::utilities::common_fread_int(f, &api_len); \
if (read_count != 1)\
{\
fclose(f); \
throw except_2(); \
...
read_count = TiValue::utilities::common_fread_octets(f, api_buf, api_len); \
if (read_count != 1)\
{\
fclose(f); \
free(api_buf); \
...
{\
read_count = TiValue::utilities::common_fread_int(f, &storage_count); \
if (read_count != 1)\
{\
fclose(f); \
throw except_1(); \
...
read_count = TiValue::utilities::common_fread_int(f, &storage_name_len); \
if (read_count != 1)\
{\
fclose(f); \
throw except_2(); \
...
read_count = TiValue::utilities::common_fread_octets(f, storage_buf, storage_name_len); \
if (read_count != 1)\
{\
fclose(f); \
free(storage_buf); \
...
read_count = TiValue::utilities::common_fread_int(f, (int*)&storage_type); \
if (read_count != 1)\
{\
fclose(f); \
free(storage_buf); \
...
namespace TiValue {
    namespace blockchain{
        ...
        Code::Code(const fc::path& path)
        {
            ...
            for (int i = 0; i < 5; ++i)
            {
                read_count = TiValue::utilities::common_fread_int(f, (int*)&digest[i]);
                if (read_count != 1)
                {
                    fclose(f);
                    FC_THROW_EXCEPTION(TiValue::blockchain::read_verify_code_fail, "Read verify code fail!");
                }
            }

            int len = 0;
            read_count = TiValue::utilities::common_fread_int(f, &len);
            if (read_count != 1 || len < 0 || (len >= (fsize - ftell(f))))
            {
                fclose(f);
                FC_THROW_EXCEPTION(TiValue::blockchain::read_bytescode_len_fail, "Read bytescode len fail!");
            }

            byte_code.resize(len);
            read_count = TiValue::utilities::common_fread_octets(f, byte_code.data(), len);
            if (read_count != 1)
            {
                fclose(f);
                FC_THROW_EXCEPTION(TiValue::blockchain::read_bytescode_fail, "Read bytescode fail!");
            }
            ...
            {
                fclose(f);
                FC_THROW_EXCEPTION(TiValue::blockchain::verify_bytescode_sha1_fail, "Verify bytescode SHA1 fail!");
            }
