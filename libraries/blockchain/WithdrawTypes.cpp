...
namespace TiValue {
    namespace blockchain {
            ...
            bool use_stealth_address)
        {
            ...
            if (memo_message.size() <= TIV_BLOCKCHAIN_MAX_MEMO_SIZE)
            {
    ...
    void from_variant(const variant& var, TiValue::blockchain::MemoData& vo)
    {
        try {
            ...
            if (obj.contains("from"))
                vo.from = obj["from"].as<TiValue::blockchain::PublicKeyType>();
            ...
            if (obj.contains("memo_flags"))
                vo.memo_flags = obj["memo_flags"].as<TiValue::blockchain::MemoFlagsEnum>();
