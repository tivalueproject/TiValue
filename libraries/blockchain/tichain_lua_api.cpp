/**
 * lua module injector header in TiValue
 */
  ...
  namespace lua {
    namespace api {
            ...
            */
            ...
            /**
            * when exception happened, use this api to tell TiValue
            ...
            */
            void GluaChainApi::throw_exception(lua_State *L, int code, const char *error_format, ...)
            {
                ...
                //如果上次的exception code为tichain_API_LVM_LIMIT_OVER_ERROR, 不能被其他异常覆盖
                //只有调用clear清理后，才能继续记录异常
                int last_code = lua::lib::get_lua_state_value(L, "exception_code").int_value;
                if (last_code == tichain_API_LVM_LIMIT_OVER_ERROR
                    && code != tichain_API_LVM_LIMIT_OVER_ERROR)
                {
                    return;
                }
            ...
            */
            ...
            {
                return 0; // FIXME: need fill by TiValue api
            }
            ...
            {
                blockchain::TransactionEvaluationState* pevaluate_state = (blockchain::TransactionEvaluationState*)lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
                blockchain::oContractEntry entry = pevaluate_state->_current_state->get_contract_entry(TiValue::blockchain::Address(std::string(address), AddressType::contract_address));
            ...
            void GluaChainApi::get_contract_address_by_name(lua_State *L, const char *name, char *address, size_t *address_size)
            {
                ...
                */
                ...
                TiValue::blockchain::ChainInterface* cur_state = eval_state_ptr->_current_state;
            ...
            bool GluaChainApi::check_contract_exist_by_address(lua_State *L, const char *address)
            {
                ...
                oContractEntry entry = cur_state->get_contract_entry(TiValue::blockchain::Address(std::string(address), AddressType::contract_address));
            ...
            {
                /*
                char *filename = lutil_concat_str4("tichain_lua_modules", file_separator_str(), "tichain_contract_", name);
            ...
            /**
            * load contract lua byte stream from TiValue api
            */
            ...
            /**
            * store contract lua module byte stream to TiValue api
            */
            //need to delete
            /*
            ...
            */
            ...
            GluaStorageValue GluaChainApi::get_storage_value_from_tichain(lua_State *L, const char *contract_name, std::string name)
            {
                GluaStorageValue null_storage;
                null_storage.type = TiValue::blockchain::StorageValueTypes::storage_value_null;
                ...
                return get_storage_value_from_tichain_by_address(L, entry->id.AddressToString(AddressType::contract_address).c_str(), name);
            }

            GluaStorageValue GluaChainApi::get_storage_value_from_tichain_by_address(lua_State *L, const char *contract_address, std::string name)
            {
                GluaStorageValue null_storage;
                null_storage.type = TiValue::blockchain::StorageValueTypes::storage_value_null;
                ...
                TiValue::blockchain::StorageDataType storage_data = iter->second;

                return TiValue::blockchain::StorageDataType::create_lua_storage_from_storage_data(L, storage_data);
            }

            bool GluaChainApi::commit_storage_changes_to_tichain(lua_State *L, AllContractsChangesMap &changes)
            {
                TiValue::blockchain::TransactionEvaluationState* eval_state_ptr =
                    (TiValue::blockchain::TransactionEvaluationState*)
                    (TiValue::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
            ...
            {
                // TODO
                printf("registered storage %s[%s] to TiValue\n", contract_name, name);
                return true;
            }

            intptr_t GluaChainApi::register_object_in_pool(lua_State *L, intptr_t object_addr, GluaOutsideObjectTypes type)
			{
				auto node = TiValue::lua::lib::get_lua_state_value_node(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY);
				...
				if (node.type == GluaStateValueType::LUA_STATE_VALUE_nullptr)
				{
					...
					TiValue::lua::lib::set_lua_state_value(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY, node.value, node.type);
				}
				else
				{
            ...
            void GluaChainApi::release_objects_in_pool(lua_State *L)
			{
				...
				for (const auto &p : *object_pools)
				{
					...
					for (const auto &object_item : *pool)
					{
						...
						{
							auto stream = (TiValue::lua::lib::GluaByteStream*) object_addr;
							delete stream;
						} break;
						default: {
							continue;
				...
				TiValue::lua::lib::set_lua_state_value(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY, null_state_value, GluaStateValueType::LUA_STATE_VALUE_nullptr);
			}
              ...
              TiValue::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    TiValue::blockchain::TransactionEvaluationState* eval_state_ptr =
                        (TiValue::blockchain::TransactionEvaluationState*)
                        (TiValue::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
                    TiValue::blockchain::ChainInterface* cur_state;
      ...
      const std::string& requestor,
			...
			const std::string& node_id)
		{
			...
			int GluaChainApi::allow_upload_request_wrapper_func(lua_State *L)
			{
				if (lua_gettop(L) < 6)
				{
					throw_exception(L, tichain_API_SIMPLE_ERROR, "allow_upload_request need 6 arguments");
					return 0;
				}
				if (!lua_isstring(L, 1))
				{
				...
				auto piece_array = luaL_create_lua_table_map_in_memory_pool(L);
				luaL_traverse_table_with_nested(L, 3, lua_table_to_map_traverser_with_nested, piece_array, jsons, 0);
				std::vector<PieceUploadInfo> infos;
				PieceUploadInfo info;
				for (size_t i = 1; i <= piece_array->size(); i++)
				{
					auto item = piece_array->at(std::to_string(i));
					auto item_map = item.value.table_value;
					info.pieceid = FilePieceIdType(item_map->at("piece_id").value.string_value);
					info.piece_size = item_map->at("size").value.int_value;
        ...
        if (!lua_isstring(L, 6))
        {
          ...
          return 0;
			...
			int GluaChainApi::allow_piece_saved_wrapper_func(lua_State *L)
			{
				if (lua_gettop(L) < 3)
				{
				...
				bool res = TiValue::lua::api::global_glua_chain_api->allow_piece_saved(L, string(file_id), string(file_piece_id), node);
				lua_pushboolean(L, res);
				return 1;
			}
      ...
      bool GluaChainApi::allow_declare_piece_saved(lua_State *L, const blockchain::FileIdType& file_id, const std::string& piece_id, const std::string& storer, const std::string& node_id) {
        ...
        try {
          ...
          return false;
			...
			const char * GluaChainApi::get_publickey_address(lua_State * L, const char * pubkey)
			{
				PublicKeyType key(pubkey);
				Address addr(key);
				std::string str = addr.AddressToString();
				return TiValue::lua::lib::malloc_and_copy_string(L, str.c_str());

			}
			bool GluaChainApi::contract_api_check(lua_State * L, const std::string & contract_id, const std::string & api_name)
			{
				TiValue::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
				try {
					TiValue::blockchain::TransactionEvaluationState* eval_state_ptr =
					...
					try
					{
						ContractIdType c_id(contract_id,AddressType::contract_address);
						auto contract_entry= eval_state_ptr->_current_state->get_contract_entry(c_id);
						if (!contract_entry.valid())
							return false;
						if (contract_entry->code.abi.count(api_name) < 1)
							return false;
						return true;
					}
					catch (...)
