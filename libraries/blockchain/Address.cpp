...
namespace TiValue {
    namespace blockchain {
        ...
        {
            std::string prefix;
			if (address_type == AddressType::tic_address)
				prefix = TIV_ADDRESS_PREFIX;
			...
			else if (address_type == file_id)
				prefix = FILE_ID_PREFIX;
			else if (address_type == file_piece_id)
				prefix = FILE_PIECE_ID_PREFIX;
        ...
        Address::Address(const TiValue::blockchain::PublicKeyType& pub)
        {
     ...
     //   }
        ...
        Address::operator std::string()const
        {
            ...
            auto checksum = fc::ripemd160::hash((char*)&addr, sizeof(addr));
            memcpy(((char*)&bin_addr) + 20, (char*)&checksum._hash[0], 4);
            return TIV_ADDRESS_PREFIX + fc::to_base58(bin_addr.data, sizeof(bin_addr));
        }
...
namespace fc
{
    void to_variant(const TiValue::blockchain::Address& var, variant& vo)
    {
        vo = std::string(var);
    }
    void from_variant(const variant& var, TiValue::blockchain::Address& vo)
    {
        vo = TiValue::blockchain::Address(var.as_string());
    }
}
