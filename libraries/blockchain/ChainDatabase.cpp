...
namespace TiValue {
    namespace blockchain {
        ...
        namespace detail
        {
            void ChainDatabaseImpl::revalidate_pending()
            {
                ...
                while (itr.valid())
                {
                    ...
                    try
                    {
                        ...
                        count++;
                        if (count > TIV_BLOCKCHAIN_REVALIDATE_MAX_TRX_COUNT)
                            break;
            ...
            bool ChainDatabaseImpl::replay_required(const fc::path& data_dir)
            {
                try {
                    ...
                    return !entry.valid() || entry->value.as_uint64() != TIV_BLOCKCHAIN_DATABASE_VERSION;
                } FC_CAPTURE_AND_RETHROW((data_dir))
            }
            ...
            DigestType ChainDatabaseImpl::initialize_genesis(const optional<path>& genesis_file, const bool statistics_enabled)
            {
                try {
                    ...
                    // Check genesis state
                    FC_ASSERT(config.delegates.size() >= TIV_BLOCKCHAIN_NUM_DELEGATES,
                        "genesis.json does not contain enough initial delegates!",
                        ("required", TIV_BLOCKCHAIN_NUM_DELEGATES)("provided", config.delegates.size()));
                    ...
                    base_asset.symbol = TIV_BLOCKCHAIN_SYMBOL;
                    base_asset.name = TIV_BLOCKCHAIN_NAME;
                    base_asset.description = TIV_BLOCKCHAIN_DESCRIPTION;
                    base_asset.public_data = variant("");
                    base_asset.issuer_account_id = 0;
                    base_asset.precision = TIV_BLOCKCHAIN_PRECISION;
                    base_asset.registration_date = timestamp;
                    base_asset.last_update = timestamp;
                    base_asset.current_share_supply = total_base_supply;
                    base_asset.maximum_share_supply = TIV_BLOCKCHAIN_MAX_SHARES;
                    ...
                    for (const GenesisAsset& asset : config.market_assets)
                    {
                        ...
                        rec.maximum_share_supply = TIV_BLOCKCHAIN_MAX_SHARES;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    for (const auto& trx : block_data.user_transactions)
                    {
                        ...
                        FC_ASSERT(trx.data_size() <= TIV_BLOCKCHAIN_MAX_TRX_SIZE, "trx size is out of the max trx size range");
            ...
            {
                try {
                    // if( pending_state->get_head_block_num() < TIV_V0_6_0_FORK_BLOCK_NUM )
                    ...
                    static const uint32_t blocks_per_two_weeks = 14 * TIV_BLOCKCHAIN_BLOCKS_PER_DAY;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    if (block_num > TIV_BLOCKCHAIN_MAX_UNDO_HISTORY)
                    {
                        const uint32_t old_block_num = block_num - TIV_BLOCKCHAIN_MAX_UNDO_HISTORY;
            ...
            void ChainDatabaseImpl::verify_header(const DigestBlock& block_digest, const PublicKeyType& block_signee)const
            {
                try {
                    ...
                    if (block_digest.timestamp.sec_since_epoch() % TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0)
                    ...
                    fc::time_point_sec now = TiValue::blockchain::now();
                    auto delta_seconds = (block_digest.timestamp - now).to_seconds();
                    if (block_digest.timestamp > (now + TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            /**
             *  A block should be produced every TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
                ...
                const PendingChainStatePtr& pending_state)const
            {
                try {
                    ...
                    if (head_block.block_num > 0) block_timestamp = head_block.timestamp + TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    ...
                    for (; block_timestamp < block_header.timestamp;
                        block_timestamp += TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
                    ...
                    if (required_confirmations > TIV_BLOCKCHAIN_NUM_DELEGATES * 3)
                        required_confirmations = 3 * TIV_BLOCKCHAIN_NUM_DELEGATES;
            ...
            {
                try {//todo remove????
                    // if( pending_state->get_head_block_num() < TIV_V0_7_0_FORK_BLOCK_NUM )
                    // return update_active_delegate_list_v1( block_num, pending_state );

                    if (block_num % TIV_BLOCKCHAIN_NUM_DELEGATES != 0)
                        return;
            ...
            void ChainDatabaseImpl::extend_chain(const FullBlock& block_data)
            {
                try {
                    ...
                    try
                    {
                        ...
                        if (TiValue::client::g_client->get_wallet() != nullptr&&TiValue::client::g_client->get_wallet()->is_open())
                        {
                            if (!TiValue::client::g_client->get_wallet()->get_my_delegates(TiValue::wallet::enabled_delegate_status).empty())
                            {
                                auto fee = TiValue::client::g_client->get_delegate_config().transaction_min_fee;
                                _relay_fee = fee > TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee : TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                            else
                            {
                                auto fee = TiValue::client::g_client->get_wallet()->get_transaction_fee();
                                _relay_fee = fee.amount > TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee.amount : TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                                //_relay_fee = TIV_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                        }
                    }
                    catch (const fc::exception& e)
                    ...
                    if ((now() - block_data.timestamp).to_seconds() < TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
            ...
            void ChainDatabaseImpl::pop_block()
            {
                try {
                    ...
                    TiValue::blockchain::PendingChainStatePtr undo_state_ptr = std::make_shared<TiValue::blockchain::PendingChainState>(undo_state);
        ...
        {
            return get_delegates_by_vote(0, TIV_BLOCKCHAIN_NUM_DELEGATES);
        }
            ...
            try {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        my->open_database(data_dir);
                        store_property_entry(PropertyIdType::database_version, variant(TIV_BLOCKCHAIN_DATABASE_VERSION));
                        ...
                        const auto insert_block = [&](const FullBlock& block)
                        {
                            if (blocks_indexed % 200 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    progress = float(blocks_indexed * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC) / seconds;
                                }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (last_known_block_num > TIV_BLOCKCHAIN_MAX_UNDO_HISTORY)
                                    my->_min_undo_block = last_known_block_num - TIV_BLOCKCHAIN_MAX_UNDO_HISTORY;
                        ...
                        catch (TiValue::blockchain::store_and_index_a_seen_block&  e)
                        {
            ...
            const std::vector<AccountIdType>& ordered_delegates)const
        {
            try {
                ...
                auto delegate_pos = slot_number % TIV_BLOCKCHAIN_NUM_DELEGATES;
        ...
        optional<time_point_sec> ChainDatabase::get_next_producible_block_timestamp(const vector<AccountIdType>& delegate_ids)const
        {
            try {
                ...
                if (next_block_time <= now()) next_block_time += TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                auto last_block_time = next_block_time + (TIV_BLOCKCHAIN_NUM_DELEGATES * TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

                auto active_delegates = get_active_delegates();
                for (; next_block_time < last_block_time; next_block_time += TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
                {
            ...
            const ShareType required_fees, bool contract_vm_exec, bool skip_signature_check, bool throw_exec_exception)
        {
            try {
                ...
                catch (TiValue::blockchain::ignore_check_required_fee_state& e)
                {
        ...
        BlockForkData ChainDatabase::push_block(const FullBlock& block_data)
        {
            try {
                ...
                if (head_block_num > TIV_BLOCKCHAIN_MAX_UNDO_HISTORY &&
                    block_data.block_num <= (head_block_num - TIV_BLOCKCHAIN_MAX_UNDO_HISTORY))
                {
                    elog("block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
                        ...
                        ("head_block_num", head_block_num)("undo_history", TIV_BLOCKCHAIN_MAX_UNDO_HISTORY));
        ...
        */
        double ChainDatabase::get_average_delegate_participation()const
        {
            try {
                ...
                const auto now = TiValue::blockchain::now();
                if (head_num < 1)
                {
                    return 0;
                }
                else if (head_num <= TIV_BLOCKCHAIN_NUM_DELEGATES)
                {
                    // what percent of the maximum total blocks that could have been produced
                    // have been produced.
                    const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                ...
                else
                {
                    ...
                    const auto starting_time = get_block_header(head_num - TIV_BLOCKCHAIN_NUM_DELEGATES).timestamp;
                    const auto expected_production = (now - starting_time).to_seconds() / TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    return 100 * double(TIV_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
                }
            } FC_RETHROW_EXCEPTIONS(warn, "")
        }
        ...
        TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction(const SignedTransaction& trx, bool override_limits, bool contract_vm_exec, bool cache)
        {
            try {
                ...
                if (!override_limits)
                {
                    if (my->_pending_fee_index.size() > TIV_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE)
                    {
                        auto overage = my->_pending_fee_index.size() - TIV_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
        ...
        FullBlock ChainDatabase::generate_block(const time_point_sec block_timestamp, const DelegateConfig& config)
        {
            try {
                ...
                //if( pending_state->get_head_block_num() >= TIV_V0_4_9_FORK_BLOCK_NUM )
                        ...
                        try
                        {
                            ...
                            auto pending_trx_state = origin_trx_state;
                            {
                                ...
                                if (trx_eval_state->p_result_trx.operations.size() > 0)
                                {
                                    ...
                                    if (iMessageLength > TIV_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
                                    {
                                        imessage_fee = config.transaction_imessage_min_fee_coe * (iMessageLength - TIV_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
                                    }
                ...
                // if( new_block.block_num < TIV_V0_7_0_FORK_BLOCK_NUM )
        ...
        string ChainDatabase::export_fork_graph(uint32_t start_block, uint32_t end_block, const fc::path& filename)const
        {
            ...
            for (auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr)
            {
                ...
                {
                    unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
        ...
        bool ChainDatabase::store_account_entries_for_sandbox(const vector<TiValue::wallet::WalletAccountEntry>& vec_account_entry)
        {
        ...
        oResultTIdEntry TiValue::blockchain::ChainDatabase::contract_lookup_resultid_by_reqestid(const TransactionIdType & id) const
        {
        ...
        void TiValue::blockchain::ChainDatabase::contract_store_resultid_by_reqestid(const TransactionIdType & req, const ResultTIdEntry & res)
        {
            my->_request_to_result_iddb.store(req, res);
        }

        void TiValue::blockchain::ChainDatabase::contract_erase_resultid_by_reqestid(const TransactionIdType & req)
        {
            my->_request_to_result_iddb.remove(req);
        }

        oRequestIdEntry TiValue::blockchain::ChainDatabase::contract_lookup_requestid_by_resultid(const TransactionIdType &id) const
        {
        ...
        void TiValue::blockchain::ChainDatabase::contract_store_requestid_by_resultid(const TransactionIdType & res, const RequestIdEntry & req)
        {
            my->_result_to_request_iddb.store(res, req);
        }

        void TiValue::blockchain::ChainDatabase::contract_erase_requestid_by_resultid(const TransactionIdType & result)
        {
            my->_result_to_request_iddb.remove(result);
        }

        oContractinTrxEntry TiValue::blockchain::ChainDatabase::contract_lookup_contractid_by_trxid(const TransactionIdType &id) const
        {
        ...
        oContractTrxEntry TiValue::blockchain::ChainDatabase::contract_lookup_trxid_by_contract_id(const ContractIdType &id) const
        {
        ...
        void TiValue::blockchain::ChainDatabase::contract_store_contractid_by_trxid(const TransactionIdType & tid, const ContractinTrxEntry & cid)
        {
            my->_trx_to_contract_iddb.store(tid, cid);
        }

        void TiValue::blockchain::ChainDatabase::contract_store_trxid_by_contractid(const ContractIdType & cid, const ContractTrxEntry & tid)
        {
            my->_contract_to_trx_iddb.store(cid, tid);
        }

        void TiValue::blockchain::ChainDatabase::contract_erase_trxid_by_contract_id(const ContractIdType &id)
        {
            my->_contract_to_trx_iddb.remove(id);
        }

        void TiValue::blockchain::ChainDatabase::contract_erase_contractid_by_trxid(const TransactionIdType & tid)
        {
            my->_trx_to_contract_iddb.remove(tid);
        }

		void TiValue::blockchain::ChainDatabase::uploadrequest_insert_into_id_map(const FileIdType & file_id, const UploadRequestEntry & entry)
		{
		...
		{
			my->_piece_saved_db.store(piece_id, entry);
		}

		void TiValue::blockchain::ChainDatabase::filesaved_insert_into_id_map(const FileIdType & piece_id, const FileSavedEntry & entry)
		{
      ...
      for (; itr != my->_upload_request_db.unordered_end(); itr++)
      {
		...
		{
			auto it = my->_piece_saved_db.unordered_find(file_id);
			if (it != my->_piece_saved_db.unordered_end())
				return it->second;
			return oPieceSavedEntry();
		}

		oFileSavedEntry TiValue::blockchain::ChainDatabase::filesaved_lookup_by_id(const FileIdType & file_id)const
		{
			auto it = my->_file_saved_db.unordered_find(file_id);
			if (it != my->_file_saved_db.unordered_end())
				return it->second;
		...
		{
			my->_piece_saved_db.remove(file_id);
		}

		void TiValue::blockchain::ChainDatabase::filesaved_remove_by_id(const FileIdType & file_id)
		{
    ...
    std::vector<FileSavedEntry> TiValue::blockchain::ChainDatabase::get_file_saved() const
    {
      ...
      for (auto itr = my->_file_saved_db.unordered_begin(); itr != my->_file_saved_db.unordered_end(); itr++) {
		...
		void TiValue::blockchain::ChainDatabase::scan_upload_request(const function<void(const UploadRequestEntry&)> callback) const
		{
			for (auto iter = my->_upload_request_db.unordered_begin();
				iter != my->_upload_request_db.unordered_end(); ++iter)
			{
				callback(iter->second);
			}
