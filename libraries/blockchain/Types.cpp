        ...
        PublicKeyType::PublicKeyType(const std::string& base58str)
        {
            ...
            try
            {
                if (is_valid_v1(base58str))
                    prefix = std::string("TV");
            }
            catch (...)
            {
            }
            const size_t prefix_len = prefix.size();
            FC_ASSERT(base58str.size() > prefix_len, "Address must be longger than TIV_ADDRESS_PREFIX");
...
namespace fc
{
    ...
    {
        vo = TiValue::blockchain::PublicKeyType(var.as_string());
    }
} // fc
