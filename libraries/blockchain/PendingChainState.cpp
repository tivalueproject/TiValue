...
namespace TiValue {
    namespace blockchain {
		...
		void PendingChainState::uploadrequest_insert_into_id_map(const FileIdType & file_id, const UploadRequestEntry & entry)
		{
		...
		void PendingChainState::piecesaved_insert_into_id_map(const FilePieceIdType & piece_id, const PieceSavedEntry & entry)
		{
			...
			_piece_saved_remove.erase(piece_id);
		}
		void PendingChainState::filesaved_insert_into_id_map(const FileIdType & piece_id, const FileSavedEntry & entry)
		{
      ...
      //_savedecl_db.erase(file_id);
		...
		oUploadRequestEntry PendingChainState::uploadrequest_lookup_by_id(const FileIdType & file_id) const
		{
			...
			if (it != _upload_request_db.end())
				return it->second;
			if (_upload_request_remove.count(file_id) > 0)
		...
		{
			auto it = _piece_saved_db.find(file_id);
			if (it != _piece_saved_db.end())
				return it->second;
			if (_piece_saved_remove.count(file_id) > 0)
				return oPieceSavedEntry();
			const ChainInterfacePtr prev_state = _prev_state.lock();
			if (!prev_state)
				return oPieceSavedEntry();
			return prev_state->lookup<PieceSavedEntry>(file_id);
		}
		oFileSavedEntry PendingChainState::filesaved_lookup_by_id(const FileIdType & file_id)const
		{
			auto it = _file_saved_db.find(file_id);
			if (it != _file_saved_db.end())
				return it->second;
			if (_file_saved_remove.count(file_id) > 0)
		...
		void PendingChainState::piecesaved_remove_by_id(const FilePieceIdType & file_id)
		{
			...
			_piece_saved_remove.insert(file_id);
		}
		void PendingChainState::filesaved_remove_by_id(const FileIdType & file_id)
		{
        ...
        TiValue::blockchain::BlockIdType PendingChainState::get_block_id(uint32_t block_num) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
        ...
        TiValue::blockchain::SignedBlockHeader PendingChainState::get_block_header(const BlockIdType& id) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
