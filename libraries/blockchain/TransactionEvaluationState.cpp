    ...
    namespace blockchain {
        ...
        void TransactionEvaluationState::post_evaluate()
        {
            try {
                ...
                /* if( _current_state->get_head_block_num() >= TIV_V0_6_4_FORK_BLOCK_NUM )
                {
                for( const auto& op : trx.operations )
                {
                if( operation_type_enum( op.type ) == cover_op_type )
                {
                FC_ASSERT( balance[ 0 ] <= TIV_BLOCKCHAIN_PRECISION );
                }
                }
                }*/
            } FC_CAPTURE_AND_RETHROW()
        ...
        void TransactionEvaluationState::evaluate(const SignedTransaction& trx_arg, bool ignore_state)
        {
            ...
            try {
                ...
                try {
                    ...
                    if ((_current_state->now() + TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC + TIV_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));

                    if (origin_trx_basic_verify(trx) && (trx.data_size() > TIV_BLOCKCHAIN_MAX_TRX_SIZE))
                    {
        ...
        SignedTransaction TransactionEvaluationState::sandbox_evaluate(const SignedTransaction &trx_arg, bool& ignore_check_required_fee)
        {
            try
            {
                ...
                try
                {
                    ...
                    if ((_current_state->now() + TIV_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));
        ...
        bool TransactionEvaluationState::is_contract_op(const TiValue::blockchain::OperationTypeEnum& op_type)const
        {
            if (op_type >= TiValue::blockchain::OperationTypeEnum::register_contract_op_type
                && op_type <= TiValue::blockchain::OperationTypeEnum::transfer_contract_op_type)
                return true;
            return false;
        }
        ...
        bool TransactionEvaluationState::origin_trx_basic_verify(const SignedTransaction& trx)const
        {
            ...
            for (const auto& op : trx.operations)
            {
                if (op.type == TiValue::blockchain::OperationTypeEnum::transaction_op_type)
                    return false;
