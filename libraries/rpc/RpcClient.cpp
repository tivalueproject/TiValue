...
namespace TiValue {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcClientImpl : public TiValue::rpc_stubs::CommonApiRpcClient
            {
            public:
            ...
            void RpcClientImpl::connect_to(const fc::ip::endpoint& remote_endpoint,
                const TiValue::blockchain::PublicKeyType& remote_public_key)
            {
                fc::buffered_istream_ptr buffered_istream;
                fc::buffered_ostream_ptr buffered_ostream;

                if (remote_public_key != TiValue::blockchain::PublicKeyType())
                {
                    net::StcpSocketPtr socket = std::make_shared<TiValue::net::StcpSocket>();
