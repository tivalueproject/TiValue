...
namespace TiValue {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcServerImpl : public TiValue::rpc_stubs::CommonApiRpcServer
            {
            public:
                ...
                typedef std::map<std::string, TiValue::api::MethodData> MethodMapType;
                ...
                RpcServerImpl(TiValue::client::Client* client) :
                    _client(client),
                ...
                virtual TiValue::api::CommonApi* get_client() const override;
                ...
                virtual void store_method_metadata(const TiValue::api::MethodData& method_metadata);

                std::string help(const std::string& command_name) const;

                std::string make_short_description(const TiValue::api::MethodData& MethodData, bool show_decription = true) const
                {
                    std::string help_string;
                    std::stringstream short_description;
                    short_description << std::setw(100) << std::left;
                    help_string = MethodData.name + " ";
                    for (const TiValue::api::ParameterData& parameter : MethodData.parameters)
                    {
                        if (parameter.classification == TiValue::api::required_positional)
                            help_string += std::string("<") + parameter.name + std::string("> ");
                        else if (parameter.classification == TiValue::api::optional_named)
                            help_string += std::string("{") + parameter.name + std::string("} ");
                        else if (parameter.classification == TiValue::api::required_positional_hidden)
                            continue;
                        else
                            help_string += std::string("[") + parameter.name + std::string("] ");
                    }
                    ...
                    if (show_decription)
                    {
                        short_description << "  " << MethodData.description << "\n";
                    }
                    else
                    {
                        short_description << "\n";
                    ...
                    try {
                        if (_config.rpc_user.size())
                        {
                                ...
                                _config.rpc_password != password)
                            {
                                ...
                                s.add_header("WWW-Authenticate", "Basic realm=\"TiValue wallet\"");
                ...
                fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s)
                {
                    ...
                    try {
                        ...
                        if (fc::time_point(TiValue::blockchain::now()) - _last_cache_clear_time > fc::seconds(TIV_BLOCKCHAIN_BLOCK_INTERVAL_SEC))
                        {
                            _last_cache_clear_time = TiValue::blockchain::now();
                            _call_cache.clear();
                        }
                ...
                void encrypted_accept_loop(const TiValue::blockchain::PrivateKeyType& server_key)
                {
                ...
                void register_methods(fc::rpc::json_connection_ptr con)
                {
                    ...
                    register_CommonApi_methods(con);
                } // register methods
                    ...
                    const fc::variants& arguments_from_caller)
                {
                    ...
                    if (!MethodData.method)
                    {
                        ...
                        return direct_invoke_positional_method(MethodData.name, arguments_from_caller);
                    }
                    //ilog("method: ${method_name}, arguments: ${params}", ("method_name", MethodData.name)("params",arguments_from_caller));
                    if (MethodData.prerequisites & TiValue::api::wallet_open)
                        verify_wallet_is_open();
                    if (MethodData.prerequisites & TiValue::api::wallet_unlocked)
                        verify_wallet_is_unlocked();
                    if (MethodData.prerequisites & TiValue::api::connected_to_network)
                        verify_connected_to_network();
                    if (MethodData.prerequisites & TiValue::api::sandbox_open)
                    ...
                    {
                        if (parameter.classification == TiValue::api::required_positional
                            || parameter.classification == TiValue::api::required_positional_hidden)
                        {
                        ...
                        else if (parameter.classification == TiValue::api::optional_positional)
                        {
                    ...
                    //ilog("After processing: method: ${method_name}, arguments: ${params}", ("method_name", MethodData.name)("params",modified_positional_arguments));

                    return MethodData.method(modified_positional_arguments);
                }
            ...
            TiValue::api::CommonApi* RpcServerImpl::get_client() const
            {
                return _client;
            }
            ...
            void RpcServerImpl::store_method_metadata(const TiValue::api::MethodData& method_metadata)
            {
                _self->register_method(method_metadata);
            }

            //register_method(MethodData{"login", JSON_METHOD_IMPL(login),
            ...
            std::string RpcServerImpl::help(const std::string& command_name) const
            {
                ...
                { //display detailed description of requested command
                    ...
                    if (alias_iter != _alias_map.end())
                    {
                        ...
                        const TiValue::api::MethodData& MethodData = method_iter->second;
                        help_string += "Usage:\n";
                        help_string += make_short_description(MethodData);
                        help_string += MethodData.detailed_description;
                        if (MethodData.aliases.size() > 0)
                            help_string += "\naliases: " + boost::join(MethodData.aliases, ", ");
                    }
                    else
                    {
        ...
        RpcServer::RpcServer(TiValue::client::Client* client) :
            my(new detail::RpcServerImpl(client))
        {
            my->_thread = &fc::thread::current();
            my->_self = this;
            try {
                my->register_CommonApi_method_metadata();
        ...
        const TiValue::api::MethodData& RpcServer::get_method_data(const std::string& method_name)
        {
        ...
        std::vector<TiValue::api::MethodData> RpcServer::get_all_method_data() const
        {
            std::vector<TiValue::api::MethodData> result;
            for (const detail::RpcServerImpl::MethodMapType::value_type& value : my->_method_map)
                result.emplace_back(value.second);
            return result;
        }

        void RpcServer::register_method(TiValue::api::MethodData data)
        {
