...
namespace glua {
    namespace parser {
				...
				if (mr->is_complex())
				{
						...
						|| prop_mr->as_final()->token.type == TOKEN_RESERVED::LTK_NAME))
					{
						...
						if (obj_type_info->is_contract_type() && is_checking_contract)
						{
							...
							{
								if (glua::util::vector_contains(TiValue::lua::lib::contract_special_api_names, prop_name)
									|| prop_name == "storage")
								{
        ...
        {
            auto *global_var_infos = TiValue::lua::lib::get_globalvar_type_infos();
		...
		bool GluaTypeChecker::check_contract_syntax_tree_type(MatchResult *mr, GluaTypeInfoP *ret_contract_type, GluaTypeInfoP *ret_contract_storage_type_out)
        {
			...
			for(const auto &api_type : ret_type->record_props)
			{
				...
				else if(glua::util::vector_contains(TiValue::lua::lib::contract_special_api_names, api_prop_name)
					&& !glua::util::vector_contains(TiValue::lua::lib::contract_int_argument_special_api_names, api_prop_name))
				{
				...
				else if(glua::util::vector_contains(TiValue::lua::lib::contract_int_argument_special_api_names, api_prop_name))
				{
        ...
        bool GluaTypeChecker::check_syntax_tree_type(MatchResult *mr, GluaTypeInfoP ret_type, bool is_contract)
        {
			...
			ldf = std::make_shared<TiValue::lua::core::LuaDebugFileInfo>();
                ...
                if (mr->is_complex())
                {
                    ...
                    if (cmr->size() >= 2)
                    {
						...
						else if(item2->is_final() && item2->as_final()->token.token == ":" && cmr->size()==4)
						{
							...
							{
								if(glua::util::vector_contains(TiValue::lua::lib::contract_special_api_names, funcname_token.token))
								{
