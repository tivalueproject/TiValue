...
{
    if (strcmp(vartypename, "string") == 0)
        return LUA_TSTRING;
    else if (strcmp(vartypename, "integer") == 0)
        return LUA_TNUMBER;
    else if (strcmp(vartypename, "boolean") == 0)
        return LUA_TBOOLEAN;
    else if (strcmp(vartypename, "table") == 0)
        return LUA_TTABLE;
    else if (strcmp(vartypename, "function") == 0)
...
static void statlist(LexState *ls) {
    ...
    if (ls->L->force_stopping)
    {
        if (nullptr == ls->L->compile_error)
        {
            lua_set_compile_error(ls->L, "forced stoppping in compile");
        }
    }
}
...
static void gotostat(LexState *ls, int pc) {
	...
	*/
    //const char *msg = luaO_pushfstring(ls->L, "TiValue lua not support `goto` symbol");
    //ls->L->force_stopping = true;
    //TiValue::lua::api::throw_exception(ls->L, tichain_API_PARSER_ERROR, msg);
    //semerror(ls, msg);
    //if (ls->L->force_stopping)
    //    return;
    //luaX_syntaxerror(ls, msg);
