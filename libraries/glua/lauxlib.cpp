...
static int typeerror(lua_State *L, int arg, const char *tname) {
    ...
    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, msg);
...
static int errfile(lua_State *L, const char *what, int fnameindex) {
    ...
    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, luaL_checkstring(L, -1));
...
static const char *progname = "tichain_lua";
...
LUA_API int lua_docompiledfile(lua_State *L, const char *filename)
{
	...
	{
      global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "error in load bytecode file, %s", e.what());
		return LUA_ERRRUN;
	}
}
...
int luaL_require_module(lua_State *L)
{
    if (lua_gettop(L) < 1)
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "require need 1 argument of contract name");
        return 0;
    }
...
static int contract_api_wrapper_func(lua_State *L)
{
	...
	// contract_id压栈
	auto contract_id_stack = TiValue::lua::lib::get_using_contract_id_stack(L, true);
...
{
    int *stopped_pointer = TiValue::lua::lib::get_lua_state_value(L, LUA_STATE_STOP_TO_RUN_IN_LVM_STATE_MAP_KEY).int_pointer_value;
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        if (lua_istable(L, -1)) {
            ...
            lua_pushnil(L);
            int apis_count = 0;
            char *contract_apis[tichain_CONTRACT_APIS_LIMIT];
            ...
            while (lua_next(L, it))
            {
                ...
                lua_pop(L, 1);
                // store module info into TiValue, limit not too many apis
                if (strlen(key) > tichain_CONTRACT_API_NAME_MAX_LENGTH) {
                    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "contract module api name must be less than 1024 characters\n");
                    return false;
                }
            ...
            // if the contract info stored in TiValue before, fetch and check whether the apis are the same. if not the same, error
            /*char *stored_contract_apis[tichain_CONTRACT_APIS_LIMIT];
			...
			const char *msg = "this TiValue contract not return a table";
        ...
        global_glua_chain_api->get_contract_address_by_name(L, TiValue::lua::lib::unwrap_any_contract_name(name.c_str()).c_str(), contract_id, &contract_id_size);
...
void lua_fill_contract_info_for_use(lua_State *L)
{
    ...
    lua_getglobal(L, "TiValue"); // {}, TiValue
    lua_pushvalue(L, -2); // {}, TiValue, {}
    lua_getfield(L, -2, "storage_mt"); // {}, TiValue, {}, storage_mt
    lua_setmetatable(L, -2); // {}, TiValue, {}   设置storage的metatable为storage_mt
    lua_pop(L, 3);
}
...
static std::string get_contract_name_using_in_lua(std::string namestr)
{
    ...
    return use_self_name ? CURRENT_CONTRACT_NAME : TiValue::lua::lib::unwrap_any_contract_name(namestr.c_str());
}
...
static std::string get_contract_id_using_in_lua(lua_State *L, std::string namestr, bool is_pointer, bool is_stream)
{
    ...
    else if (!is_pointer && !is_stream)
    {
        ...
        global_glua_chain_api->get_contract_address_by_name(L, TiValue::lua::lib::unwrap_any_contract_name(namestr.c_str()).c_str(), address, &address_len);
        address[CONTRACT_ID_MAX_LENGTH-1] = '\0';
        return address;
    }
    else
    {
		auto name = TiValue::lua::lib::unwrap_any_contract_name(namestr.c_str());
		char address[CONTRACT_ID_MAX_LENGTH];
		memset(address, 0x0, sizeof(char) * CONTRACT_ID_MAX_LENGTH);
		size_t address_len = 0;
        global_glua_chain_api->get_contract_address_by_name(L, TiValue::lua::lib::unwrap_any_contract_name(name.c_str()).c_str(), address, &address_len);
		address[CONTRACT_ID_MAX_LENGTH - 1] = '\0';
		return address;
...
int luaL_import_contract_module_from_address(lua_State *L)
{
    if (lua_gettop(L) < 1)
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "import_contract_from_address need 1 argument of contract name");
        return 0;
    }
    ...
    if (glua::util::ends_with(std::string(name), std::string(".lua"))
        ...
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "this contract byte stream not matched with the info stored in TiValue api");
        return 0;
    }
    std::string name_str;
    name_str = std::string(ADDRESS_CONTRACT_PREFIX) + TiValue::lua::lib::unwrap_any_contract_name(contract_id);
    name = name_str.c_str();
    auto unwrap_name = TiValue::lua::lib::unwrap_any_contract_name(contract_id);
    ...
    if (!exists)
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "this contract not found");
        return 0;
    }
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        if (lua_istable(L, -1)) {
            ...
            int apis_count = 0;
            char *contract_apis[tichain_CONTRACT_APIS_LIMIT];
            memset(contract_apis, 0x0, tichain_CONTRACT_APIS_LIMIT*sizeof(char*));
            while (lua_next(L, it))
            {
                if (apis_count >= tichain_CONTRACT_APIS_LIMIT)
                {
                    lua_pop(L, 1);
                    break;
                }
                ...
                // store module info into TiValue, limit not too many apis
                if (strlen(key) > tichain_CONTRACT_API_NAME_MAX_LENGTH) {
                    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "contract module api name must be less than %d characters", tichain_CONTRACT_API_NAME_MAX_LENGTH);
            ...
            if (global_glua_chain_api->get_stored_contract_info_by_address(L, address.c_str(), stored_contract_info))
            {
                ...
                // found this contract stored in the TiValue api before
                if (stored_contract_info->contract_apis.size() != apis_count)
                {
                    char error_msg[LUA_COMPILE_ERROR_MAX_LENGTH];
                    snprintf(error_msg, LUA_COMPILE_ERROR_MAX_LENGTH - 1, "this contract byte stream not matched with the info stored in TiValue api, need %d apis but only found %d", stored_contract_info->contract_apis.size(), apis_count);
                    if (strlen(L->compile_error) < 1)
                        memcpy(L->compile_error, error_msg, LUA_COMPILE_ERROR_MAX_LENGTH);
                    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, error_msg);
                    TiValue::lua::lib::notify_lua_state_stop(L);
                    return 0;
                }
                for (auto i = 0; i < apis_count; ++i)
                {
                    ...
                    {
                        char error_msg[LUA_COMPILE_ERROR_MAX_LENGTH];
                        snprintf(error_msg, LUA_COMPILE_ERROR_MAX_LENGTH - 1, "the contract api not match info stored in TiValue");
                        if (strlen(L->compile_error) < 1)
                            memcpy(L->compile_error, error_msg, LUA_COMPILE_ERROR_MAX_LENGTH);
                        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, error_msg);
                        TiValue::lua::lib::notify_lua_state_stop(L);
            ...
            char error_msg[LUA_COMPILE_ERROR_MAX_LENGTH] = "\0";
            /*
            snprintf(error_msg, LUA_COMPILE_ERROR_MAX_LENGTH - 1, "this TiValue contract not return a table");
            if (strlen(L->compile_error) < 1)
            memcpy(L->compile_error, error_msg, LUA_COMPILE_ERROR_MAX_LENGTH);
            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, error_msg);
            */
            lcompile_error_set(L, error_msg, "this TiValue contract not return a table");
            return 0;
        }
        ...
        {
			for(const auto &special_api_name : TiValue::lua::lib::contract_special_api_names)
			{
				lua_pushnil(L);
    ...
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "this TiValue contract not return a table");
        return 0;
    }
    /*
    if (lua_getfield(L, 2, filename) == LUA_TNIL) {   // module set no value?
 ...
 * restrain name must be pure file name, with no '.lua' extension. And can load bytestream from TiValue api.
...
int luaL_import_contract_module(lua_State *L)
{
    ...
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "import_contract need 1 string argument of contract name");
        return 0;
    }
    ...
    {
        name_str = TiValue::lua::lib::wrap_contract_name(origin_contract_name);
        name = name_str.c_str();
    }
    auto unwrap_name = TiValue::lua::lib::unwrap_any_contract_name(origin_contract_name);
    const char *filename = name;
    lua_settop(L, 1);  /* _LOADED table will be at index 2 */
    lua_getfield(L, LUA_REGISTRYINDEX, "_LOADED");
    lua_getfield(L, 2, filename);  /* _LOADED[name] */
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        if (lua_istable(L, -1)) {
            ...
            if (global_glua_chain_api->get_stored_contract_info_by_address(L, address.c_str(), stored_contract_info))
            {
                ...
                for (auto i = 0; i < apis_count; ++i)
                {
                    ...
                    for (int j = 0; j < stored_contract_info->contract_apis.size(); ++j)
                    {
                        ...
                        {
                            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "empty contract api name");
                            return 0;
                        }
                        if (strcmp(a, b) == 0)
                        {
                    ...
                    if (!matched)
                    {
                        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "the contract api not match info stored in TiValue");
                        TiValue::lua::lib::notify_lua_state_stop(L);
                        return 0;
                    }
                }
                ...
                {
                    // check all global variables not changed, don't call code eg. ```_G['abc'] = nil; abc = 1;```
                    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "contract can't use global variables");
                    TiValue::lua::lib::notify_lua_state_stop(L);
                    return 0;
                }
            }
            ...
            {
                global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "contract info not stored before");
                TiValue::lua::lib::notify_lua_state_stop(L);
                return 0;
            }
        }
        ...
        if (!is_starting_contract)
        {
			for(const auto &api_name : TiValue::lua::lib::contract_special_api_names)
			{
				lua_pushnil(L);
...
)
{
    ...
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "can't find this contract");
    ...
    std::string wrapper_contract_name_str = TiValue::lua::lib::wrap_contract_name(contract_name);
    std::string unwrapper_name = TiValue::lua::lib::unwrap_any_contract_name(contract_name);
    if (!is_address)
    ...
    if (!lua_toboolean(L, -1))  /* is it there? */
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "need load contract before execute contract api");
        lua_pushinteger(L, LUA_ERRRUN);
        return 1;
    }
    if (!lua_istable(L, -1))
    ...
    if (lua_isfunction(L, -1))
    {
		...
		if (glua::util::vector_contains(TiValue::lua::lib::contract_int_argument_special_api_names, api_name_str))
		{
    ...
    {
		global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "Can't find api %s in this contract", api_name_str.c_str());
		lua_pop(L, 1);
		return 0;
    }
    lua_pop(L, 1);
...
{
	auto contract_address = TiValue::lua::lib::malloc_managed_string(L, CONTRACT_ID_MAX_LENGTH + 1);
	...
	if (address_size > 0)
	{
		...
		TiValue::lua::lib::set_lua_state_value(L, STARTING_CONTRACT_ADDRESS, value, LUA_STATE_VALUE_STRING);
	}
	...
	struct LuaCompileFilePreloadResult *result, bool use_type_check = false, GluaModuleByteStreamP stream = nullptr, bool is_contract = false)
{
	...
	auto code = TiValue::lua::lib::pre_modify_lua_source_code(L, filename, error, nullptr, use_type_check, stream, is_contract);
	if (global_glua_chain_api->has_exception(L))
    {
    ...
    if (status != 0)
    {
		...
		if (lf.f) fclose(lf.f);
        const char *error_str = "parse TiValue lua code error";
        if (strlen(L->compile_error) > 0)
            error_str = L->compile_error;
        if (error)
            strncpy(error, error_str, LUA_COMPILE_ERROR_MAX_LENGTH);
        global_glua_chain_api->throw_exception(L, tichain_API_PARSER_ERROR, error_str);
        // lcompile_error_set(L, error, "parse TiValue lua code error");
        return LUA_ERRFILE;
    }
    readstatus = ferror(lf.f);
...
{
    TiValue::lua::lib::GluaStateScope scope;
    lua_State* L = scope.L();
    LoadF lf;
...
{
    auto lua_table_map_list_p = TiValue::lua::lib::get_lua_state_value(L, LUA_TABLE_MAP_LIST_STATE_MAP_KEY).pointer_value;
    if (nullptr == lua_table_map_list_p)
    {
        ...
        TiValue::lua::lib::set_lua_state_value(L, LUA_TABLE_MAP_LIST_STATE_MAP_KEY, value, LUA_STATE_VALUE_POINTER);
    }
    //auto p = (GluaTableMapP)lua_malloc(L, sizeof(GluaTableMap));
    auto p = new GluaTableMap();
    if (nullptr == p)
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "out of memory");
        TiValue::lua::lib::notify_lua_state_stop(L);
        return nullptr;
    }
    // new(p)GluaTableMap();
...
struct GluaStorageValue lua_type_to_storage_value_type_with_nested(lua_State *L, int index, size_t len, std::list<const void *> &jsons, size_t recur_depth)
{
    ...
    if (index > lua_gettop(L))
    {
        storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_not_support;
        storage_value.value.int_value = 0;
        return storage_value;
    }
	if(recur_depth>LUA_MAP_TRAVERSER_MAX_DEPTH)
	{
		storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_null;
		storage_value.value.int_value = 0;
		return storage_value;
	}
    ...
    case LUA_TNIL:
        ...
        return storage_value;
    case LUA_TBOOLEAN:
        storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_bool;
        storage_value.value.bool_value = BOOL_VAL(lua_toboolean(L, index));
        return storage_value;
    case LUA_TNUMBER:
        if (lua_isinteger(L, index))
        {
            storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_int;
            storage_value.value.int_value = (lua_Integer)lua_tointeger(L, index);
            return storage_value;
        }
        storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_number;
        storage_value.value.number_value = lua_tonumber(L, index);
        return storage_value;
    case LUA_TSTRING:
        storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_string;
    ...
    case LUA_TTABLE:
		...
		// FIXME: 根据子项类型修改
        storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_unknown_table;
        if (len > 0)
            storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_unknown_array;
		...
		// 如果是Stream，转成stream类型
	{
		...
		{
			storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_stream;
			storage_value.value.userdata_value = addr;
		}
		else
		{
			storage_value.type = TiValue::blockchain::StorageValueTypes::storage_value_userdata;
...
bool lua_table_to_map_traverser_with_nested(lua_State *L, void *ud, size_t len, std::list<const void*> &jsons, size_t recur_depth)
{
    ...
    if (lua_gettop(L) < 2)
        return false;
    if (!lua_isstring(L, -2) && !lua_isinteger(L, -1)) // now only support integer and string as table key, when using by TiValue
        return false;
    std::string key;
...
static void luatablemap_to_json_stream(GluaTableMapP map, std::stringstream &ss, bool is_array=false)
{
    ...
    for (auto it = map->begin(); it != map->end(); ++it)
    {
        ...
        switch (value.type)
        {
		case TiValue::blockchain::StorageValueTypes::storage_value_null:
            ss << "null";
            break;
		case TiValue::blockchain::StorageValueTypes::storage_value_bool:
            ss << (value.value.bool_value ? "true" : "false");
            break;
		case TiValue::blockchain::StorageValueTypes::storage_value_int:
            ss << value.value.int_value;
            break;
		case TiValue::blockchain::StorageValueTypes::storage_value_number:
            ss << value.value.number_value;
            break;
		case TiValue::blockchain::StorageValueTypes::storage_value_string:
        {
            auto str=std::string(value.value.string_value);
            ss << "\"" << glua::util::escape_string(str) << "\"";
            break;
        }
		case TiValue::blockchain::StorageValueTypes::storage_value_userdata:
            ss << "\"userdata\"";
		default: 
		{
