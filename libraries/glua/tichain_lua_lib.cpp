...
#define LUA_MAYBE_CHANGE_STORAGE_CONTRACT_IDS_STATE_KEY "maybe_change_storage_contract_ids_state"
			...
			static int transfer_from_contract_to_public_account(lua_State *L)
      {
				...
				{
					TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "contract transfer must be called in contract api");
					return 0;
				}
				...
				if (amount_str <= 0)
				{
					TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "amount must be positive");
					return 0;
				}
      ...
      static int allow_declare_piece_saved(lua_State *L) {
        return TiValue::lua::api::global_glua_chain_api->allow_declare_piece_saved_wrapper_func(L);
            ...
            static int transfer_from_contract_to_address(lua_State *L)
            {
                if (lua_gettop(L) < 3)
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "transfer_from_contract_to_address need 3 arguments");
                    return 0;
                }
                const char *contract_id = get_contract_id_in_api(L);
                if (!contract_id)
                ...
                lua_Integer transfer_result = TiValue::lua::api::global_glua_chain_api->transfer_from_contract_to_address(L, contract_id, to_address, asset_type, amount_str);
                lua_pushinteger(L, transfer_result);
                return 1;
            }
            ...
            static int get_contract_address_lua_api(lua_State *L)
            {
                ...
                if (!cur_contract_id)
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "can't get current contract address");
                    return 0;
                }
            ...
            static int get_contract_balance_amount(lua_State *L)
            {
                ...
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "get balance amount need asset symbol");
                    return 0;
                }

                auto assert_symbol = luaL_checkstring(L, 2);
                
                auto result = TiValue::lua::api::global_glua_chain_api->get_contract_balance_amount(L, contract_address, assert_symbol);
            ...
            static int panic_message(lua_State *L)
            {
                ...
                if (nullptr != msg)
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, msg);
                return 0;
            }

            static int get_chain_now(lua_State *L)
            {
                auto time = TiValue::lua::api::global_glua_chain_api->get_chain_now(L);
            ...
            {
                auto rand = TiValue::lua::api::global_glua_chain_api->get_chain_random(L);
                lua_pushinteger(L, rand);
                return 1;
            }
            static int get_transaction_id(lua_State *L)
            {
                std::string tid = TiValue::lua::api::global_glua_chain_api->get_transaction_id(L);
                lua_pushstring(L, tid.c_str());
                return 1;
            }
            static int get_transaction_fee(lua_State *L)
            {
                int64_t res = TiValue::lua::api::global_glua_chain_api->get_transaction_fee(L);
                lua_pushinteger(L, res);
                return 1;
            }
            static int get_header_block_num(lua_State *L)
            {
                auto result = TiValue::lua::api::global_glua_chain_api->get_header_block_num(L);
                lua_pushinteger(L, result);
                return 1;
            }
            ...
            static int wait_for_future_random(lua_State *L)
            {
                ...
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "wait_for_future_random need a integer param");
                    return 0;
                }
                auto next = luaL_checkinteger(L, 1);
                if (next <= 0)
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "wait_for_future_random first param must be positive number");
                    return 0;
                }
                auto result = TiValue::lua::api::global_glua_chain_api->wait_for_future_random(L, (int)next);
                lua_pushinteger(L, result);
                return 1;
            }
            ...
            static int get_waited_block_random(lua_State *L)
            {
                if (lua_gettop(L) < 1 || !lua_isinteger(L, 1))
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "get_waited need a integer param");
                    return 0;
                }
                auto num = luaL_checkinteger(L, 1);
                auto result = TiValue::lua::api::global_glua_chain_api->get_waited(L, (uint32_t)num);
                lua_pushinteger(L, result);
                return 1;
            }

            static int emit_tichain_event(lua_State *L)
            {
                if (lua_gettop(L) < 2 && (!lua_isstring(L, 1) || !lua_isstring(L, 2)))
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "emit need 2 string params");
                    return 0;
                }
                const char *contract_id = get_contract_id_in_api(L);
				...
				if (!contract_id || strlen(contract_id) < 1)
					return 0;
                TiValue::lua::api::global_glua_chain_api->emit(L, contract_id, event_name, event_param);
                return 0;
            }
			...
			static int glua_core_lib_Stream_size(lua_State *L)
            {
				...
				if(TiValue::lua::api::global_glua_chain_api->is_object_in_pool(L, (intptr_t) stream,
			...
			static int glua_core_lib_Stream(lua_State *L)
            {
				...
				TiValue::lua::api::global_glua_chain_api->register_object_in_pool(L, (intptr_t) stream, GluaOutsideObjectTypes::OUTSIDE_STREAM_STORAGE_TYPE);
			...
			*/
			static int glua_core_lib_contract_metatable_newindex(lua_State *L)
			{
				...
				if(key == "id" || key == "name" || key == "storage")
				{
					...
					TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "attempt to update a read-only table!");
			...
			//    TiValue.error('only string can be storage key')
			//	  return nil
			//	  end
			//	  return TiValue.get_storage(s.contract, key)
			//	  end
			static int glua_core_lib_storage_metatable_index(lua_State *L)
            {
				if(!lua_isstring(L, 2))
				{
					TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "only string can be storage key");
				...
				auto ret_count = glua::lib::tichainlib_get_storage_impl(L, contract_id, key); // top=ret_count + 4
				if(ret_count>0)
				{
					auto tmp_global_key = "_glua_core_lib_tichain_get_storage_tmp_value";
			...
			//	return nil
			//	end
			//	return TiValue.set_storage(s.contract, key, val)
			//	end
			static int glua_core_lib_storage_metatable_new_index(lua_State *L)
            {
                ...
                return glua::lib::tichainlib_set_storage_impl(L, contract_id, key, 3);

                /*
				int old_top = lua_gettop(L); // 3
				lua_getfield(L, 1, "contract"); // top=4
				lua_pushcfunction(L, glua::lib::tichainlib_set_storage); // storage, key, val, contract, set_storage
    ...
    return function()  
        ...
        return hashes[i-int_key_size], t[hashes[i-int_key_size]] 
			...
			*/
            ...
            {
                if (!TiValue::lua::api::global_glua_chain_api->has_exception(L))
                {
            ...
            void close_lua_state(lua_State *L)
            {
				...
				TiValue::lua::api::global_glua_chain_api->release_objects_in_pool(L);
                LStatesMap *states_map = get_lua_states_value_hashmap();
            ...
            */
            ...
            void set_lua_state_value(lua_State *L, const char *key, GluaStateValue value, enum GluaStateValueType type)
            {
                ...
                if (node_v.type == LUA_STATE_VALUE_STRING)
                    node_v.value.string_value = TiValue::lua::lib::malloc_and_copy_string(L, value.string_value);
                std::string key_str(key);
                map->erase(key_str);
...
#define MYK(x)		(-1-(x))
	...
	storage: S
				...
				bool throw_exception, bool in_repl, bool is_contract)
            {
				...
				if (use_type_check)
				{
					...
					if(is_contract)
					{
						...
						{
							if(!TiValue::lua::api::global_glua_chain_api->check_contract_exist(L, p.first.c_str()))
							{
					...
					if (!check_syntax_has_errors || type_checker.has_error() || has_contract_import_error)
					{
						for (const auto & item : type_checker.errors())
							ss << item.second << "\n";
						if(throw_exception)
							TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_COMPILE_ERROR, ss.str().c_str());
					...
					if(stream)
					{
						...
						if(is_contract && contract_storage_type)
						{
							...
							{
								if (throw_exception)
									TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_COMPILE_ERROR, e.what());
								if (error && throw_exception)
								{
				...
				{
					TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_PARSER_ERROR, e.what());
					if (changed)
						*changed = false;
					return origin_code;
				}
					...
					if(is_importing_contract)
					{
						...
						if(getOpMode(o) == OP_LOADK)
						{
							...
							if (idx_in_kst >= 0 && idx_in_kst < proto->sizek)
							{
								...
								if (contract_name && !TiValue::lua::api::global_glua_chain_api->check_contract_exist(L, contract_name))
								{
					...
					else if(is_importing_contract_address)
					{
						...
						if (getOpMode(o) == OP_LOADK)
						{
							...
							if (idx_in_kst >= 0 && idx_in_kst < proto->sizek)
							{
								...
								if (contract_address && !TiValue::lua::api::global_glua_chain_api->check_contract_exist_by_address(L, contract_address))
								{
                ...
                char *error, GluaStatePreProcessorFunction *preprocessor, bool use_type_check)
            {
                ...
                if (TiValue::lua::api::global_glua_chain_api->has_exception(L))
                    TiValue::lua::api::global_glua_chain_api->clear_exceptions(L);
                ...
                if (!compilefile_to_stream(L, filename, stream, error, use_type_check, true))
                {
                    if (error && strlen(error) > 0)
                    {
                        ...
                        TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_COMPILE_ERROR, error);
                    }
                    else
                        TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_COMPILE_ERROR, "compile error");
                    return false;
                }
                if (TiValue::lua::api::global_glua_chain_api->has_exception(L))
                    TiValue::lua::api::global_glua_chain_api->clear_exceptions(L);
                ...
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_COMPILE_ERROR, "compile error when check contract bytecode");
                    return false;
                }
                ...
                if (!lua_istable(L, -1))
                {
                    ...
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, error_str);
                    return false;
                }
                else
                {
			...
			std::stack<std::string> *get_using_contract_id_stack(lua_State *L, bool init_if_not_exist)
            {
				...
				auto contract_id_stack_value_in_state_map = TiValue::lua::lib::get_lua_state_value(L, GLUA_CONTRACT_API_CALL_STACK_STATE_MAP_KEY);
				if (!contract_id_stack_value_in_state_map.pointer_value)
				{
					...
					TiValue::lua::lib::set_lua_state_value(L, GLUA_CONTRACT_API_CALL_STACK_STATE_MAP_KEY, contract_id_stack_value_in_state_map, GluaStateValueType::LUA_STATE_VALUE_POINTER);
				}
				else
				...
				const char *api_name, const char *arg1, std::string *result_json_string)
            {
                ...
                TiValue::lua::api::global_glua_chain_api->get_contract_address_by_name(L, contract_name, contract_address, &address_size);
                if (address_size > 0)
                {
                    GluaStateValue value;
            ...
            {
				const auto &state_node = get_lua_state_value_node(L, tichain_CONTRACT_INITING);
            ...
            {
				auto starting_contract_address_node = TiValue::lua::lib::get_lua_state_value_node(L, STARTING_CONTRACT_ADDRESS);
            ...
            bool execute_contract_init_by_address(lua_State *L, const char *contract_address, const char *arg1, std::string *result_json_string)
            {
                ...
                set_lua_state_value(L, tichain_CONTRACT_INITING, state_value, LUA_STATE_VALUE_INT);
            ...
            */
            const char *get_contract_id_in_api(lua_State *L)
            {
                ...
                {
                    TiValue::lua::api::global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "the api must have first local self, or need be defined as <M>:<api_name>");
                }
                lua_pop(L, 1);
                return nullptr;
				*/
