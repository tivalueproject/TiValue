...
static int luaB_print(lua_State *L) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        if (i > 1) luaL_writestring(L, "\t", 1);
        luaL_writestring(L, s, l);
 ...
 * pretty print
 */
static int luaB_pprint(lua_State *L)
{
    int n = lua_gettop(L);  /* number of arguments */
    int i;
    lua_getglobal(L, "tojsonstring");
    for (i = 1; i <= n; i++) {
        const char *s;
        size_t l;
        lua_pushvalue(L, -1);  /* function to be called */
        ...
        s = lua_tolstring(L, -1, &l);  /* get result */
        if (s == nullptr)
            return luaL_error(L, "'tojsonstring' must return a string to 'pprint'");
        if (i > 1) luaL_writestring(L, "\t", 1);
        luaL_writestring(L, s, l);
        lua_pop(L, 1);  /* pop result */
    }
...
static int luaB_todouble(lua_State *L)
{
...
{
    int res = luaB_tonumber(L);
    if (res > 0 && lua_isinteger(L, -1))
    {
        lua_Integer int_result = luaL_checkinteger(L, -1);
        lua_pop(L, 1);
	...
	{
		lua_pop(L, res);
		lua_set_run_error(L, "need pass number or integer value to func tointeger");
		return luaL_error(L, "need pass number or integer value to func tointeger");
		// return res;
	}
}
...
static int luaB_error(lua_State *L) {
	...
	global_glua_chain_api->throw_exception(L, tichain_API_THROW_ERROR, luaL_checkstring(L, 1));
    return lua_error(L);
}
...
static int luaB_tojsonstring(lua_State *L) {
    luaL_checkany(L, 1);
    luaL_tojsonstring(L, 1, nullptr);
    return 1;
}

static int luaB_totable(lua_State *L) {
    luaL_checkany(L, 1);
    if (lua_istable(L, 1))
    {
        lua_pushvalue(L, 1);
...
static const luaL_Reg base_funcs[] = {
    ...
    { "todouble", luaB_todouble },
    { "tojsonstring", luaB_tojsonstring },
