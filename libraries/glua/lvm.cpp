...
// FIXME: duplicate code in tichain_lua_lib.cpp
...
// FIXME: end duplicate code in tichain_lua_lib.cpp
...
if (!(cond)) {      \
  ...
  global_glua_chain_api->throw_exception(L, tichain_API_LVM_ERROR, (error_msg));       \
  vmbreak;                                   \
     }                             \
}
...
newframe:  /* reentry point when frame changes (call/return) */
    ...
    int insts_limit = TiValue::lua::lib::get_lua_state_value(L, INSTRUCTIONS_LIMIT_LUA_STATE_MAP_KEY).int_value;
    ...
    if (nullptr == stopped_pointer)
    {
        ...
        TiValue::lua::lib::resume_lua_state_running(L);
        stopped_pointer = TiValue::lua::lib::get_lua_state_value(L, LUA_STATE_STOP_TO_RUN_IN_LVM_STATE_MAP_KEY).int_pointer_value;
    }
    int has_insts_limit = insts_limit > 0 ? 1 : 0;
    int *insts_executed_count = TiValue::lua::lib::get_lua_state_value(L, INSTRUCTIONS_EXECUTED_COUNT_LUA_STATE_MAP_KEY).int_pointer_value;
    if (nullptr == insts_executed_count)
    {
        ...
        *insts_executed_count = 0;
        ...
        TiValue::lua::lib::set_lua_state_value(L, INSTRUCTIONS_EXECUTED_COUNT_LUA_STATE_MAP_KEY, lua_state_value_of_exected_count, LUA_STATE_VALUE_INT_POINTER);
    }
    ...
    for (;;) {
		...
		if(L->bytecode_debugger_opened)
		{
			...
			TiValue::lua::core::LuaDebugFileInfo *ldf = nullptr;
			if (proto_source_ldf_filename[0] == '@')
			{
				...
				if (ldf_file)
				{
					auto ldf_deserialized = TiValue::lua::core::LuaDebugFileInfo::deserialize_from_file(ldf_file);
					ldf = new TiValue::lua::core::LuaDebugFileInfo();
        ...
        if (!ci || ci->u.l.savedpc == nullptr) {
          global_glua_chain_api->throw_exception(L, tichain_API_LVM_LIMIT_OVER_ERROR, "wrong bytecode instruction, can't find savedpc");
          vmbreak;
        }
		...
		*insts_executed_count += 1; // executed instructions count
        ...
        {
            global_glua_chain_api->throw_exception(L, tichain_API_LVM_LIMIT_OVER_ERROR, "over instructions limit");
            vmbreak;
        }
        ...
        vmdispatch(GET_OPCODE(i)) {
            ...
            vmcase(OP_GETTABLE) {
                ...
                if (!istable)
                {
                    global_glua_chain_api->throw_exception(L, tichain_API_LVM_ERROR, "getfield of nil, need table here");
                    L->force_stopping = true;
                    vmbreak;
                }
            ...
            vmcase(OP_SETTABUP) {
				...
				{
					global_glua_chain_api->throw_exception(L, tichain_API_LVM_ERROR, "set upvalue of nil, need table here");
					L->force_stopping = true;
					vmbreak;
				}
                TValue *upval = cl->upvals[upval_index]->v;
            ...
            vmcase(OP_ADD) {
                ...
                if (ttisinteger(rb) && ttisinteger(rc)) {
                    ...
                    global_glua_chain_api->throw_exception(L, tichain_API_LVM_ERROR, "+ can only accept numbers");
            ...
            vmcase(OP_BAND) {
                ...
                if (tointeger(rb, &ib) && tointeger(rc, &ic)) {
                    ...
                    global_glua_chain_api->throw_exception(L, tichain_API_LVM_ERROR, "& can only accept integer");
            ...
            vmcase(OP_UNM) {
                ...
                if (ttisinteger(rb)) {
                    ...
                    global_glua_chain_api->throw_exception(L, tichain_API_LVM_ERROR, "-<exp> can only accept numbers");
            ...
            vmcase(OP_JMP) {
                ...
                // tichain_api_lua_throw_exception(L, tichain_API_LVM_ERROR, "TiValue lua not support goto symbol");
                vmbreak;
            }
            vmcase(OP_EQ) {
                TValue *rb = RKB(i);
