...
#define ARGS_ERROR()   { global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "arguments wrong"); return 0; }
...
static int str_split(lua_State *L)
{
    ...
    while (i < sstr.length())
    {
        std::string sub = sstr.substr(i, sepstr.length());
        if (sub == sepstr && last_pos == i)
        {
            i += sepstr.length();
            last_pos = i;
            continue;
        ...
        if (sub == sepstr)
        {
            if (i > last_pos)
            {
                std::string item = sstr.substr(last_pos, i - last_pos);
                i += sepstr.length();
                last_pos = i;
...
static int gmatch_aux(lua_State *L) {
    ...
    for (src = gm->src; src <= gm->ms.src_end; src++) {
        const char *e;
        reprepstate(&gm->ms);
        if ((e = match(&gm->ms, src, gm->p)) != nullptr) {
