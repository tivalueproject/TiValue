...
static GluaStorageValue tokens_to_lua_value(lua_State *L, GluaTokenParser *token_parser, bool *result)
{
    ...
    if (token_parser->current_position() == token_parser->size() - 1 || (cur_token.type != '{' && cur_token.type != '['))
    {
        ...
        case TOKEN_RESERVED::LTK_INT:
        {
            ...
            ss >> token_int;
            GluaStorageValue value;
            value.type = TiValue::blockchain::StorageValueTypes::storage_value_int;
        ...
        case TOKEN_RESERVED::LTK_FLT:
        {
            ...
            ss >> token_num;
            GluaStorageValue value;
            value.type = TiValue::blockchain::StorageValueTypes::storage_value_number;
        ...
        case TOKEN_RESERVED::LTK_NAME:
        {
            ...
            {
                GluaStorageValue value;
                value.type = TiValue::blockchain::StorageValueTypes::storage_value_bool;
            ...
            {
                global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "parse json error(unknown symbol name %s)", token_str.c_str());
                if (nullptr != result)
                    *result = false;
                return nil_storage_value();
            }
        ...
        default:
            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "parse json error(unknown token %s)", token.token.c_str());
            if (nullptr != result)
                *result = false;
            return nil_storage_value();
        }
    ...
    case '{':
    {
        ...
        if (token_parser->current().type == '}')
        {
            token_parser->next();
            GluaStorageValue value;
            value.type = TiValue::blockchain::StorageValueTypes::storage_value_unknown_table;
            value.value.table_value = TiValue::lua::lib::create_managed_lua_table_map(L);
        ...
        table_value.type = TiValue::blockchain::StorageValueTypes::storage_value_unknown_table; // FIXME: 根据子项类型修改
        table_value.value.table_value = TiValue::lua::lib::create_managed_lua_table_map(L);
    ...
    case '[':
    {
        ...
        if (token_parser->current().type == ']')
        {
            token_parser->next();
            GluaStorageValue value;
            value.type = TiValue::blockchain::StorageValueTypes::storage_value_unknown_array;
            value.value.table_value = TiValue::lua::lib::create_managed_lua_table_map(L);
            assert(nullptr != value.value.table_value);
            if (nullptr != result)
                *result = true;
        ...
        GluaStorageValue table_value;
        table_value.type = TiValue::blockchain::StorageValueTypes::storage_value_unknown_array; // FIXME: 根据子项类型修改
        table_value.value.table_value = TiValue::lua::lib::create_managed_lua_table_map(L);
        assert(nullptr != table_value.value.table_value);
        size_t count = 0;
        do
