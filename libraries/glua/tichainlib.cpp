...
static int unit_test_check_equal_number(lua_State *L)
{
    ...
    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "check equal error happen");
    return 0;
}
...
static int unit_test_check(lua_State *L)
{
    ...
    global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "check condition error happen");
    return 0;
}

static int stop_vm(lua_State *L)
{
    TiValue::lua::lib::notify_lua_state_stop(L);
    return 0;
}
...
static int throw_error(lua_State *L)
{
    ...
    {
        global_glua_chain_api->throw_exception(L, tichain_API_THROW_ERROR, "empty error");
        // TiValue::lua::lib::notify_lua_state_stop(L);
        return 0;
    }
    const char *msg = luaL_checkstring(L, -1);
    global_glua_chain_api->throw_exception(L, tichain_API_THROW_ERROR, msg);
...
static int register_storage(lua_State *L)
{
    if (!lua_istable(L, 1))
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "first argument of register_storage must be contract");
        return 0;
    }
    ...
    if (TiValue::lua::lib::check_in_lua_sandbox(L))
    {
...
{
    GluaStateValueNode state_value_node = TiValue::lua::lib::get_lua_state_value_node(L, LUA_STORAGE_READ_TABLES_KEY);
    ...
    if (state_value_node.type != LUA_STATE_VALUE_POINTER || nullptr == state_value_node.value.pointer_value)
    {
        ...
        TiValue::lua::lib::set_lua_state_value(L, LUA_STORAGE_READ_TABLES_KEY, value_to_store, LUA_STATE_VALUE_POINTER);
    }
    else
    {
  ...
  GluaStorageChangeList *list, const std::string &key)
{
    ...
    nil_value.type = TiValue::blockchain::StorageValueTypes::storage_value_null;
    ...
    if (!list || list->size() < 1)
    {
        ...
        if (!list) {
          ...
          GluaStateValue value_to_store;
          value_to_store.pointer_value = list;
          TiValue::lua::lib::set_lua_state_value(L, LUA_STORAGE_CHANGELIST_KEY, value_to_store, LUA_STATE_VALUE_POINTER);
        }
        GluaStorageChangeItem change_item;
        change_item.before = value;
...
static bool lua_push_storage_table_value(lua_State *L, GluaTableMap *map, int type)
{
    ...
    if (lua_storage_is_array((TiValue::blockchain::StorageValueTypes) type))
    {
...
bool lua_push_storage_value(lua_State *L, const GluaStorageValue &value)
{
    ...
    switch (value.type)
    {
	case TiValue::blockchain::StorageValueTypes::storage_value_int: lua_pushinteger(L, value.value.int_value); break;
	case TiValue::blockchain::StorageValueTypes::storage_value_bool: lua_pushboolean(L, value.value.bool_value); break;
	case TiValue::blockchain::StorageValueTypes::storage_value_number: lua_pushnumber(L, value.value.number_value); break;
	case TiValue::blockchain::StorageValueTypes::storage_value_null: lua_pushnil(L); break;
	case TiValue::blockchain::StorageValueTypes::storage_value_string: lua_pushstring(L, value.value.string_value); break;
	case TiValue::blockchain::StorageValueTypes::storage_value_stream: {
		auto stream = (TiValue::lua::lib::GluaByteStream*) value.value.userdata_value;
		lua_pushlightuserdata(L, (void*) stream);
		luaL_getmetatable(L, "GluaByteStream_metatable");
		lua_setmetatable(L, -2);
	} break;
	default: {
		if(TiValue::blockchain::is_any_table_storage_value_type(value.type)
			|| TiValue::blockchain::is_any_array_storage_value_type(value.type))
		{
...
static GluaStorageChangeItem diff_storage_change_if_is_table(lua_State *L, GluaStorageChangeItem change_item)
{
    ...
    for (auto it1 = change_item.before.value.table_value->begin(); it1 != change_item.before.value.table_value->end(); ++it1)
    {
        ...
        if (it1->second.type == TiValue::blockchain::StorageValueTypes::storage_value_null)
            continue;
        new_before->insert(new_before->end(), std::make_pair(it1->first, it1->second));
...
bool luaL_commit_storage_changes(lua_State *L)
{
  ...
  if (maybe_storage_changed_contract_ids && maybe_storage_changed_contract_ids->size()>0)
  {
    ...
    GluaStateValueNode storage_changelist_node = TiValue::lua::lib::get_lua_state_value_node(L, LUA_STORAGE_CHANGELIST_KEY);
      ...
      && changes.size()==0)
    {
      ...
      {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "some storage of this contract not init");
        return false;
      }
    }
    ...
    for (auto it = changes.begin(); it != changes.end(); ++it)
    {
		...
		if (TiValue::lua::lib::is_calling_contract_init_api(L))
		{
			...
			if (it->first == starting_contract_address)
			{
				...
				{
					//if (p1.second.before.type == TiValue::blockchain::StorageValueTypes::storage_value_null)
					//	continue;
					if(storage_properties_in_chain.find(p1.first) == storage_properties_in_chain.end())
					{
						global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "Can't find storage %s", p1.first.c_str());
						return false;
					}
						...
						|| TiValue::blockchain::is_any_array_storage_value_type(p1.second.after.type))
					{
						...
						{
							global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "storage %s type not matched in chain", p1.first.c_str());
							return false;
						}
						if(p1.second.after.value.table_value->size()>0)
						{
							...
							{
								item_in_table.second.try_parse_type(TiValue::blockchain::get_storage_base_type(storage_info_in_chain));
							}
							auto item_after = p1.second.after.value.table_value->begin()->second;
							if(item_after.type != TiValue::blockchain::get_item_type_in_table_or_array(storage_info_in_chain))
							{
								global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "storage %s type not matched in chain", p1.first.c_str());
								return false;
							}
					...
					} else if(p1.second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_number
						&& storage_info_in_chain==TiValue::blockchain::StorageValueTypes::storage_value_int)
					{
						p1.second.after.type = TiValue::blockchain::StorageValueTypes::storage_value_int;
						p1.second.after.value.int_value = (lua_Integer)p1.second.after.value.number_value;
					}
					else if (p1.second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_int
						&& storage_info_in_chain == TiValue::blockchain::StorageValueTypes::storage_value_number)
					{
						p1.second.after.type = TiValue::blockchain::StorageValueTypes::storage_value_number;
        ...
        while (it2 != it->second->end())
        {
			...
			if(!is_in_starting_contract_init)
			{
				...
				if (it2->second.before.type != TiValue::blockchain::StorageValueTypes::storage_value_null
					&& storage_properties_in_chain.find(it2->first) != storage_properties_in_chain.end())
				{
						...
						|| TiValue::blockchain::is_any_array_storage_value_type(it2->second.after.type))
					{
						...
						if (!TiValue::blockchain::is_any_table_storage_value_type(storage_info_in_chain)
							&& !TiValue::blockchain::is_any_array_storage_value_type(storage_info_in_chain))
						{
							global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "storage %s type not matched in chain", it2->first.c_str());
							return false;
						}
					...
					else if (it2->second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_number
						&& storage_info_in_chain == TiValue::blockchain::StorageValueTypes::storage_value_int)
					{
						it2->second.after.type = TiValue::blockchain::StorageValueTypes::storage_value_int;
						it2->second.after.value.int_value = (lua_Integer)it2->second.after.value.number_value;
					}
					else if (it2->second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_int
						&& storage_info_in_chain == TiValue::blockchain::StorageValueTypes::storage_value_number)
					{
						it2->second.after.type = TiValue::blockchain::StorageValueTypes::storage_value_number;
			...
			if(lua_storage_is_table(it2->second.after.type))
			{
				...
				for(const auto &p : *it2->second.after.value.table_value)
				{
					...
					{
						global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR,
            ...
            {
                if (it2->second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_int
					&& it2->second.after.value.int_value == it2->second.before.value.int_value)
                {
                    it2 = it->second->erase(it2);
                    continue;
                }
                else if (it2->second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_bool
                ...
                else if (it2->second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_string
					&& strcmp(it2->second.after.value.string_value, it2->second.before.value.string_value) == 0)
                {
                    it2 = it->second->erase(it2);
                    continue;
                }
				else if (it2->second.after.type == TiValue::blockchain::StorageValueTypes::storage_value_stream
					&& ((TiValue::lua::lib::GluaByteStream*) it2->second.after.value.userdata_value)->equals((TiValue::lua::lib::GluaByteStream*) it2->second.before.value.userdata_value))
				{
					it2 = it->second->erase(it2);
					continue;
				}
                else if (!lua_storage_is_table(it2->second.after.type)
					&& it2->second.after.type != TiValue::blockchain::StorageValueTypes::storage_value_string
    ...
    auto result = global_glua_chain_api->commit_storage_changes_to_tichain(L, changes);
    if (storage_changelist_node.type == LUA_STATE_VALUE_POINTER && nullptr != storage_changelist_node.value.pointer_value)
    {
        GluaStorageChangeList *list = (GluaStorageChangeList*)storage_changelist_node.value.pointer_value;
        list->clear();
...
static const char *get_contract_id_in_storage_operation(lua_State *L)
{
	...
	auto contract_id_str = TiValue::lua::lib::malloc_and_copy_string(L, contract_id.c_str());
	return contract_id_str;
}

static std::string get_contract_id_string_in_storage_operation(lua_State *L)
{
  return TiValue::lua::lib::get_current_using_contract_id(L);
}
...
namespace glua {
	namespace lib {
			...
			const char *contract_id, const char *name)
		{
			...
			{
				global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "contract can only access its own storage directly");
				TiValue::lua::lib::notify_lua_state_stop(L);
				L->force_stopping = true;
				return 0;
			}
			...
			const auto &state_value_node = TiValue::lua::lib::get_lua_state_value_node(L, LUA_STORAGE_CHANGELIST_KEY);
			int result;
			...
			else if (TiValue::lua::lib::check_in_lua_sandbox(L))
			{
		...
		*/
		int tichainlib_get_storage(lua_State *L)
		{
			...
			if (lua_gettop(L) < 2)
			{
				global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "get_storage need 2 arguments");
				TiValue::lua::lib::notify_lua_state_stop(L);
				return 0;
			}
			if (!lua_istable(L, 1))
			{
				global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "first argument of get_storage must be contract");
				return 0;
			}
			lua_getfield(L, 1, "id");
			...
			{
				global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "second argument of set_storage must be string or integer");
				TiValue::lua::lib::notify_lua_state_stop(L);
				L->force_stopping = true;
				return 0;
			}
			const char *name = luaL_checkstring(L, 2);
            return tichainlib_get_storage_impl(L, contract_id, name);
		}
          ...
          const char *contract_id, const char *name, int value_index)
        {
          ...
          {
            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "second argument of set_storage must be name");
            return 0;
          }
          ...
          {
          global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "third argument of set_storage must be value");
          return 0;
          }
          */

          // log the value before and the new value
          GluaStateValueNode state_value_node = TiValue::lua::lib::get_lua_state_value_node(L, LUA_STORAGE_CHANGELIST_KEY);
          ...
          auto after = arg2;
          if (after.type == TiValue::blockchain::StorageValueTypes::storage_value_null)
          {
            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, (name_str + "storage can't change to nil").c_str());
            TiValue::lua::lib::notify_lua_state_stop(L);
            return 0;
          }
          if (before.type != TiValue::blockchain::StorageValueTypes::storage_value_null
            && (before.type != after.type && !lua_storage_is_table(before.type)))
          {
            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, (std::string(name) + "storage can't change type").c_str());
            TiValue::lua::lib::notify_lua_state_stop(L);
            return 0;
          }
          ...
          if (!TiValue::lua::lib::is_calling_contract_init_api(L)
            && before.type == TiValue::blockchain::StorageValueTypes::storage_value_null)
          {
            // when not in init api
            global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, (std::string(name) + "storage can't register storage after inited").c_str());
            TiValue::lua::lib::notify_lua_state_stop(L);
            return 0;
          }
          ...
          if (lua_storage_is_table(after.type))
          {
            ...
            if (nullptr != inner_table)
            {
              for (auto it = inner_table->begin(); it != inner_table->end(); ++it)
              {
                ...
                {
                  global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "storage not support nested map");
                  TiValue::lua::lib::notify_lua_state_stop(L);
                  return 0;
                }
              }
          ...
          if (lua_storage_is_table(after.type))
          {
            ...
            if (lua_storage_is_table(before.type))
            {
              for (auto it = before.value.table_value->begin(); it != before.value.table_value->end(); ++it)
              {
                ...
                if (it->second.type != TiValue::blockchain::StorageValueTypes::storage_value_null)
                {
                  ...
                  else
                  {
                    ...
                    {
                      global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "storage table type must be same");
                      TiValue::lua::lib::notify_lua_state_stop(L);
                      return 0;
                    }
                  }
              ...
              for (auto it = list->begin(); it != list->end(); ++it)
              {
                if (it->contract_id == std::string(contract_id) && it->key == std::string(name))
                {
                  if (lua_storage_is_table(it->after.type) && it->after.value.table_value->size() > 0)
                  {
                    ...
                    {
                      if (it2->second.type != TiValue::blockchain::StorageValueTypes::storage_value_null)
                      {
		...
		* when lua_State finish(close successfully without error), commit all changes of all contracts' storages to TiValue
		* when lua_State finished/closed with error, just not commit he changes. so get/set storage api will not call TiValue until commit.
		...
		*/
		int tichainlib_set_storage(lua_State *L)
		{
			...
			if (lua_gettop(L) < 3)
			{
				global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "set_storage need 2 arguments");
				TiValue::lua::lib::notify_lua_state_stop(L);
				return 0;
			}
			if (!lua_istable(L, 1))
			{
				global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "first argument of set_storage must be contract");
				return 0;
			}
			lua_getfield(L, 1, "id");
			...
			char *name = (char*)luaL_checkstring(L, 2);
            return tichainlib_set_storage_impl(L, contract_id, name, 3);
		}
        ...
        namespace lib {
            ...
            GluaStorageValue ltichain_get_storage(lua_State *L, const char *contract_id, const char *name)
            {
                ...
                else
                {
                    ...
                    value.type = TiValue::blockchain::StorageValueTypes::storage_value_null;
                    value.value.int_value = 0;
                    return value;
                }
            }

            bool ltichain_set_storage(lua_State *L, const char *contract_id, const char *name, GluaStorageValue value)
            {
                lua_pushstring(L, contract_id);
                lua_pushstring(L, name);
                lua_push_storage_value(L, value);
                glua::lib::tichainlib_set_storage(L);
...
/**
* rollback storage, and store the init storage in TiValue
* arg1 is contract
*/
static int rollback_storage(lua_State *L)
{
    if (lua_gettop(L) < 1)
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "rollback_storage need 1 arguments");
        TiValue::lua::lib::notify_lua_state_stop(L);
        return 0;
    }
    if (!lua_istable(L, 1))
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "first argument of rollback_storage must be contract");
        return 0;
    }
    lua_getfield(L, 1, "name");
    auto contract_name = (const char*)luaL_checkstring(L, -1);
...
static const luaL_Reg tichainlib[] = {
    { "check", unit_test_check },
    { "check_equal", unit_test_check_equal_number },
    { "stop_vm", stop_vm },
    { "error", throw_error },
    { "set_storage", glua::lib::tichainlib_set_storage },
    { "get_storage", glua::lib::tichainlib_get_storage },
...
/*
** Open TiValue library
*/
LUAMOD_API int luaopen_tichain(lua_State *L) {
    luaL_newlib(L, tichainlib);
