...
static const char *searchpath(lua_State *L, const char *name,
    ...
    const char *sep,
    const char *dirsep) {
    //if (glua::util::starts_with(name, ADDRESS_CONTRACT_PREFIX))
    //	return nullptr;
...
/**
 * module searcher from TiValue api
 */
static int searcher_tichain(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    if (!name)
        return LUA_ERRERR;
    char error[LUA_VM_EXCEPTION_STRNG_MAX_LENGTH];
    memset(error, 0x0, sizeof(error));
    std::string origin_contract_name_str = TiValue::lua::lib::unwrap_contract_name(name);
    ...
    if (!stream)
    {
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, "load contract %s error", origin_contract_name);
        return 1;
    }
    struct StreamScope {
        lua_State *L;
        ...
        ~StreamScope() {
            ...
            {
                // TiValue::lua::api::free_contract(L, stream);
            }
        }
    } stream_scope(L, name, stream.get());
    LClosure *closure = TiValue::lua::lib::luaU_undump_from_stream(L, stream.get(), TiValue::lua::lib::unwrap_any_contract_name(origin_contract_name).c_str());

    if (!TiValue::lua::lib::check_contract_proto(L, closure->p, error))
    {
        if (strlen(L->compile_error) < 1)
        {
            memcpy(L->compile_error, error, sizeof(char)*(strlen(error) + 1));
        }
        global_glua_chain_api->throw_exception(L, tichain_API_SIMPLE_ERROR, error ? error : "contract bytecode stream error");
        return 1;
    }
...
static void createsearcherstable(lua_State *L) {
    ...
    { searcher_preload, searcher_tichain, searcher_C, searcher_Croot, nullptr }; // searcher_Lua 在searcher_preload后

    static const lua_CFunction tichain_searchers[] =
    { searcher_preload, searcher_tichain };
    ...
    /*
    lua_createtable(L, sizeof(tichain_searchers) / sizeof(tichain_searchers[0]) - 1, 0);

    for (i = 0; tichain_searchers[i] != nullptr; i++) {
    lua_pushvalue(L, -2);
    lua_pushcclosure(L, tichain_searchers[i], 1);
    lua_rawseti(L, -2, i + 1);
    }
    */
    // lua_setfield(L, -4, "tichain_searchers");
}
