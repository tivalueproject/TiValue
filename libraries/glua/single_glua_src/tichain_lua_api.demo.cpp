	...
	namespace lua {
		namespace api {
			...
			*/
			...
			std::shared_ptr<GluaModuleByteStream> DemoGluaChainApi::get_bytestream_from_code(lua_State *L, const TiValue::blockchain::Code& code)
			{
				return nullptr;
			}
			...
			{
				std::string filename = std::string("tichain_lua_modules") + glua::util::file_separator_str() + "tichain_contract_" + name;
			...
			*/
			...
			{
              // fetch storage value from tichain
			...
			GluaStorageValue DemoGluaChainApi::get_storage_value_from_tichain_by_address(lua_State *L, const char *contract_address, std::string name)
			{
				// fetch storage value from tichain
                if (_demo_chain_storage_buffer.find(L) == _demo_chain_storage_buffer.end()) {
                  if (_demo_chain_storage_buffer.size() > 5)
			...
			bool DemoGluaChainApi::commit_storage_changes_to_tichain(lua_State *L, AllContractsChangesMap &changes)
			{
				// printf("commited storage changes to tichain\n");
                if (_demo_chain_storage_buffer.find(L) == _demo_chain_storage_buffer.end()) {
                  if (_demo_chain_storage_buffer.size() > 5)
                  {
                    _demo_chain_storage_buffer.clear();
			...
			{
				// printf("registered storage %s[%s] to tichain\n", contract_name, name);
				return true;
			}
			...
			bool DemoGluaChainApi::contract_api_check(lua_State *L, const std::string& contract_id, const std::string& api_name)
			{
				return true;
			}
			bool DemoGluaChainApi::allow_upload_request(lua_State *L, const blockchain::FileIdType& file_id, const std::string& requestor,
			...
			int DemoGluaChainApi::allow_upload_request_wrapper_func(lua_State *L)
			{
				return 0;
			}
			int DemoGluaChainApi::allow_piece_saved_wrapper_func(lua_State *L)
			{
				return 0;
			}
			int DemoGluaChainApi::allow_enable_access_wrapper_func(lua_State *L)
			{
				return 0;
			}
			int DemoGluaChainApi::allow_store_reject_wrapper_func(lua_State *L)
			{
				return 0;
			}
			int DemoGluaChainApi::allow_store_request_wrapper_func(lua_State *L)
			{
				return 0;
			}
			bool DemoGluaChainApi::allow_store_request(lua_State *L, const blockchain::FileIdType& file_id, const std::string& piece_id, const std::string& requester, const std::string& node_id)
			{
				return true;
			}
			bool DemoGluaChainApi::allow_piece_saved(lua_State *L, const blockchain::FileIdType& file_id, const  std::string& piece_id, const std::string& Node)
			{
				return true;
			}
			bool DemoGluaChainApi::allow_enable_access(lua_State *L, const std::string& file_id, const std::string& requestor)
			{
				return true;
			}
			bool DemoGluaChainApi::allow_store_reject(lua_State *L, const blockchain::FileIdType& file_id, const std::string& piece_id, const std::string& node_id)
			{
				return true;
			}
			const char* DemoGluaChainApi::get_publickey_address(lua_State *L, const char* pubkey)
			{
				return "dummy";
			}
