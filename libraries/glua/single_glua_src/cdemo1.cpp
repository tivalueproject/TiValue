...
#include <tichain_lua_api.demo.h>
...
{
	// tichain_api_lua_throw_exception(L, 100, "test error happen");
	return 0;
}
...
static bool compile_and_run(lua_State *L, std::string src_filename)
{
	...
	int compile_status = TiValue::lua::lib::compilefile_to_file(src_filename.c_str(), "tmp_to_run", nullptr);
	if (!compile_status)
	{
	...
	bool run_status = TiValue::lua::lib::run_compiledfile(L, src);
...
{	 
	...
	if (use_type_check)
	{
		...
		token_parser->parse(TiValue::lua::lib::get_typed_lua_lib_code() + code2);
	...
	if (!TiValue::lua::lib::compilefile_to_stream(L, src_filename.c_str(), stream.get(), error, use_type_check))
	{
		printf("compile %s error\n", src_filename.c_str());
		if (strlen(error) > 0)
		{
        ...
        // TiValue::lua::lib::free_bytecode_stream(stream);
		return 0;
	}
    global_glua_chain_api->clear_exceptions(L);
	bool run_status = TiValue::lua::lib::run_compiled_bytestream(L, stream.get());
	// TiValue::lua::lib::free_bytecode_stream(stream);
	...
	bool use_type_check = false, std::vector<std::string> *errors = nullptr)
{
	...
	if (!TiValue::lua::lib::compile_contract_to_stream(src_filename.c_str(), stream.get(), error, nullptr, use_type_check))
	{
	...
	, bool run_start = false, bool use_type_check = false, std::vector<std::string> *errors = nullptr)
{
	...
	if (nullptr == stream)
		return false;
	if (!TiValue::lua::lib::compile_contract_to_stream(src_filename.c_str(), stream.get(), nullptr, nullptr, use_type_check))
	{
		printf("compile %s error\n", src_filename.c_str());
		// TiValue::lua::lib::free_bytecode_stream(stream);
		return false;
    ...
    global_glua_chain_api->clear_exceptions(L);
	bool run_status = TiValue::lua::lib::run_compiled_bytestream(L, stream.get());
	TiValue::lua::lib::execute_contract_init(L, "tmp", stream.get(), "abcd", nullptr);
	if (run_start)
	{
		intptr_t pt = (intptr_t)stream.get();
		std::string name = STREAM_CONTRACT_PREFIX + std::to_string(pt);
		TiValue::lua::lib::GluaStateScope scope2;
		scope2.L()->debugger_pausing = L->debugger_pausing;
		std::string result_json_string;
		TiValue::lua::lib::execute_contract_api(L, name.c_str(), "start", "abcd", &result_json_string);
...
GTEST(TEST_MATH_FLOOR)
{
	...
	TiValue::lua::lib::compilefile_to_file(filename.c_str(), "test_upval.out", nullptr, false);
	LClosure *closure = TiValue::lua::lib::luaU_undump_from_file(L, "test_upval.out", "test_upval");
	GCHECK(closure != nullptr);
	FILE *tmp = fopen("test_upval.bytes.out", "w");
	luaU_fprint(tmp, closure->p, 1);
	fclose(tmp);
	printf("executed lua instructions count %d\n", TiValue::lua::lib::get_lua_state_instructions_executed_count(scope.L()));
}
*/
...
GTEST(TEST_GLUA_CALL_CONTRACT_API_PERFORMANCE)
{
	...
	CHECK(TiValue::lua::lib::compile_contract_to_stream("tests_typed/empty_correct_contract.glua",
	...
	{
		TiValue::lua::lib::GluaStateScope scope_for_run;
		auto address = std::string(STREAM_CONTRACT_PREFIX) + std::to_string((intptr_t)stream.get());
		if (LUA_OK != TiValue::lua::lib::execute_contract_api(scope_for_run.L(), address.c_str(), "start", "testarg", nullptr))
		{
...
GTEST(TEST_JSON_LIB)
{
	...
	TiValue::lua::lib::GluaStateScope scope;
	std::vector<std::string> errors;
	GCHECK(compile_and_run_with_no_file(scope.L(), "tests_typed/test_json_lib.glua", true, &errors));
...
{
	lua_State *L = TiValue::lua::lib::create_lua_state();
	TiValue::lua::lib::add_global_c_function(L, "ADD", add);
	GCHECK(compile_and_run(L, "tests_lua/test_c_function.lua") == true);
	printf("executed lua instructions count %d\n", TiValue::lua::lib::get_lua_state_instructions_executed_count(L));
...
{
	lua_State *L = TiValue::lua::lib::create_lua_state();
	TiValue::lua::lib::register_module(L, "hello", luaopen_hello);
...
{
  TiValue::lua::lib::GluaStateScope scope;
  TiValue::lua::lib::compilefile_to_file("tichain_lua_modules/tichain_contract_contract_transfer_demo.lua", "tichain_lua_modules/tichain_contract_contract_transfer_demo", nullptr);
...
GTEST(TEST_TOSTRING)
{
  ...
  GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(), "b") == "table: 0");
  GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(), "c") == "{\"b\":\"userdata\",\"c\":{\"a\":1,\"b\":\"hi\"},\"name\":1}");
  printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
}
...
GTEST(TEST_STORAGE_PERFORMANCE)
{
  ...
  auto stream = std::make_shared<GluaModuleByteStream>();
  TiValue::lua::lib::compile_contract_to_stream(filename, stream.get(), nullptr, nullptr, true);
  glua::util::TimeDiff time_diff(true);
  TiValue::lua::lib::GluaStateScope scope1;
  TiValue::lua::lib::run_compiled_bytestream(scope1.L(), stream.get());
  TiValue::lua::lib::execute_contract_init(scope1.L(), "tmp", stream.get(), "abcd", nullptr);

  intptr_t pt = (intptr_t)stream.get();
  std::string name = STREAM_CONTRACT_PREFIX + std::to_string(pt);
  // TiValue::lua::lib::GluaStateScope scope2;
  // TiValue::lua::lib::run_compiled_bytestream(scope2.L(), stream.get());
  TiValue::lua::lib::execute_contract_api_by_stream(scope1.L(), stream.get(), "start", "abcd", nullptr);
...
GTEST(TEST_TYPED_TEST_MARKET)
{
    ...
    for (size_t i = 0; i < cycle_count; ++i)
    {
      ...
      if (!TiValue::lua::lib::compile_contract_to_stream(src_filename.c_str(), stream.get(), error, nullptr, true))
      {
      }
    }
...
GTEST(TEST_TYPED_TOJSONSTRING)
{
	...
	GCHECK_EQUAL(errors.size(), 0);
	GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(), "t1") == "[1,2,\"abc\",{\"age\":24,\"name\":\"hello\"}]");
	GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(), "t2") == "{\"m\":234,\"n\":123,\"ab\":1}");
	GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(), "t3") == "[1,3,6,5]");
	GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(), "t5") == "{\"2\":123}");
	printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
}
...
GTEST(TEST_ERROR_CHECK)
{
	...
	TiValue::lua::lib::add_global_c_function(L, "test_error", test_error);
...
{
	TiValue::lua::lib::GluaStateScope scope;
	int compile_status = TiValue::lua::lib::compilefile_to_file("tests_lua/test_error_syntax.lua", "error_syntax.out", nullptr);
	if (compile_status)
	{
...
GTEST(TEST_LOAD_CONTRACT)
{
	...
	TiValue::lua::lib::execute_contract_api(scope.L(), "demo2", "start", "tichain", nullptr);
	printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
}
...
GTEST(TEST_NO_RETURN_CONTRACT)
{
	...
	GCHECK(errors.at(0).find("this tichain contract not return a table") != std::string::npos);
	printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
}
...
GTEST(TEST_UNDUMP_BYTECODE)
{
	...
	CHECK(TiValue::lua::lib::undump_from_bytecode_file_to_file(L, "test4.out", tmp));
	fclose(tmp);
	...
	auto decompiled_result = TiValue::lua::lib::decompile_bytecode_file_to_source_code(L, "test4.out", "test4", decompile_error);
	printf("decompiled result is:\n%s\n", decompiled_result.c_str());
	auto disassemble_result = TiValue::lua::lib::disassemble_bytecode_file(L, "test4.out", "test4", nullptr);
...
GTEST(TEST_EXECUTE_CONTRACT_API_BY_ADDRESS)
{
    ...
    TiValue::lua::lib::GluaStateScope scope;
    // TiValue::lua::lib::execute_contract_api_by_address(scope.L(), "1234", "start", "abc");
}
...
GTEST(TEST_CHECK_CONTRACT_BYTECODE)
{
	...
	lua_State *L = TiValue::lua::lib::create_lua_state();
	LClosure *closure = TiValue::lua::lib::luaU_undump_from_file(L, "test4.out", "test4");
	GCHECK(closure != nullptr);
	FILE *tmp = fopen("test4.bytes.out", "w");
	luaU_fprint(tmp, closure->p, 1);
	fclose(tmp);
	if (!TiValue::lua::lib::check_contract_proto(L, closure->p))
	{
...
GTEST(TEST_STOP_VM)
{
	...
	printf("executed lua instructions count %d\n", TiValue::lua::lib::get_lua_state_instructions_executed_count(L));
	GCHECK(TiValue::lua::lib::get_lua_state_instructions_executed_count(L) < 50);
	TiValue::lua::lib::close_lua_state(L);
}
...
GTEST(TEST_STORAGE)
{
	TiValue::lua::lib::GluaStateScope scope;
	auto stream = std::make_shared<GluaModuleByteStream>();
	TiValue::lua::lib::compile_contract_to_stream("tests_typed/test_storage.glua", stream.get(), nullptr, nullptr, true);
	...
	printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
	TiValue::lua::lib::commit_storage_changes(scope.L());
}  

GTEST(TEST_DEBUGGER)
{
	TiValue::lua::lib::GluaStateScope scope;
	TiValue::lua::lib::GluaStateScope repl_scope;
	bool run_repl = false;
	if (run_repl)
	{
	...
	//TiValue::lua::lib::compilefile_to_file("tests_lua/test_debugger.lua", "test_debugger.bc", nullptr);
	//LClosure *cl = TiValue::lua::lib::luaU_undump_from_file(scope.L(), "test_debugger.bc", "test_debugger");
...
GTEST(TEST_WRONG_BYTECODE)
{
	...
	TiValue::lua::lib::GluaStateScope scope;
	std::vector<std::string> errors;
	bool run_status = TiValue::lua::lib::run_compiledfile(scope.L(), "wrong_byte_code.bytes");
	GCHECK(!run_status);
	printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
}
...
GTEST(TEST_TYPED_TEST_STREAM_TYPE)
{
	...
	GCHECK_EQUAL(errors.size(), 0);
	GCHECK_EQUAL(TiValue::lua::lib::get_global_string_variable(scope.L(), "t1"), "4");
	GCHECK_EQUAL(TiValue::lua::lib::get_global_string_variable(scope.L(), "t2"), "123979899");
	printf("executed lua instructions count %d\n", scope.get_instructions_executed_count());
}
...
GTEST(TEST_TYPED_PAIRS)
{
	...
	GCHECK_EQUAL(errors.size(), 0);
	GCHECK(TiValue::lua::lib::get_global_string_variable(scope.L(),
...
int simple_tests()
{
	auto L = TiValue::lua::lib::create_lua_state();
	
	TiValue::lua::lib::register_module(L, "hello", luaopen_hello);

	printf("lua state instructions limit count: %d\n", TiValue::lua::lib::get_lua_state_instructions_limit(L));
	TiValue::lua::lib::set_lua_state_instructions_limit(L, 100);

	TiValue::lua::lib::add_global_c_function(L, "ADD", add);
	TiValue::lua::lib::add_global_c_function(L, "test_error", test_error);
	TiValue::lua::lib::add_global_c_function(L, "check_equal", unit_test_check_equal_number);
	...
	{
		TiValue::lua::lib::close_lua_state(L);
		L = TiValue::lua::lib::create_lua_state();
	}
	const char *to_compile_filename = "tests_lua/test3.lua";
	compile_status = TiValue::lua::lib::compilefile_to_file("tests_lua/test3.lua", "tichain_lua_modules/tichain_contract_demo2", nullptr);

	auto stream = std::make_shared<GluaModuleByteStream>();					 
	if (TiValue::lua::lib::compilefile_to_stream(L, to_compile_filename, stream.get(), nullptr))
	{
	...
	TiValue::lua::lib::run_compiledfile(L, "tichain_lua_modules/tichain_contract_demo2");	// may not affect?
	printf("executed compiled file done\n");

	TiValue::lua::lib::compilefile_to_file("tests_lua/test4.lua", "test4.out", nullptr);
...
{
    global_glua_chain_api = new TiValue::lua::api::DemoGluaChainApi();
	...
	{
		TiValue::lua::lib::GluaStateScope L_scope;
		L_scope.add_system_extra_libs();
		// L_scope.change_out_file(fopen("REPL.out", "w"));
		// L_scope.change_err_file(fopen("REPL.err", "w"));
		TiValue::lua::lib::start_repl_async(L_scope.L());
		while (TiValue::lua::lib::check_repl_running(L_scope.L()))
		{
